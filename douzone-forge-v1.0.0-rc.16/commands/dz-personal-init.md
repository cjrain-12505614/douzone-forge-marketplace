...
### 7. git 신원 + 비밀 스캔 훅 자동 설정 (v0.5.0 신설)

§5.1 lookup 결과(이름·Amaranth 10 ID·본부)로 워크스페이스 git 신원과 비밀 스캔 훅을 멱등 설정. **user.name·이메일은 Amaranth 10 ID 기반 — 사번 아님** (SSoT: [`Forge-GitLab-운영가이드.md`](../../규칙/프로세스/Forge-GitLab-운영가이드.md) §2 · [`사용자ID-매핑.md`](../../규칙/프로세스/사용자ID-매핑.md)).
...
- 이로써 **clone → 플러그인 설치 → `dz-personal-init` 한 번**으로 신원·비밀훅·개인영역이 모두 정착.
