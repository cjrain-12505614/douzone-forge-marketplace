...
case "$FILE_PATH" in
  */규칙/프로세스/*.md)
    ...
    ;;
  */_plugin/douzone-forge/rules/*.md)
    PROTECTION_AREA="플러그인 rules"
    ;;
  */_plugin/douzone-forge/hooks/*.sh)
    PROTECTION_AREA="플러그인 hooks"
    ;;
  */_plugin/douzone-forge/skills/*/SKILL.md)
    PROTECTION_AREA="플러그인 skills (SKILL.md)"
    ;;
  */_plugin/douzone-forge/commands/*.md)
    PROTECTION_AREA="플러그인 commands"
    ;;
  */_plugin/douzone-forge/.claude-plugin/plugin.json)
...
⚠️  [rules-protection-check] 관리자 영역 진입 검출 — $PROTECTION_AREA
    파일: $FILE_PATH
    → Douzone Forge 관리자 영역. 운영 주체: Forge 관리자
