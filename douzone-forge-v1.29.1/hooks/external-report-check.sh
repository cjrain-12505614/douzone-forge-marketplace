...
#
# 배선 상태: 미배선(고아) — 의도적 비배선, 등록 누락 결함 아님 (2026-07-02 판정)
#   구조 사유: 훅 JSON 프로토콜이 아닌 평문 stdin/인자 검사기 — Claude Code 훅으로 배선 불가.
#         외부 보고 산출(원피스 주입·메일 발신 등)은 Write/Edit 파일 경로로 특정되지 않아 이벤트 대상도 모호.
#   용도: dz-external-report 스킬의 보조 검사기(SKILL.md 참조 자산) — 본문 텍스트를 직접 파이프해 검사.
#         배선하려면 프로토콜 개조 + 대상 판별 기준 신설 필요 (HOOKS.md §2 검토 대기).
