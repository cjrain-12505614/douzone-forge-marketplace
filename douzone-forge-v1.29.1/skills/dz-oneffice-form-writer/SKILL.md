...
## 0. 원피스 DOM 배관 = OF 라이브러리 인용 (2026-07-14, T4 마이그레이션)

이 스킬의 **원피스 문서 객체 모델(DOM, 문서 객체 모델) 조작**(중첩 iframe[문서 안에 끼운 또 다른 문서 틀] 접근·편집모드 판정/전환·저장 등 배관)은 정본 스킬 [`dz-oneffice-kit`](../dz-oneffice-kit/SKILL.md)의 **OF 라이브러리**(ONEFFICE kit, 원피스 자동화 공용 함수 모음)를 사용한다. 셀렉터(요소를 집는 규칙)·좌표·프리셋을 한 곳(`OF.SEL`/`OF.COORD`/`OF.PRESET`)에만 두어 스킬마다 값이 어긋나는 드리프트를 없앤다.

**실행 시**: `dz-oneffice-kit` SKILL.md §1의 OF 코드블록을 `javascript_tool`로 원피스 탭에 **1회 주입**(`window.OF` 상주) → 이후 `OF.*` 호출.

**이 스킬이 실제로 쓰는 OF 함수** (배관만 — 업무 콘텐츠는 아래 §8·§9의 SCHEMA·wrap 7종이 워크스페이스 SSoT로 잔류):

| OF 함수 | 이 스킬에서의 용도 |
|---|---|
| `OF.editorDoc()` | 안쪽 dzeditor 문서 접근(`#open_oneffice_body_iframe → #dzeditor_0`) — injectRow/addRow의 셀 탐색 기준 |
| `OF.enterEdit()` | 재열람 양식이 읽기(R)모드면 "편집" 토글 JS `.click()`으로 편집 전환(신뢰 이벤트 불필요) |
| `OF.assertEditable()` | 셀 주입·저장 전 편집 가능 확보(읽기모드면 enterEdit 시도 후 재확인, 그래도 안 되면 throw) |
| `OF.save()` | 2단계 저장(편집 전환 폴링 → 저장버튼 클릭 → 탭 제목 판정 → 실패 시 신호 반환) |

> **경계**: OF는 DOM 배관만. 이 스킬 고유의 SCHEMA·wrap 7종·표별 룰(§4·§8·§9)은 워크스페이스 SSoT `규칙/프로세스/원피스-작성방식-선택-자동화-표준.md`가 정본이며 OF로 옮기지 않는다. dirty(변경 감지) 유발용 실제 키입력·실제 마우스 저장 폴백은 호출부 `computer`(Chrome 확장) 도구가 수행한다(JS로 캡슐화 불가).
...
### Step 6 — 저장 (OF 2단계 정본, 2026-07-14 T4 전환)

form-writer는 **재열람한 기존 표준 양식**을 편집하므로 아래 순서를 따른다:

1. **편집모드 확보 + 셀 주입** — `OF.assertEditable()`로 편집 가능 확보(재열람 문서는 읽기[R]모드로 열리므로 내부에서 `OF.enterEdit()`가 "편집" 토글을 JS `.click()`으로 전환). 단 **enterEdit 반영은 ~2초 지연**되므로, 재열람 문서 첫 주입은 `OF.enterEdit()` 호출 후 `OF.isEditable()` 폴링 대기 → 그다음 `injectRow`로 셀 주입.
2. **dirty(변경 감지) 유발** — 문서 객체 모델(DOM) 직접 수정만으로는 dirty가 서지 않으므로, 주입한 셀에 커서를 두고 호출부 `computer`(Chrome 확장) 도구로 **실제 키입력 1글자 후 Backspace**(writer 실측: 실제 키입력은 dirty를 올려 저장을 통하게 함). 합성 이벤트(`dispatchEvent`)는 `isTrusted=false`라 무효 가능성 — 실제 키입력 권장. **이 dirty 유발은 JS로 캡슐화 불가라 OF 밖 호출부 책임.**
3. **저장** — `OF.save()` 호출. 내부에서 ① 읽기모드면 `OF.enterEdit()` 편집 전환 후 최대 3초 폴링 → ② 저장버튼(`#TB_FILE_SAVE_REMOTE_0`) JS `.click()`(저장 API `one001A17` 호출) → ③ 탭 제목 "데이터를 저장하고 있습니다." 전환으로 저장 발생 판정.
4. **실패 시** — `OF.save()`가 `{needsPhysicalClick:true}` 또는 `{checkoutConflict:true}`를 신호하면(async 반환이 `javascript_tool`에서 비어 올 수 있으니 **탭 제목 전환을 직접 확인**), = 체크아웃 경쟁("다른 창에서 편집 중" 팝업) 또는 dirty 미유발. 다른 편집창을 먼저 정리하거나 호출부 `computer` 실제 마우스로 저장버튼 클릭 폴백.

> **OF 정본 사용 권장.** 위 절차는 [`dz-oneffice-kit`](../dz-oneffice-kit/SKILL.md)의 `OF.enterEdit`/`assertEditable`/`save`를 인용한다. 아래는 되돌림 안전판용 폴백 사본(OF 미주입 상황용, 1 마이너 버전 병행) — OF 없이도 같은 동작을 재현할 수 있도록 인라인 스니펫 취지를 남긴다:
> - 편집 전환: `#open_oneffice_body_iframe` 내부 문서의 `.dze_style_editmode_toolbar_normal`(텍스트 "편집") `.click()`
> - 저장: `#TB_FILE_SAVE_REMOTE_0` `.click()` → 탭 제목 "데이터를 저장하고 있습니다." 감지
>
> ⚠️ **form-writer 자체 실측은 미완** — 위 절차는 writer(자유 문서) 실측 + OF 키트 §3.5 실측을 form-writer(양식 셀 주입)에 적용한 설계다. **다음 사이클에 form-writer로 직접 검증** 후 확정. `dzeditor.save()`·`setDirty` 등 순수-JS dirty 해법(§6.1)은 여전히 미검증. 상세: forge `참고자료/리포트/2026-07-13-원피스-사내망-API규명-점검.md` §7.
...
| 3 | 서버 저장 자동화 — 부분 규명 (2026-07-14) | 재열람 양식은 읽기모드 → 편집 토글 JS 전환 + 셀 실제 키입력 1글자로 dirty 유발 후 JS 저장(`one001A17`). writer 실측 기반이며 form-writer 직접 검증은 미완 (Step 6·§6.1) |
...
> **실측 진전 (2026-07-14, writer 세션)**: 후보 2(**실제 키입력** 1글자→dirty 유발)는 writer 실측에서 저장 성공을 확인 — **유력**. 단 form-writer 셀 주입 맥락 직접 검증은 미완. 후보 4의 저장 함수는 **`one001A17 updateDocumentContent`**로 규명(사내망 위키+실측). 합성 이벤트(후보 3)는 `isTrusted=false`라 무효 가능성. 상세: forge `참고자료/리포트/2026-07-13-원피스-사내망-API규명-점검.md` §7.
...
> **iframe 접근 배관은 OF 정본 사용 권장** — 아래 `injectRow`·`addRow`의 `document.getElementById('open_oneffice_body_iframe').contentDocument.getElementById('dzeditor_0').contentDocument` 인라인 iframe 체인은 [`dz-oneffice-kit`](../dz-oneffice-kit/SKILL.md)의 `OF.editorDoc()`로 대체할 수 있다(중첩 iframe 진입 정본, `|| contentWindow.document` 폴백 포함). 아래 인라인 체인은 **되돌림 안전판용 폴백 사본**(OF 미주입 상황용, 1 마이너 버전 병행)으로 삭제하지 않고 남긴다. wrap 7종(SCHEMA·정렬·블릿·아마링크)은 이 스킬 고유 업무 콘텐츠이므로 OF로 옮기지 않는다.
...
| 2026-07-14 | dz-oneffice-kit OF 인용 전환 (T4 마이그레이션) — §0 OF 사용 안내 신설(editorDoc·enterEdit·assertEditable·save 인용), Step 6 저장을 OF 2단계 정본으로, 인라인 iframe 체인·편집/저장 스니펫은 폴백 사본으로 병행 유지. SCHEMA·wrap 7종·표별 룰은 워크스페이스 SSoT 잔류 |
