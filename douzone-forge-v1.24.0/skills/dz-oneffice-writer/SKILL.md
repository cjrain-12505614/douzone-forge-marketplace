...
) → {resultCode, resultData:{fileUID, docUID, editorUrl, folderUID, name, docProperty, eagerApplied}}
# folder_uid 비우면 내문서함 루트(PR_ROOT). HTML은 <dze_doc_property> 래퍼 없이 순수 본문만.
# 문서 옵션(생성 시점 확정 — 브라우저 문서설정 불필요, 실측 2026-07-22):
#   paper  = A4(기본)·A3·B5·LETTER 또는 "가로,세로"(mm)
#   margin = normal(20mm 기본)·narrow(10mm)·wide(30mm) 또는 "상,하,좌,우,머리말,꼬리말"
#   onepage= True 면 단일 페이지(웹페이지형 긴 문서)   landscape= True 면 가로
```

### ⚠️ 폭·이미지 — 라이브 실측 확정 (2026-07-23)

**① 본문 폭은 지정하지 마라.** 12조합(A4·A3·B5·LETTER × 좁게·보통·넓게) 전건 실측 결과,
주입 루트에 폭을 **안 주면 지면을 정확히 채운다**. `width:1047px !important` 같은 픽셀 고정은
넣을 필요가 없고, 인쇄·PDF·배율에서 깨지는 원인이다(v0.4.0 판정과 동일).

| 용지 | 좁게(10mm) | 보통(20mm) | 넓게(30mm) |
|---|---|---|---|
| A4 | 718 | 642 | 566 |
| A3 | **1047** | 971 | 895 |
| B5 | 590 | 514 | 438 |
...
**폭이 좁게 나오면 폭을 강제하지 말고 여백 설정을 의심하라** — A3에서 971px이면 여백이
보통(20mm)으로 적용된 것이지 폭 결함이 아니다. 계산은 `OF.pageMetrics()`(kit) 참조.

**② 이전 판 HTML 재사용 시 문서옵션 함정** — 본문에 `<dze_doc_property>`가 남아 있으면
종전에는 용지·여백 파라미터가 **조용히 무시**됐다(A3·좁게를 지정했는데 20mm로 렌더되는 실사고).
2026-07-23부터 **옵션을 명시하면 본문의 것을 교체**하고 응답 `docProperty`에 결과를 표기한다.
넘기는 HTML에는 `<dze_doc_property>`를 넣지 않는 것이 여전히 원칙이다.

**③ 이미지는 즉시 로딩이 필수** — 편집기는 `loading` 없는 `<img>`에 `lazy`를 자동으로 붙이고,
그 결과 **외부 주소 이미지가 열람 화면에 뜨지 않는다**(첫 화면 안이든 밖이든). `eager_images=True`
(기본)가 자동으로 `loading="eager"`를 부여하므로 보통은 신경 쓸 것이 없다. 직접 HTML을 쓸 때도
`<img ... loading="eager">`를 붙인다. 영구 보존이 필요하면 **내장 base64**를 쓴다(본문 4MB까지
실측 확인, 크기 지정은 `width`/`height` 속성·인라인 `style` 모두 보존).
PDF 출력은 서버 렌더라 지연 지정과 무관하게 이미지가 모두 포함된다 — **화면 열람 한정 문제**다.
...
> 종전 폴백 사유였던 **단일페이지·회의록 골격(A4 단일페이지 좁은여백)은 이제 트랙 A로 처리한다** —
> 문서 옵션이 생성 파라미터로 지정되기 때문(v0.6.1). 브라우저 문서설정 팝업 조작이 필요 없다.
...
> **v0.6.1 변천사 (2026-07-22 문서 옵션 + 크롬 자동 표시)**: `create_oneffice_doc` 에 **문서 옵션**
> (`paper`·`margin`·`onepage`·`landscape`) 파라미터 추가 — 용지(A4·A3)·여백(좁게 10/보통 20/넓게 30mm)·
> **단일 페이지**·가로를 **생성 시점에 확정**한다(dze_doc_property 주입이 그대로 저장됨을 라이브 실측).
> 이로써 종전 폴백 사유였던 **단일페이지·회의록 골격이 트랙 A로 처리 가능**해져 트랙 B 사유에서 제외.
> 또한 생성 후 **편집기를 크롬으로 항상 열기**를 기본 동작으로 하고 **추가 검증은 매번 하지 않음**을
> 명시(차민수 수석 지시).
>
...
- §4 ONEFFICE 전용 필수 CSS **4건** (`<style>` outline 제거 + **인라인 `outline:none`(트랙 A 필수)** + 라이트 테마 + min-height 해제)

> ⛔ **모드 C(평면 주입)·트랙 A 형식 요건 — 인라인 `outline:none`** (실측 2026-08-14).
> 편집기가 `td`·`th`·지면 영역의 **직계 자식** `div`·`pre` 에 회색 점선을 얹고 **읽기모드에도 남는다.**
> `<style>` 을 쓸 수 있는 모드 B 는 `* {outline:none !important;}` 로 덮이지만, `style` 태그가 없는
> 모드 C·트랙 A 는 **각 `div`·`pre` 의 인라인 `style` 에 `outline:none` 이 있어야** 막힌다.
> 편집기 규칙에 `!important` 가 없어 인라인이 항상 이긴다. 일괄 주입은 `OF.outlineGuard(html)`(kit §1, 멱등).
...
> ⚠️ **평문 링크 vs 아마링크 '카드'** — 위처럼 탭 URL 을 `<a href>` 에 넣으면 **평문 하이퍼링크**다.
> 파란 **아마링크 카드**(📄 아이콘 + `dze_amalink`)로 넣으려면 [`dz-oneffice-kit`](../dz-oneffice-kit/SKILL.md) §5
> 의 완성 앵커(`OF.amalinkAnchor`)를 쓴다 — `resolve_amalink` 로 제목·`?token=` 공유링크·moduleGbn 을
> 얻어 앵커를 만든다. **convertText 마크업·cmd+v 붙여넣기는 렌더 안 됨**(카드로 변환 실패). 트랙 A(헤드리스
> `create_oneffice_doc`) 발행도 동일하게 완성 앵커를 본문 HTML 에 직접 넣는다.
...
- **읽기모드에서 카드/섹션에 점선(dashed outline)이 보임** → 편집기 규칙
  `:is(.dze_page_main,…,td,th) > div:not(.dze_editable_div)` 와 `td>pre`·`th>pre` 가 얹는 선이다.
  실측(2026-08-14) 대상은 사실상 **표 셀 안에 직접 넣은 `div`** 이며 **읽기모드에도 남는다.**
  - **모드 B(`<style>` 보존)** → Step 4 Python 스니펫에 `* { outline: none !important; }` 가 포함됐는지 확인.
    즉시 수정은 Step 8 JS 스니펫으로 편집모드에서 `<style>` 주입 후 재저장.
  - **모드 C(평면)·트랙 A(헤드리스)** → `<style>` 을 쓸 수 없으므로 **인라인** 이 유일한 수단이다.
    본문 HTML 을 `OF.outlineGuard(html)`(kit §1) 에 통과시켜 각 `div`·`pre` 에 `outline:none` 을 넣는다.
    편집기 규칙에 `!important` 가 없어 인라인이 이긴다. 확인은 발행 후 dashed 개수 계수 0.
