...
description: >
  ...
  Forge 15 업무별 서브에이전트 중 디자인 시스템 영역 담당.
version: 0.1.0
---

# PRISM — 프리즘 (디자인 시스템)

Forge 운영 15 업무별 서브에이전트 중 **디자인 시스템** 영역 전문 에이전트.
...
- SSoT (표준 정의): `규칙/프로세스/에이전트-구성-역할-정의.md`(forge) §3 PRISM 행
- 디스패치 표준: [`dz-agent-dispatch SKILL`](../dz-agent-dispatch/SKILL.md)
- 호환 매트릭스 변천사: 기존 5-agent 의 A3 도윤 (Designer) → VECTOR 1:1 + PRISM 영역 확장
- Figma Make 검증: [`dz-figma-make-reviewer`](../dz-figma-make-reviewer/SKILL.md) cross-ref
