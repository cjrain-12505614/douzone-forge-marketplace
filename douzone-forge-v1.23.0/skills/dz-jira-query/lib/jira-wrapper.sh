#!/usr/bin/env bash
# dz-jira-query — 공통 헬퍼 (수집기 디렉토리 탐색 + 선결 조건 검증)
...
verify_collector_installed() {
  ...
  [ -d "$dir" ] \
    || { echo "[dz-jira] 수집기 디렉토리 없음: $dir"; return 1; }

  [ -f "$dir/.env" ] \
    || { echo "[dz-jira] .env 파일 없음: $dir/.env"; return 1; }

  [ -x "$dir/scripts/fetch-issues.sh" ] \
    || { echo "[dz-jira] fetch-issues.sh 실행 권한 없음"; return 1; }

  # .env 내용은 grep -q 로 존재 여부만 체크 (값 출력 금지)
  if ! grep -q "^JIRA_MODULE_FIELD_ID=customfield_" "$dir/.env"; then
    echo "[dz-jira] 🟡 JIRA_MODULE_FIELD_ID 미설정 — --find-field 모듈 필요"
  fi
