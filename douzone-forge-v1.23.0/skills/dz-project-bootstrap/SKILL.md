...
| `프로젝트/_dashboard.md` | project-tracker, status-reporter, session-protocol |
| `[모듈]/module-overview.md` | context-manager, context-analyzer |
| `_개인/sessions/{대상}/_current.md` | session-protocol, session-manager |
...
> ⛔ **세션은 모듈 안에 두지 않는다** — `_개인/sessions/{대상}/` 이다(룰 5 「모듈 외부 배치」).
> 2026-04-25 에 활성 9개 모듈의 `sessions/` 를 모듈 밖으로 이전 완료했는데 이 스킬만 따라오지 않아, 새 모듈을 만들 때마다 **폐지된 구조가 되살아나고 있었다**(2026-08-13 교정). 골격 정본: `규칙/프로세스/워크스페이스-폴더-구조-표준.md`(forge) 룰 5
...
⚠️ 모듈명은 **변수로 받는다.** `[모듈명]` 처럼 대괄호로 적으면 셸이 글롭(파일명 패턴)으로 읽어 zsh 에서는 `no matches found` 로 **아무것도 만들어지지 않는다**(2026-08-13 실측).

```bash
M="법무관리(LTE)"        # 모듈명 — 괄호·한글이 있어도 따옴표로 감싸면 안전
mkdir -p "$M"/{context,tasks/enhancements,유지보수}
mkdir -p "$M"/history/{phases,requests,plans,meetings,reports}
mkdir -p "$M"/문서/{01_신규,02_삭제가능,03_장기참조}
mkdir -p "_개인/sessions/$M"          # ⛔ 세션은 모듈 밖 (룰 5)
```
...
- `_개인/sessions/{대상}/_current.md` — 초기 세션 체크포인트

#### Step 4: 연쇄 업데이트 (Cascade R1)
- douzone-forge CLAUDE.md의 모듈 관련 섹션을 확인하고 필요 시 갱신

#### Step 5: 온보딩 안내
설계서 PDF가 있으면 `dz-context-analyzer`로 분석 시작을 권장.
스크린샷만 있으면 운영 화면 분석부터 시작.

### 유형 B: 새 프로젝트 등록

`프로젝트/`에 새 프로젝트 파일을 생성한다.

#### Step 1: ID 채번
`프로젝트/_dashboard.md`에서 최신 ID를 확인하고 아래 규칙으로 다음 순번 부여.
...
#### Step 3: 프로젝트 파일 생성
`프로젝트/_templates/project-detail.md`를 복사하여 채번 규칙에 맞는 파일명으로 생성 (신규 `PRJ-{YYYY}-{모듈코드}-{NNN}_{프로젝트명}.md` 또는 구규칙 `PRJ-YYYY-NNN_{프로젝트명}.md`).
01. 프로젝트 개요와 02. TASK 목록 초안을 작성.

#### Step 4: 연쇄 업데이트 (Cascade R2)
- `프로젝트/_dashboard.md`에 행 추가
...
| 상황 | 커맨드 |
|------|--------|
| 모듈/프로젝트 처음 시작 | `/dz-init-project` |
| 지금 뭐 해야 하지? | `/dz-guide` |
| 세션 시작 | `/dz-start-session` |
| 세션 끝날 때 | `/dz-end-session` |
| 현황 보고 | `/dz-status` |
| 의사결정 기록 | `/dz-decision` |
| 에이전트 투입 | `/dz-dispatch 하준 경쟁사 조사` |
| 교훈 기록 | `/dz-lesson 내용` |
| 설계서 분석 | context-analyzer 스킬 |
| 맥락 로드 | `/dz-load-context` |
