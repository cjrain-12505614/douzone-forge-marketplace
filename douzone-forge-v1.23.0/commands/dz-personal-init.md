...
### 3. `_개인/sessions/{대상}/` 9 폴더 신설 (Step 3.5 활성 9 모듈)
...
§5.1 lookup 결과(이름·Amaranth 10 ID·본부)로 워크스페이스 git 신원·비밀 스캔 훅·세션 동기화 기본값을 멱등 설정. **user.name·이메일은 Amaranth 10 ID 기반 — 사번 아님** (SSoT: `규칙/프로세스/Forge-GitLab-운영가이드.md`(forge) §2·§6 · `규칙/프로세스/사용자ID-매핑.md`(forge)).
...
- ST Step 3.5 산출 (활성 9 모듈 + `_개인/sessions/{대상}/` 이전)
...
- ST Step 3.5 (2026-04-25) — 활성 9 모듈 sessions/ → `_개인/sessions/{대상}/` 이전
