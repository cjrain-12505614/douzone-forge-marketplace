...
description: >
  ...
  수집하여 douzone-forge의 프로젝트 포트폴리오(프로젝트/PRJ-*/00_overview.md)와 모듈
...
- **듀얼 마운트 환경**: `<Workspace_a10 루트>`(소스, 예: `~/Workspace_a10/`)와
  `<워크스페이스 루트>`(기획·설계, 예: `~/Workspace/douzone-forge/`)가 모두 접근 가능해야 함
...
각 레포에서 실행:
```bash
cd ~/Workspace_a10/{레포}   # Workspace_a10 클론 위치는 개발자마다 다를 수 있다
git fetch --all --prune
...
- Group 1: PRJ ID
  - 구규칙: `2026-010` (연도-순번)
  - 신규규칙: `2026-LTE-001` (연도-모듈코드-순번) — 모듈 코드: `LTE/CRM/KISS/BOARD/AB/GA/PUB/DSPORTS`
...
**활성 PRJ 로드**: `프로젝트/_dashboard.md`에서 "진행 중" 테이블을
파싱해 PRJ ID 목록을 얻는다. 각 ID별로 `프로젝트/PRJ-{ID}_*/00_overview.md`의 `01.E 연계 키`
섹션을 읽어 매칭 사전을 구축.
...
| 40 ~ 69 | **수동 확인 필요** | 레포트 `## ⚠️ 수동 확인 필요` 섹션에 표로 나열. 사용자가 보고 직접 PRJ에 반영하거나 넘긴다 |
| < 40 | **미태깅** | 기존 미태깅 리스트와 동일 처리 (규약 보강 권고) |

**동점 처리**: 70점 이상 다수 PRJ가 동점 → 가장 높은 단일 신호(JIRA키 > 경로 >
담당자) 우선. 여전히 동점이면 수동 확인 필요로 강등.

> **2026-08-13 — 승인 큐 파이프라인 폐기.** 종전 40~69 구간은 `프로젝트/_mapping-queue.md`에 쌓아 `/dz-confirm`·`/dz-reject` 로 처리하는 설계였으나, **큐 파일이 한 번도 만들어진 적이 없고**(git 이력 0건) 엔진 입력인 PRJ `01.E 연계 키` 도 2026-04 폴더 압축 전환 때 사라져 성립하지 않는 상태였다. 커맨드 2종과 큐·히스토리 파일 규격을 폐기하고 **레포트 표기만 남긴다.** 경위: `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §12.3
...
`<Workspace_a10 루트>/_doc/git-daily/YYYYMMDD.md` 생성 — 덮어쓰기 (예: `~/Workspace_a10/_doc/git-daily/YYYYMMDD.md`):
...
### 법무관리 (12건)
- [PRJ-2025-001/개발-24] 사건등록 팝업 유효성 추가 ({이름}, amaranth10-lte, develop) — a1b2c3d
- [PRJ-2026-005/기획-07] WBS 재작성 반영 ({이름}, klago-ui-lte-micro, devqa) — e4f5g6h
- ...
...
## ⚠️ 수동 확인 필요 (2건) — 40~69점

스코어링 엔진 추론 결과 확신이 부족한 커밋들입니다. 아래를 보고 직접 PRJ에 반영하거나 넘기세요.

| # | SHA | 레포 | 저자 | 메시지 | 추정 PRJ | 점수 | 신호 내역 |
|---|-----|------|------|--------|---------|------|-----------|
| 1 | d9106ea | amaranth10-kiss | {이름} | 메일게시판 줌인 작업 | PRJ-2026-003 | 68 | 경로40 + 담당30 − 키워드X − 브랜치X + JIRAX = 68 (주보 v0.9.1) |
| 2 | a1b2c3d | klago-ui-crm-micro | {이름} | 상담 목록 정렬 변경 | PRJ-2026-006 | 55 | 경로40 + 담당10(기간밖) + 키워드X + 브랜치X = 50 (_(주의: 기간밖 +10))_ |
...
|---|---|---|---|---|---|
| amaranth10-kiss | develop | {이름} | 17:22 | 메일 본문 노출 임시 수정 | 7h8i9j0 |
| ... |
```
...
PRJ별로 커밋을 그룹화하여 `프로젝트/PRJ-{ID}_*/00_overview.md`의 **`## 03. 상세 진행현황`** 섹션
맨 위에 일자별 블록으로 append (최신순 유지):

```markdown
### 2026-04-18 (git-daily 자동 반영)
- **[개발-24]** 사건등록 팝업 유효성 추가 — {이름} @ amaranth10-lte develop
- **[개발-25]** 상담목록 정렬 버그 수정 — {이름} @ amaranth10-lte devqa ✅ 설계검수 진입
```

**B. 모듈 history/_timeline.md append**

각 모듈 `Amaranth10/{모듈}/history/_timeline.md`에 두 블록을 추가/갱신:

① **프로젝트 활동 요약** (PRJ 귀속 커밋만):
```markdown
## 2026-04-18 (git-daily)
- amaranth10-lte develop: 3건 ({이름} 2, {이름} 1) — PRJ-2025-001, PRJ-2026-010
- klago-ui-lte-micro devqa: 1건 ({이름}) — PRJ-2025-001 ✅ 설계검수
...
|------|------|------|------|------|----------|
| 2026-04-17 14:19 | amaranth10-board devqa 2682f550 | [UC]{이름} | 게시판 이동 시 즐겨찾기 해제 | CSA10-40261 | MAINT/CSA |
```

유지보수 로그는 **일자별 섹션이 아니라 누적 테이블**로 관리(최신 상단). PRJ 파일엔 기록하지 않음.

**C. _개인/sessions/{대상}/_current.md 업데이트**

모듈별 `_개인/sessions/{대상}/_current.md`가 존재하면 "최근 git 활동" 섹션에 요약 갱신.
...
- 저자명이 조직도에 없는 사람 → `참고자료/조직정보/더존비즈온-조직구조.md` 업데이트 권고

## 보조 스킬 연계

- 결과를 바탕으로 `/dz-status` 실행 시 "미태깅 커밋 N건 경고" 자동 포함
- `/dz-start-session` 훅에서 전일 git-daily 파일 자동 로드 (있는 경우)
- 주간 합산은 `--weekly` 옵션으로 생성 → PRJ별 주간 리포트 소스

## 규약 명문화

이 스킬이 전제로 하는 커밋 메시지 규약은 별도 가이드에 명시한다:
- 개발 가이드: `규칙/프로세스/깃-커밋-메시지-규약.md`
- 브랜치 명명 규약: `feature/PRJ-NNNN-TASK-CODE-short-desc`
- MR 제목 규약: `[PRJ-NNNN/TASK-CODE] 제목`

## 승인 큐 지속성 — ⛔ 폐기 (2026-08-13)

**v0.9.0에서 설계했던 승인 큐 파이프라인을 폐기했다.** 아래 두 파일과 두 커맨드는 더 이상 쓰지 않는다.

| 폐기 자산 | 상태 |
|---|---|
| `프로젝트/_mapping-queue.md` | **한 번도 만들어진 적 없음** (워크스페이스 git 전체 이력 추가 기록 0건) |
| `프로젝트/_mapping-history.md` | 동일 |
| `/dz-confirm` · `/dz-reject` 커맨드 | 2026-08-13 삭제 |

**왜 폐기했나** — 스코어링 엔진은 `🚧 설계 완료` 상태에서 실운영으로 넘어간 적이 없다(유일 산출물은 엔진 도입 전 v0.8.1 드라이런 1건). 게다가 엔진의 입력인 PRJ `01.E 연계 키` 섹션이 2026-04-26/27 프로젝트 폴더 압축 전환 때 사라져(원본 12건은 각 PRJ `99_archive/_original-*.md`에 보존), **큐 파일을 만들어 줘도 확정·기각 절차가 성립하지 않는다.**

**되살리려면** 아래가 선행돼야 한다 — ① `프로젝트/_templates/project-detail.md`에 `01.E 연계 키` 섹션 복원 ② 활성 PRJ 개요에 연계 키 재이식 ③ 드라이런 1회 실측.

40~69점 구간은 이제 **레포트의 「⚠️ 수동 확인 필요」 표기로만** 남는다(파일 지속성 없음).

경위·근거: `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §12.3
...
- **케이스 6 자동 초안**: 커밋만 있고 검토안·일일보고 없음 → `/dz-triage --auto-draft {JIRA키}` 자동 호출 (결정 3)
- **케이스 5 알림**: 커밋+일일보고 있지만 검토안 없음 → 알림만, 자동 초안 ❌
- **고도화 트랙 자동 스텁**: `KLAGOP1/A10D/UCAIMP` 새 키 발견 시 `Amaranth10/{모듈}/tasks/enhancements/` 스텁 생성 (결정 7 — `_dashboard.md` 등록은 사용자 승인 필요)
