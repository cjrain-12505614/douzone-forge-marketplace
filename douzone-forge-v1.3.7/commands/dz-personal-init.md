...
### 8. MCP 서버 등록 (jira) + 자격증명 안내 (v0.7.0 신설 / v0.8.0 워크스페이스 내장 전환)

**워크스페이스 내장** jira-mcp(`.jira-mcp/`)를 Claude Code(`~/.claude.json`) + Cowork(`claude_desktop_config.json`) **양쪽에 등록**한다. 본체가 워크스페이스에 있어(GitLab 클론에 포함) **플러그인 버전과 무관 — 경로가 고정**이라 플러그인을 업데이트해도 등록이 안 깨진다(구 동봉 방식의 "Could not attach" 결함 해소). Cowork 는 플러그인 `.mcp.json` 자동로드를 안 하므로(검증됨), 본 단계가 양쪽 설정 파일을 직접 써서 등록한다. (a10-mcp 등록·쿠키 자동추출은 별도 단계로 추후 확장)
...
{ command -v python3.12 || command -v python3.11 || command -v python3.10; } >/dev/null 2>&1 && echo "✅ python 3.10+" || echo "⬜ python 3.10+ 필요"
# 플러그인 버전
...
# MCP 등록 (jira)
python3 -c "import json,os; cfg=json.load(open(os.path.expanduser('~/.claude.json'))); print('✅ jira MCP 등록' if 'jira' in cfg.get('mcpServers',{}) else '⬜ jira MCP 미등록 — §8')" 2>/dev/null || echo "⬜ ~/.claude.json 확인 필요"
# 자격증명 (비밀번호 노출 없이 '채워졌는지'만)
...
> **브라우저 MCP·코워크 MCP 연결**은 Claude 가 현재 세션 도구 목록으로 판정·안내한다:
> - 브라우저 MCP: `mcp__claude-in-chrome__*` 도구 존재 → ✅, 없으면 "CoWork 앱에서 Chrome 확장 연결 필요"(a10 쿠키 자동추출용) 안내.
