...
| 40 ~ 69 | **승인 대기 큐** | 레포트 `## ⚠️ 승인 대기 매핑` 섹션에 표로 나열. 사용자는 `/dz-confirm N` 또는 `/dz-reject N`으로 결정 |
...
스코어링 엔진 추론 결과 확신이 부족한 커밋들입니다. PM 확인 후 `/dz-confirm N` 또는 `/dz-reject N`으로 결정하세요.
...
**`프로젝트/_mapping-queue.md`**

```markdown
# 승인 대기 매핑 큐

> `/dz-git-daily` 실행 결과 40~69점 구간 커밋을 수집. 최신 실행 결과로 대체됨.
> 결정: `/dz-confirm N` (자동 귀속 확정) / `/dz-reject N` (유지보수·미태깅 처리)
> 수동 귀속: `/dz-confirm N PRJ-YYYY-NNN` (다른 PRJ로 이동)

## 실행: 2026-04-18 09:15 (범위 2026-04-17)

| # | SHA | 레포 | 브랜치 | 저자 | 시각 | 메시지 | 추정 PRJ | 점수 | 신호 내역 |
|---|-----|------|--------|------|------|--------|---------|------|-----------|
| 1 | d9106ea | amaranth10-kiss | develop | {이름} | 14:22 | 메일게시판 줌인 작업 | PRJ-2026-003 | 68 | JIRA0 경로40 담당30 주보0 키워드0 브랜치0 −2 (동점감점) |
| 2 | ...  | ... | ... | ... | ... | ... | ... | ... | ... |
```

**필드 명세**:
- `#`: 큐 내부 순번 (매 실행마다 1부터 재시작)
- `SHA`: 7자리 short hash (`/dz-confirm` 인자로 사용 가능)
- `신호 내역`: 6신호 개별 점수 표기 (디버깅·규약 학습 용도)

매 `/dz-git-daily` 실행 시 이 파일은 **덮어쓰기**된다 (누적 아님). 미결 항목은
다음 실행 전까지 처리하거나, 처리 안 하면 다음 실행 시 동일 커밋에 대해 다시
큐에 올라온다.

## 학습 레이어 (v0.9.0 기본형)

**`프로젝트/_mapping-history.md`**
...
|------|-----|------|------|-------------|---------|------|------|----------|
| 2026-04-18 | d9106ea | amaranth10-kiss | {이름} | 메일게시판 줌인 | PRJ-2026-003 | 68 | auto | PRJ-2026-003 |
| 2026-04-18 | a1b2c3d | klago-ui-crm-micro | {이름} | mode:local 수정 | PRJ-2026-001 | 45 | reject | (유지보수) |
```

**결정 값**:
- `auto`: 70점 이상 자동 귀속
- `confirm`: 사용자가 `/dz-confirm`으로 승인
- `confirm-override`: `/dz-confirm N PRJ-...`로 다른 PRJ 지정
- `reject`: `/dz-reject`로 기각 (유지보수 or 미태깅으로 강등)
