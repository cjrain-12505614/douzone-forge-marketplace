...
description: >
  ...
  dz-people-index(정태 ID/사번/소속/R&R) + 진행 중 PRJ + 담당 모듈 + 트래킹 노트 +
  본인 업무현황 보고 (Phase S S-07 보강) +
  본인 _R&R.md 우선 lookup (Phase V V-07-04 — `_개인/{본인이름}_{ID}/_R&R.md`)
  자동 lookup. Phase Q-2 G50 일관 — SBUnit 한정 표현 0.
version: 0.3.1
---

# 사람 컨텍스트 (dz-people-context) — Phase Q-3 Q-15 신설 + Phase S S-07 본인 업무현황 보강 + Phase V V-07-04 _R&R.md 우선 lookup

사람 시점에서 정태 메타 + 동태 활동을 종합 lookup하는 스킬. dz-people-index(정태)와 워크스페이스 동태 데이터를 결합.
...
1. **1순위**: `_개인/{본인이름}_{ID}/_R&R.md` (ID = xlsx 13번 사원명(ID) — 사번 아님, 예: `차민수_cjrain`. v1.11.3 정정) — dz-personal-init v0.4.0 자동 정착된 본인 R&R 메타 파일
...
3. **3순위**: xlsx 13번 동적 lookup (`참고자료/조직정보/Amaranth_10_OrganizationChart_*.xlsx`)
   - 1·2순위 모두 부재 시 (예: 신규 사용자 첫 호출) — dz-people-index Skill 직접 호출

### 부하 직원 자동 식별 흐름

본인이 직책자 (Cell 리더 / Unit장 / 본부장)인 경우:
- `_개인/{본인이름}_{ID}/_R&R.md` §4 "부하 직원" 섹션이 SSoT
...
- 동작: 활성/휴직/퇴사 메타 분기 자동 인식
  ...
  - 휴직 2명 ({이름}·이승현G): "육아휴직" 표기
  - 퇴사 1명 ({이름}): "4/30 퇴사 → 5/1 archive 예정" 표기
  - Unit장 1명 ({이름}): 본인 운영자 명시

### 자비스 메모리 SSoT 인용
- 인용 SSoT: `reference_sbunit_21_member_reports.md` (자비스 자체 시스템 — Cowork agent 영역, Code 미접근)
- 정책: Claude Code는 인용만 작성 → 자비스 cascade가 일상 운영 시 자동 채움

### 의존 보강
- `skills/dz-amalink-recursive-extract/` (S-07 신설 — 4건 룰 통합)
- `skills/dz-cascade-from-report/` (R-04 + S-05 보강 — Step 0·2.5·3.5)
...
- `skills/dz-people-index` (G8) — 정태 메타 (필수, xlsx 13번 동적 lookup의 외부화)
...
- **`_개인/{본인이름}_{ID}/_R&R.md`** — 본인 R&R 메타 (V-07-04 신설, 1순위 lookup)

## 실행 흐름

1. 입력 ID/사번/이름 매칭 → dz-people-index 호출
2. 4 영역 lookup 동시
3. 종합 출력 (마크다운 형식)
...
| 부하 직원 종합 조회 | "{이름} 활동" |
| 신규 인원 R&R 파악 | "{ID} 종합" |
| Cell 리더 멤버 점검 | dz-personal-tracking 작성 전 사전 lookup |

## 갱신 정책

- 정태 메타 변경 시 → dz-people-index SSoT 갱신만
- 동태 활동 변경 시 → 조직 _index.md "진행 중 PRJ" 섹션 + dashboard.md 동시 갱신 (R3 캐스케이드)

## 관련 자산

- `skills/dz-people-index` (G8) — 정태 lookup (의존)
- `skills/dz-personal-tracking` (G10, Q-14) — 트래킹 노트 작성
- `commands/dz-personal-init` (Q-12) — 4계층 lookup + 팀트래킹 자동

## 변천사

- Phase Q-3 Q-15 (2026-04-27) — 본 스킬 신설 + 조직 _index.md 26명 "진행 중 PRJ" 섹션 보강
- 자비스 v1 §A8 일관 — 단기 정태 _index.md / 중기 dz-people-context 동태
- **Phase V V-07-04 (2026-05-11)** — `_개인/{본인이름_사번}/_R&R.md` 1순위 lookup 도입 (V-05 갭 1-2 해결, xlsx 13번 동적 lookup 3순위 fallback)
- **2026-07-03 (플러그인 v1.11.3)** — v0.3.0 → v0.3.1: `_개인/` 폴더명 표기 정정 `{본인이름_사번}` → `{본인이름}_{ID}` (정본 = {이름}_{ID}, ID = xlsx 13번 사원명(ID). 근거: 사용자ID-매핑.md §1 Q-01 ID 최우선 표준 + 정착 실측 `차민수_cjrain`·조직 트리 전건 {이름}_{ID} 일치. 변천사 항목 원문은 보존)


## R13 소스 연쇄 (2026-05-25 — 소스분석 레이어 편입)
<!-- r13-hook -->
본 cascade 완료 후 **소스 분석 레이어 정합**을 위해 forge 루트에서 실행한다:
- `python3 "Amaranth10/_소스분석/_tools/gen_context_graph.py"` — 담당자(커밋터) 인덱스 재생성
- `python3 "Amaranth10/_소스분석/_tools/gen_crosslinks.py"` — 연결매트릭스·각 `_source-architecture.md` 관계헤더 재생성
- (사내망) `python3 "Amaranth10/_소스분석/_tools/gen_inventory.py"` — 레포 인벤토리 갱신
