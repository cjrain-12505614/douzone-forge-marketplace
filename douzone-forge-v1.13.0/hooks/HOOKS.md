...
## 1. 배선된 훅 (plugin.json 등록 = 실제 작동, 21)
...
## 2. 고아 훅 (plugin.json 미배선, 3 — 헬퍼 1 + 구조적 예외 2)

| 훅 | 의도 트리거 | 성격 | 상태·사유 |
|---|---|---|---|
| `_forge-gate.sh` | (없음) | **헬퍼** | 리마인더 3종이 `source`로 호출하는 게이트 판정 함수. 단독 훅 아님(정상) |
| `commit-message-check.sh` | git commit-msg | 권고 | **구조적 예외** — Claude Code에 커밋 시점 이벤트 없음(배선 불가). 커밋 규약은 룰 계층에서 안내 |
