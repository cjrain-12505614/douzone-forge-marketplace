...
- **공용** (GitLab 중앙): `Amaranth10/`·`프로젝트/`·`참고자료/`·`deliverables/`·`meta/`·`조직/`
- **개인** (로컬 only): `_개인/` — 각 사용자 로컬에 본인 영역만 존재 (`_개인/팀트래킹/` 포함)
...
대안:
1. 공유가 필요한 진행 보고 → `조직/{...}/{이름_사번}/daily,weekly,monthly/` (공용)
2. 임시 메모·학습·개인 PRJ·관리자 팀 운영 분석(`_개인/팀트래킹/`) → `_개인/` (개인 — 로컬 보존)
...
- 공용/개인 경계 정책 SSoT: `규칙/프로세스/공용-개인-경계-규칙.md` (douzone-forge, ST Step 1.5)
...
- ST Step 3.5 (2026-04-25) — `_개인/sessions/{대상}/` 추가
- PT-02 (2026-04-26) — 본 Rule 신설로 정책 강제 (G7)
- 2026-05-17 — 공용 폴더명 현행화(`Amaranth10/`·`프로젝트/`·`참고자료/`) + `_개인/팀트래킹/` 공용→개인 재분류 반영 + SSoT 경로 정정
