...
### ⚠️ 경로 A의 한계 — `get_oneffice_content`는 **축약본**이다 (실측 2026-07-23)

`one001A45`가 돌려주는 content는 **편집기가 여는 원본(.onex)이 아니다.** 서버가 다음을 **제거한 채** 준다:

| 제거되는 것 | 남는 것 |
|---|---|
| `<style>` 블록 · `<div>`/`<p>` 등 구조 태그 · 인라인 `style` 속성 · `loading` 속성 · **내장 이미지(data:)의 src 값** | 본문 텍스트 · 표 · 서버 이미지(`one006A02`) 참조 |

- **적합**: 텍스트·표 맥락화, 내용 파악, 이미지 목록 추출(`download_oneffice_images`가 이 참조를 씀)
- **부적합**: 재편집용 원본 확보, 서식 보존 자산화 — **이 결과로 문서를 재구성하면 서식과 내장 이미지가 유실된다**
- 원본 확인이 필요하면 **경로 B(편집기 DOM)** 로 간다. 실측 당시 이 축약본을 원본으로 오인해
  "내장 이미지가 저장되지 않는다"고 잘못 판단한 사례가 있었다(실제로는 정상 저장됨)
...
- **서식·내장 이미지가 온전한 원본**이 필요(위 축약본 한계)
