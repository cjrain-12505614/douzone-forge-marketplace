...
프레임워크로 표준화한다. JIRA 조회는 `dz-jira` 스킬에 위임하고, 이 스킬은
**판단 로직·검토 산출물**을 담당한다.
...
5. `projects/`에 해당 이관 티켓의 PRJ 파일이 있다면 연결

## 산출물 저장 경로

### 개별 검토안

`Amaranth10/{모듈}/유지보수/{YYYYMMDD}-{JIRA키}-검토안.md`

예시:
- `Amaranth10/게시판(BOARD)/유지보수/20260420-CSA10-44921-검토안.md`
- `Amaranth10/게시판(BOARD)/유지보수/20260420-CSA10-44945-검토안.md`

### 검토안 일괄 인덱스

`Amaranth10/{모듈}/유지보수/_index.md` — 이 모듈에서 처리된 검토안 목록 (최신순).
...
- 수용 → 개발중 → 설계검수 → QA → 배포완료: **자동** (`/dz-triage-status`)
- 배포완료 → 처리완료: **수동** (`/dz-triage-close`)
- 기각·보류 전이: 수동
...
수용 후 **7일** 이내 연관 커밋 0건 → `/dz-triage-status` 가 경고 리포트에 포함.

## .forge 연계 (v0.2.0)

진행 중 검토안(수용·개발중·설계검수·QA)은 `dz-forge-bridge` 에 의해
...
1. **JIRA 원본 조회** — `dz-jira` 스킬 활용, `key in (...)` 애드혹 JQL로 description·comments·components·fixVersions·labels·assignee 획득
...
6. **저장 + 인덱스 갱신** — `Amaranth10/{모듈}/유지보수/` 저장 + `_index.md` 행 추가
7. **사용자에게 요약 보고** — 판정·신뢰도·다음 액션 3~5줄 + 검토안 파일 링크

## 사용 예시

```
/dz-triage CSA10-44921                     # 단일 건
/dz-triage CSA10-44921,CSA10-44945         # 복수 건
/dz-triage CSA10-44921 --module 게시판      # 모듈 수동 지정
/dz-triage --list-recent 게시판 30          # 게시판 모듈 최근 30일 미판정 접수 나열
```

## 치명적 주의

- **JIRA 이슈 상태 변경 금지** — `/dz-jira`는 조회 전용. 상태 전이·댓글은 PM이 JIRA UI에서 수동 처리
...
- `dz-jira-classifier` — JIRA 트랙 분류 (v0.2.0 신규)
- `dz-jira-query` — JIRA 조회
- `dz-git-daily` — 과거 해결 커밋 검색 + 3-way 조인
- `dz-daily-digest` — 일일보고 캐시 조회 (v0.2.0 신규)
- `dz-forge-bridge` — `.forge/` 주입 (v0.2.0 신규)
- `dz-task-manager` — `Amaranth10/{모듈}/tasks/` 계층 관리
- `dz-history-manager` — 의사결정 기록

## 관련 파일

- 검토안 템플릿: [`templates/triage-review.md`](templates/triage-review.md)
- 커맨드: [`../../commands/dz-triage.md`](../../commands/dz-triage.md)
