...
4. **저장된 문서를 재열람 후 편집·저장할 때는 JS `.click()` 저장이 조용히 무시된다**
   (v0.5.0 실측, 2026-07-02). `TB_FILE_SAVE_REMOTE_0`.click()·`Cmd+S` 모두 no-op —
   **새 문서 생성 세션에서만 JS click 저장이 통한다.** 재편집 세션의 저장은 반드시
   **실제 마우스 클릭**(`computer` left_click, 리본 좌측 💾 저장 아이콘)으로 수행.
...
> **⚠️ 재편집 세션은 JS click 무효 (v0.5.0, 2026-07-02 실측)** — 위 JS 클릭은
> **새 문서 생성 세션에서만** 동작한다. 저장된 문서를 새로고침·재열람 후 편집한
> 세션에서는 조용히 무시된다(네트워크 요청 0건). 이 경우 **`computer` left_click
> 으로 리본 좌측 💾 저장 아이콘을 실제 클릭**할 것 (치명적 함정 4 참조).
> 판정은 (c)의 탭 제목 전환 여부 — 전환이 안 보이면 저장이 나가지 않은 것.
...
- **저장은 됐는데 `.noext` 로 보임** → Step 0 에서 "ONEFFICE 워드" 버튼을 거치지
  ...
  참조.
- **재편집 세션에서 저장을 눌러도 서버에 반영 안 됨 (탭 제목 전환 없음·요청 0건)**
  → 저장된 문서를 재열람한 세션에서는 JS `.click()`·`Cmd+S` 저장이 무시된다.
  `computer` left_click 으로 리본 💾 저장 아이콘을 **실제 클릭**할 것. 판정 =
  탭 제목 "데이터를 저장하고 있습니다." 전환. 저장 후 a10 MCP 조회 API 로 검증하면
  캐시 구버전 때문에 "실패"로 오판할 수 있음 — 새로고침 후 DOM 으로 검증.
  (2026-07-02 v0.5.0 실측 — forge `참고자료/리포트/2026-07-02-원피스-재편집-저장-함정-진단.md`)
