---
name: dz-amalink-recursive-extract
description: >
...
R-07 `dz-residual-audit` Skill 5축 (S-06 확장) 호출 — 헤더 표준 미준수 검출.
...
Step 5 — R-04 cascade 트리거 (보고 시점에)
  • dz-cascade-from-report Skill 호출
  • 본인 조직/PRJ/모듈/sessions 4건 cascade
    ↓
Step 6 — R-07 검증 (선택, 검증 모드)
  • dz-residual-audit 5축 dry-run
  • 헤더 미준수 카운트 보고
```
...
```
입력: "{이름} 본인 업무현황 추출"
→ dz-people-context lookup (본인 _index.md "본인 진행 보고" 섹션)
...
- `skills/dz-cascade-from-report/` (R-04 + S-05 보강 — Step 0·2.5·3.5 통합)
- `skills/dz-people-context/` (S-07 보강 — 본인 업무현황 lookup)
- `skills/dz-residual-audit/` (R-07 + S-06 5축 — 헤더 검증)
