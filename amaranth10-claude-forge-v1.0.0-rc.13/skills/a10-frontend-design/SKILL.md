...
Anthropic 공식 `frontend-design` 플러그인의 핵심을 amaranth10-claude-forge 플러그인 안으로 흡수한 미러 스킬. **Cowork 환경에서 원본 frontend-design 직접 호출이 불가한 갭 (TASK-S-05 핵심) 의 폴백 수단** 으로 신설.
...
- ONEFFICE 문서 주입 (편집모드 HTML 삽입) 이 핵심인 작업은 `a10-html-oneffice-builder` 또는 `a10-oneffice-writer` 호출 — 본 스킬과 혼동 금지
...
- 본 미러는 원본 핵심 보존을 우선으로 하되, 한국어 풀이 · 더존 컨텍스트 보강은 본 플러그인 (amaranth10-claude-forge) 자체 작업으로 추가
...
_본 SKILL.md 는 Anthropic `frontend-design` 플러그인 핵심을 amaranth10-claude-forge 플러그인 안에 미러한 폴백 수단. 원본 라이선스 준수. 2026-05-12 차민수 수석 옵션 B 결재 신설._
