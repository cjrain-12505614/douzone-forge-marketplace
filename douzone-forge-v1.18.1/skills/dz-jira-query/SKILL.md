...
description: >
  ...
  ({이름} 책임 배포, 2026-04-17) 수집기를 래핑하여 세션 중 애드혹 JQL 조회, 프리셋
  실행, 일일 배치 결과 로드를 지원한다. 조회 전용 — 이슈 생성·수정·코멘트는 정식
  MCP 전환 이후 과제.
version: 0.1.0
---

# dz-jira-query — JIRA 이슈 조회 스킬

## 목적

사내 JIRA(`jira.duzon.com:8080`)의 Amaranth 10 / Alpha 계열 프로젝트 미해결 이슈를
`모듈` 커스텀 필드 기준으로 수집·요약한다. 2026-04-17 {이름} 책임이 배포한
...
`dz-git-daily` 스코어링 엔진(v0.9.0+)이 커밋 메시지 내 이 패턴을 인식해
...
```
[dz-jira] JIRA 수집기 PoC가 설치되어 있지 않습니다.

설치 방법:
1. douzone-forge/프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/amaranth10-jira-collector.tar.gz 를
   원하는 위치(권장 예시: ~/Workspace/amaranth10-workspace/_workspace/jira/ — 다른 폴더도 무방, env로 지정) 에 해제
...
운영 가이드: 프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/jira-integration-guide.html
```
...
목록 조회: `/dz-jira --list` → 프리셋 8종과 각 프리셋의 요약 JQL 출력.
...
```
/dz-jira --jql 'project = CSA10 AND "모듈" = 업무관리 AND created >= -14d'
```
...
```
/dz-jira --read 2026-04-17/업무관리
/dz-jira --read 2026-04-17           # INDEX.md 반환
/dz-jira --read 업무관리              # 오늘자 해당 모듈
```
...
4. **수집기 경로 하드코딩 금지** — `$JIRA_COLLECTOR_DIR` 환경변수 우선, 없으면
   기본 탐색 경로 예: `~/Workspace/amaranth10-workspace/_workspace/jira/` (env 미설정 시 fallback).
...
- **PoC 원본**: [`프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/amaranth10-jira-collector.tar.gz`](../../../../douzone-forge/프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/amaranth10-jira-collector.tar.gz)
- **운영 가이드**: [`프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/jira-integration-guide.html`](../../../../douzone-forge/프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/jira-integration-guide.html)
- **PoC 해설 (forge)**: [`프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/README.md`](../../../../douzone-forge/프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/README.md)
- **프로젝트 상세**: [`프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/00_overview.md`](../../../../douzone-forge/프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/00_overview.md)

## 함께 쓰는 스킬

- **dz-task-manager** — JIRA 이슈를 고도화 업무 트래킹에 반영할 때
- **dz-sheets-manager** — JIRA 수집 결과를 업무관리 시트에 연동할 때
- **dz-project-tracker** — PRJ-2026-012 진행 상태 갱신 시

## 관련 Command

- **`/dz-jira`** — 이 스킬의 엔트리 커맨드 (무인자, 프리셋, JQL, --read, --module, --ping, --list)

## 확인 필요 사항 _(2026-04-18 기준 미해결)_

아래 3건은 {이름} 책임과 {이름} 수석이 별도 확인 중. 스킬은 무관하게 동작하며,
...
**단일 원천 (Single Source of Truth)**: [`참고자료/조직정보/더존비즈온-조직구조.md`](../../../../douzone-forge/참고자료/조직정보/더존비즈온-조직구조.md) 의 각 Cell 표에서 **이름 옆 괄호 안의 ID**가 곧 JIRA ID이며, 사내 이메일 프리픽스(`{id}@douzone.com`)와 동일하다.

```
예) SB설계Cell 표에서 "대리 | {이름} ({ID})" →
    JIRA ID = {ID}
    이메일 = {ID}@douzone.com
```
...
담당자 JIRA ID가 필요할 때:
1. 사용자에게 묻지 말고 먼저 `참고자료/조직정보/더존비즈온-조직구조.md` 에서 이름으로 Grep
...
| 날짜 | 내용 |
|------|------|
| 2026-04-18 | 초기 버전 — {이름} 책임 `amaranth10-jira-collector` PoC 래핑 |
