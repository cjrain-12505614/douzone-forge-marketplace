...
<!-- ⚠️ 2026-07-23 자비스: 본 항목은 v1.17.0으로 작성됐으나 그 번호를 아마링크 건이 선점했다.
     내용은 그대로 보존하며, 배포 시 작성자가 번호를 재지정할 것 — 자비스가 임의 지정하지 않음. -->
## (미배포 · 버전 재지정 필요) — 원피스 지면 폭·이미지 실측 정본화 (문서옵션 조용한 무시 해소 + 즉시 로딩 자동화)
