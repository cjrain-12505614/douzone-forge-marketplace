...
> **의존 SSoT(단일 출처, Single Source of Truth)**: `규칙/프로세스/개발환경-구성-표준.md`(forge) — 환경 5단계·스택·프로파일·FE 절차·트러블슈팅의 정본. 본 스킬은 그 표준을 **특정 모듈에 적용하는 실행 절차**다.
...
> `module-overview.md`·소스 프로파일(`Amaranth10/_소스분석/profiles/`(forge))로 레포 그룹·이름을 교차 확인. 그룹이 다르면(예: gw 계열) 실측 후 보정.
...
- **세션 기록**: `_개인/sessions/{대상}/_current.md`에 단계별 실측·마커 누적(강제원칙 2 사실 기록)
- 표준에 새 함정 발견 시 → `개발환경-구성-표준.md` §6/§9 보강(본문 변경은 승인 후)

---

## 8.5 forge 맥락 브리지 (기본 설치 권장 — VSCode Claude Code)

VSCode의 Claude Code가 이 개발환경에서 douzone-forge 맥락(소스분석·모듈·프로젝트·조직·규칙·코딩 규칙)을 인지하도록 연결한다. (CoWork 앱은 forge를 직접 마운트하므로 불필요 — VSCode Claude Code 전용)

**채택 모델(opt-out, 2026-06-22 결정)**: 이 개발환경을 세팅할 때 브리지를 **기본 설치(권장)**한다 — 순수 opt-in은 대부분 안 켜므로, 개발자에게 **의도를 1회 확인**(소스 코딩용이면 설치 권장)하고 기본은 설치한다. **진짜 강제가 아니라** 언제든 `--remove`로 깔끔히 뺄 수 있다(소스·실작업 영향 0). 구조 무결성 **강제 규칙은 브리지가 아니라 게이트(CI·머지 리뷰)**가 담당하므로, 브리지를 빼도 강제는 유지된다(브리지=보조).

```bash
FORGE=~/Workspace/douzone-forge   # 본인 워크스페이스 클론 위치로 바꾼다
# 설치(기본 권장) — forge 루트 자동 감지, 개발 폴더 생략 시 자동 결정
bash "$FORGE/규칙/프로세스/forge-bridge/setup-forge-bridge.sh"
bash "$FORGE/규칙/프로세스/forge-bridge/setup-forge-bridge.sh" ~/dev/amaranth10-lte   # 개발 폴더 명시
# 제거(언제든 — opt-out)
bash "$FORGE/규칙/프로세스/forge-bridge/setup-forge-bridge.sh" ~/dev/amaranth10-lte --remove
```

- **의도 확인(기본 설치 흐름)**: 설치 전 "이 폴더는 소스 코딩용인가요? forge 맥락 브리지를 설치할까요?(권장)"를 **1회 묻고 기본은 설치**. 비개발 용도면 skip. 거부·삭제는 `--remove`.
...
- **결과**: 개발 폴더에 `_forge` 심링크 + `CLAUDE.md`(코딩 핵심 자동 로드 + forge 전 영역 지도 + coding-rules INDEX) → 새 Claude Code 세션이 자동 로드(`/memory` 확인, 첫 세션 외부 import 승인 1회). dz-* 커맨드는 플러그인 설치 시.
- 단일 출처: `규칙/프로세스/forge-bridge/`(스크립트·`CLAUDE.md.tmpl`·README) · 표준 §11 · 결정 `프로젝트/PRJ-2026-014_douzone-forge-구축/06_의사결정/20260622-개발자하네스-결정.md`(forge).
...
- SSoT: `규칙/프로세스/개발환경-구성-표준.md`(forge) (§3 DB타게팅·§4 IDE·§5 BE셋업·§6 FE·§7 인증·§9 트러블슈팅·§10 온보딩·부록 A 샘플)
- 비밀정보: `규칙/프로세스/비밀정보-관리-표준.md`(forge) — 자격증명 트리 밖 로컬만
