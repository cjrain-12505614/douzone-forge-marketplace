...
# 정본 루트: [제품 영역] Amaranth10 · BizboxAlpha · WehagoQ · Wehago · WehagoE  +  [공용 6] GitCheck · deliverables · 규칙 · 참고자료 · 프로젝트 · 조직  +  [_ 영역 3] _개인 · _분석요청문서 · _운영
# 2026-07-22 개정 — 제품 영역 범주 신설 (룰 1-A). 제품 폴더 추가 시 본 화이트리스트도 함께 갱신할 것.
# .bak.* 패턴: 변천사 보존 영역
if [[ "$FILE_PATH" =~ /douzone-forge/([^/]+)/ ]]; then
  ROOT_FOLDER="${BASH_REMATCH[1]}"
  case "$ROOT_FOLDER" in
    Amaranth10|BizboxAlpha|WehagoQ|Wehago|WehagoE|GitCheck|deliverables|규칙|참고자료|프로젝트|조직|_개인|_분석요청문서|_운영)
      # 정본 루트 — 제품 영역 3 + 공용 6 + _ 영역 3 (2026-07-22 개정, 룰 1-A)
      ;;
    *.bak.*)
      # .bak.* 패턴 — 변천사 보존 정합
      ;;
...
⚠️  [folder-structure-check] 워크스페이스 루트 정합 외 폴더 검출 (룰 1)
    경로: $FILE_PATH
    폴더: $ROOT_FOLDER
    → 제품 영역 5 (Amaranth10·BizboxAlpha·WehagoQ·Wehago·WehagoE) + 영문 골격 2 (GitCheck·deliverables) + 한글 정합 3 (규칙·참고자료·프로젝트) + 조직 + _ 영역 3 (_개인·_분석요청문서·_운영) 외 신규 검토.
    → 새 제품 영역이면 룰 1-A 요건(제품 단위·_README.md·파스칼 표기) 충족 후 본 훅 화이트리스트에 추가.
