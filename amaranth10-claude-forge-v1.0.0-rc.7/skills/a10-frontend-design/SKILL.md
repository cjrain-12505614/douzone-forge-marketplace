---
name: a10-frontend-design
description: Cowork 환경에서 Anthropic 공식 frontend-design 플러그인 직접 호출이 불가할 때의 폴백 미러. 더존비즈온 보고서·대시보드·HTML 산출물의 고품질 시각화를 차분한 엔터프라이즈 톤으로 작성. "프론트 디자인 적용", "고품질 HTML 작성", "보고서 시각화", "대시보드 디자인", "frontend-design 폴백" 트리거. 원본 Anthropic frontend-design (claude-plugins-official) 본질 미러 + 한국어/더존 컨텍스트 보강.
version: 0.1.0
...
Anthropic 공식 `frontend-design` 플러그인의 본질을 amaranth10-claude-forge 플러그인 안으로 흡수한 미러 스킬. **Cowork 환경에서 원본 frontend-design 직접 호출이 불가한 갭 (TASK-S-05 본질) 의 폴백 수단** 으로 신설.
...
| **Cowork** (데스크탑 자율 에이전트) | Anthropic frontend-design 플러그인 활성화 후 원본 호출 (Customize Marketplace 가이드 참고) | **본 스킬 a10-frontend-design 폴백** |
...
- **ONEFFICE 환경 대상** — `a10-html-oneffice-builder` 가 ONEFFICE 제약 (dzeditor 컨테이너 보존 등) 까지 통합 처리한다. 본 스킬은 ONEFFICE 외부 자체완결 HTML 산출물 전용
- **그래프·차트 라이브러리 핵심 작업** — Chart.js·D3 등 라이브러리 사용이 본질인 경우, 본 미학 가이드는 보조적으로만 적용
...
- **Editorial Gravity** — FT Weekend × 금융 연차보고서 × 건축 매거진 톤. 차분한 권위감. 차민수 수석의 보고서·발표 자료에 적합.
...
- ONEFFICE 문서 주입 (편집모드 HTML 삽입) 이 본질인 작업은 `a10-html-oneffice-builder` 또는 `a10-oneffice-writer` 호출 — 본 스킬과 혼동 금지
...
본 스킬은 **Anthropic `claude-plugins-official` 마켓플레이스의 `frontend-design` 플러그인 본질을 미러한 폴백 수단**이다. 원본 라이선스 (`LICENSE.txt`) 의 조건을 그대로 준수한다.

원본 위치: `~/.claude/plugins/cache/claude-plugins-official/frontend-design/unknown/skills/frontend-design/SKILL.md`

- 본 미러는 원본 본질 보존을 우선으로 하되, 한국어 풀이 · 더존 컨텍스트 보강은 본 플러그인 (amaranth10-claude-forge) 자체 작업으로 추가
...
- **2026-05-12** — 차민수 수석 옵션 B 결재 (Cowork ↔ Claude Code 플러그인 상속 갭 폴백 신설). PRJ-2026-013 기술위원회 AI 개발툴 TFT 트랙의 부산물로 식별된 갭. 옵션 C (cowork-to-claudecode-migration.md SSoT 갱신) 와 병행 채택.
...
| **`a10-html-oneffice-builder`** | ONEFFICE 원피스 HTML 작성 표준 | ONEFFICE 환경 제약 통합 처리. 본 스킬은 ONEFFICE 외부 자체완결 HTML 전용 |
| **`a10-oneffice-writer`** | 원피스 편집모드 HTML 주입 | 산출물 주입 단계 — 본 스킬로 HTML 생성 후 호출 가능 |
| **`a10-figma-make-reviewer`** | Figma Make 산출물 검증 | 디자인 시스템 적합성 검증 — 본 스킬의 산출물에 후속 적용 가능 |
...
> 사용자: "/a10-frontend-design 으로 디자인해줘" (Claude Code 환경)

→ 응답: "Claude Code 환경에서는 `frontend-design:frontend-design` 원본 직접 호출이 우선입니다. 본 폴백 스킬은 Cowork 환경 전용입니다. 원본으로 진행하겠습니다." → 원본 호출로 위임.

---

_본 SKILL.md 는 Anthropic `frontend-design` 플러그인 본질을 amaranth10-claude-forge 플러그인 안에 미러한 폴백 수단. 원본 라이선스 준수. 2026-05-12 차민수 수석 옵션 B 결재 신설._
