...
#
# 검출 영역:
#   1. 워크스페이스 루트 신규 폴더 (9 정합 폴더 + .bak.* 패턴 외) — 룰 1
...
# 9 정합 폴더: AI · Amaranth10 · GitCheck · deliverables · 규칙 · 참고자료 · 프로젝트 · 조직 · _개인
# .bak.* 패턴: 변천사 보존 영역
if [[ "$FILE_PATH" =~ /douzone-forge/([^/]+)/ ]]; then
  ROOT_FOLDER="${BASH_REMATCH[1]}"
  case "$ROOT_FOLDER" in
    AI|Amaranth10|GitCheck|deliverables|규칙|참고자료|프로젝트|조직|_개인)
      # 9 정합 폴더
      ;;
    *.bak.*)
      # .bak.* 패턴 — 변천사 보존 정합
...
⚠️  [folder-structure-check] 워크스페이스 루트 정합 외 폴더 검출 (룰 1)
    경로: $FILE_PATH
    폴더: $ROOT_FOLDER
    → 영문 골격 4 (AI·Amaranth10·GitCheck·deliverables) + 한글 정합 3 (규칙·참고자료·프로젝트) + 사람 관련 2 (조직·_개인) = 9 폴더 외 신규 검토.
