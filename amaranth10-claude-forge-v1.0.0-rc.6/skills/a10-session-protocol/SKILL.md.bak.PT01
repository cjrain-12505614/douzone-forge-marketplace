...
| 프로젝트 포트폴리오 | `_projects/_dashboard.md` |
| 프로젝트 상세 | `_projects/PRJ-YYYY-NNN_*.md` |
| 모듈 세션 체크포인트 | `[모듈]/sessions/_current.md` |
...
2. **프로젝트 대시보드 확인**: `_projects/_dashboard.md`를 읽어 전체 프로젝트 현황을 파악한다.
3. **모듈 세션 체크포인트 확인**: 작업 대상 모듈의 `sessions/_current.md`를 읽는다.
...
1. 모든 모듈의 `sessions/_current.md` 존재 여부를 확인
...
1. **모듈 세션 체크포인트 갱신**: 작업한 모듈의 `sessions/_current.md`를 업데이트한다.
   - 완료된 작업에 체크 추가
   - 남은 작업 갱신
   - 현재 시점·재개 방법 명시
   - 날짜 업데이트
...
2. **프로젝트 진행현황 갱신**: 관련 프로젝트가 있으면:
   - `_projects/PRJ-*.md`의 `03. 상세 진행현황`에 일자별 로그 추가 (최신순)
   - `02. TASK 목록` 진행률 갱신
   - `01. C. 진행률` 전체 수치 갱신
   - 이슈 발생 시 `01. D. 이슈 & 리스크` 추가

3. **대시보드 갱신**: `_projects/_dashboard.md`의 진행률·상태·리스크 갱신
...
- `sessions/_current.md`의 "남은 작업"이 가장 중요하다. 다음 세션의 나 자신이 읽을 브리핑 문서.
- "무엇을 했는지"보다 **"왜 그렇게 했는지"**와 **"다음에 무엇을 해야 하는지"**를 우선한다.
- 여러 모듈을 동시에 작업했으면 각 모듈의 `sessions/_current.md`를 모두 업데이트한다.
- `sessions/_current.md`는 **Claude가 자동으로 관리**한다. 사람이 직접 편집하지 않는다.
