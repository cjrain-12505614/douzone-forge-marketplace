# dz-git-daily v0.9.0 — 비대칭 스코어 매칭 엔진 설계서

> ⛔ **2026-08-13 — 승인 큐 부분(§5·§6) 폐기.** 본 문서는 **설계 기록으로 보존**한다.
> 아래 §5 승인 큐·§6 학습 레이어가 전제한 `프로젝트/_mapping-queue.md`·`_mapping-history.md`는 **한 번도 만들어진 적이 없고**, 엔진 입력인 PRJ `01.E 연계 키`도 2026-04 폴더 압축 전환 때 사라졌다. `/dz-confirm`·`/dz-reject` 커맨드는 삭제됐다.
> 살아 있는 부분은 §1~§4 스코어링 로직이며, 40~69점 구간은 이제 레포트의 「⚠️ 수동 확인 필요」 표기로만 남는다.
> 경위: `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §12.3 · 현행 운용은 `SKILL.md` 「승인 큐 지속성 — 폐기」 절.

> 상태: **Draft (2026-04-18) · 승인 큐 부분 폐기 (2026-08-13)**
...
3. **학습 가능**: PM 확정/기각 이력을 `프로젝트/_mapping-history.md`에 누적 → 다음 스캔의 가중치 조정
...
- `dz-weekly-report-parser` 보조 스킬이 ONEFFICE 주간보고 파싱
- 각 담당자의 "이번 주 작업 라인"에서 PRJ 언급 추출:
  ```
  "{이름} — PRJ-2026-003 모바일메일 줌인 작업" → {author: {이름}, prj: PRJ-2026-003}
  ```
...
> `/dz-git-daily`가 이 PRJ로 커밋을 자동 귀속시키기 위한 지표.
...
| 이름 | Git 저자명 | 이메일 |
|------|-----------|--------|
| {이름} | seonmyeong | seonmyeong@douzone.com |
| {이름} | kimsh_c | kimsh_c@douzone.com |
...
|---|-----|------|--------|---------|------|-----------|------|
| 1 | d9106ea | {이름} | 메일게시판 줌인 | PRJ-2026-003 | 68 | 경로40+담당30-주보X+키워드20-브랜치X-10=68 | `/dz-confirm 1` / `/dz-reject 1` |
```

> ⛔ 위 「액션」 열과 아래 3줄은 **폐기된 설계**다(2026-08-13). 커맨드는 삭제됐고 큐 파일은 존재한 적이 없다. 설계 기록으로만 남긴다.

- ~~`/dz-confirm N` → 해당 PRJ 파일에 반영 + `_mapping-history.md`에 확정 기록~~
- ~~`/dz-reject N` → 미태깅으로 보냄 + 기각 기록 (다음 스캔에서 해당 저자+패턴 조합 감점 -10)~~
- ~~`/dz-confirm N PRJ-YYYY-NNN` → 추정이 잘못됐을 때 다른 PRJ로 수동 귀속~~

---

## 6. 학습 레이어 (`프로젝트/_mapping-history.md`)

```markdown
| 일자 | SHA | 저자 | 메시지 요약 | 추정 PRJ | 점수 | PM 결정 | 실제 PRJ |
|------|-----|------|-------------|---------|------|---------|----------|
| 2026-04-18 | d9106ea | {이름} | 메일게시판 줌인 | PRJ-2026-003 | 68 | confirm | PRJ-2026-003 |
| 2026-04-18 | a1b2c3d | {이름} | mode:local 수정 | PRJ-2026-001 | 45 | reject  | (유지보수) |
```
