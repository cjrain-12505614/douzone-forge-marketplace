...
```bash
# 설치(기본 권장) — forge 루트 자동 감지, DEV_DIR 생략 시 자동 결정
bash <forge>/규칙/프로세스/forge-bridge/setup-forge-bridge.sh [DEV_DIR]
# 제거(언제든 — opt-out)
bash <forge>/규칙/프로세스/forge-bridge/setup-forge-bridge.sh [DEV_DIR] --remove
```
