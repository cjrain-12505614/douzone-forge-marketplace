...
## v1.29.3 (2026-09-11) — 세션 명령이 낡은 동기화 엔진을 집던 것 교정 (이정미 리더 제보)

**계기**: 이정미 리더 제보(2026-09-11 14:39) — `dz-end-session` 이 안내하는 명령이 설치본이 아닌 옛 판 엔진을 실행한다. 그 맥에서는 v1.18.1 이 집혔다.

**원인은 「옛 판이 남아 있어서」가 아니라 「어느 판을 집을지 정해져 있지 않아서」다.** 마켓플레이스 저장소는 배포본 폴더를 판마다 보관한다(현재 78개). 저장소를 클론하면 그 폴더가 전부 내려오므로, `find ~/.claude/plugins -name dz-gitlab-sync.sh | head -1` 의 후보가 이 기기 기준 **77개**가 된다. find 의 출력 순서는 디렉터리 항목 순서라 실행할 때마다 답이 바뀐다 — **같은 명령 100회 중 설치본과 일치한 것은 7회**였고, **18회는 신원 게이트(v1.19.0)가 없는 판**이었다.

**왜 위험한가.** 집히는 옛 엔진은 껍데기가 아니라 온전히 동작한다 — `git add -A` · `commit` · `push origin` 이 다 살아 있어 **공용 저장소에 실제로 커밋·푸시한다.** 게다가 옛 판은 무조건 `exit 0` 으로 끝나 화면에는 「✅ 반영 완료」가 뜬다. 틀렸다는 신호가 어디에도 남지 않는다. 빠지는 가드 중 둘은 이미 사고가 난 것들이다 — **신원 게이트**(2026-07-28 공용 자산 98KB 0바이트 커밋)와 **`core.quotePath=false`**(한글 경로에서 비밀·소실 스캐너가 빗나감).

- **`commands/dz-end-session.md` · `commands/dz-save-session.md` 의 엔진 경로 해석을 교체했다.** ① 설치 기록(`installed_plugins.json`)의 `installPath` 를 먼저 읽고 ② 없으면 설치 캐시에서 **버전 정렬** 최신을 고른다(문자열 정렬이면 `1.9.0` 이 `1.10.0` 보다 뒤로 가므로 숫자 비교). ③ 둘 다 실패하면 조용히 넘어가지 않고 **찾지 못했다고 알린다**.
- ⛔ **`${CLAUDE_PLUGIN_ROOT}` 는 쓰지 않았다.** 훅 배선(`hooks.json`)에서는 하네스가 치환하지만 **커맨드 문서의 bash 블록에서는 비어 있다**(실측). 그대로 쓰면 `bash /hooks/dz-gitlab-sync.sh` 가 된다. 제보에 담긴 처방이었으나 검증에서 기각했다.
- **검증**: 문서에 적힌 명령을 그대로 떼어내 실행해 846줄 설치본(1.29.2)에 도달함을 확인했고, 10회 반복 결과가 하나로 수렴했다. 버전 정렬은 `1.0.0-rc.14` · `1.9.0` · `1.10.0` · `1.29.2` 혼합 표본으로 시험했다.

**남은 것** — 같은 계열 결함 11건을 전수 조사에서 확인했다(bash 블록의 `${CLAUDE_PLUGIN_ROOT}` 2건 · 워크스페이스 경로 고정 6건 · find 패턴 2건 · 하드코딩 판번호 1건). 이번 판에는 담지 않았고 별도 차수로 다룬다. 마켓플레이스 저장소의 배포본 78개를 정리할지는 롤백 여지와 맞물린 별도 판단이다.

## v1.29.2 (2026-09-11) — 회귀 시험이 PC 의 git 설정을 읽지 않게 격리 (시험 커밋의 계측 전송 차단)

**계기**: 2026-09-11 v1.29.1 배포 전후로 회귀 시험을 돌린 두 기기에서 같은 문제가 드러났다(A2 발견 · 맥미니B 대조).

- **① 시험 커밋이 사내 AI 계측(Litmus) 중앙으로 전송됐다.** 전역 `core.hooksPath` 가 Litmus 훅 폴더인 PC 에서는 시험이 만드는 임시 저장소마다 Litmus 훅이 붙는다. 실측 전송량 **맥미니B 70건(AI 10 · 사람 148줄) · A2 22건(사람 58줄)** — 대부분 시험용 가짜 자료지만 되돌릴 수 없다. 이 문제는 2026-09-08 원인진단 리포트 §9.2 에 이미 적혀 있었는데 격리 없이 계속 돌렸다.
- **② 같은 시험이 기기마다 결과가 달랐다.** Apple Git 은 시스템 설정에 `init.defaultBranch=main` 이 있고 Homebrew git 은 없어 새 저장소가 **master** 로 시작한다. 빈 원격을 main 으로 가정하는 시험 4건([7/10] 3건 · [10/10] 충돌 정지 1건)이 Homebrew git 기기(A2)에서만 실패했다 — **훅 결함이 아니라 시험 자료의 환경 의존**. 맥미니B 에서 시스템·전역 설정을 끄자 같은 4건이 그대로 재현됐다.
- **교정**: `_selftest.sh` 머리에서 `GIT_CONFIG_NOSYSTEM=1` · `GIT_CONFIG_GLOBAL=<기본 분기 main 한 줄만 둔 임시 파일>` 을 export 한다. 한 번에 ①·② 가 풀린다. **훅 본체는 바꾸지 않았다.**
- **격리 전제 검증 [0/10] 2건 신설** — 빈 저장소를 만들어 ⓐ 전역 훅 경로가 보이지 않는지 ⓑ 기본 분기가 main 인지 확인한다. `GIT_CONFIG_GLOBAL` 은 git 2.32 이상이라, 오래된 git 에서 격리가 조용히 안 먹는 경우를 여기서 드러낸다. 음성 대조(격리 줄을 뺀 사본)에서 ⓐ 가 FAIL 로 잡히는 것을 확인했다.
- **전체 PASS 62 · FAIL 0** (플러그인 폴더·forge 워크스페이스 폴더 두 곳에서 실행) · 실행 중 **Litmus 기록 0건**.
- 진단: 워크스페이스 `참고자료/리포트/2026-09-11-맥미니B-rebase중단-미푸시0오표시-진단.md` §8 · §8.1

## v1.29.1 (2026-09-11) — 고착 안내가 「미푸시 0건」이라 말하던 결함 교정

**계기**: 2026-09-11 맥미니B 실사고 — 공용 forge 동기화가 **리베이스 중단 상태로 약 43시간** 멈춰 있었고, 그동안 **원격에 없는 내 커밋이 97줄** 매달려 있었다. 그런데 훅은 매 턴 **「미푸시 0건」** 이라 알렸다. v1.28.1 에서 방치를 막으려고 넣은 숫자가 **「걱정할 것 없음」으로 읽혔다.** 진단: 워크스페이스 `참고자료/리포트/2026-09-11-맥미니B-rebase중단-미푸시0오표시-진단.md`

- **원인**: 고착 안내가 `origin/<브랜치>..HEAD` 로 셌다. 리베이스가 멈추면 **HEAD 는 원격 커밋(onto) 위에 앉고 내 커밋은 `orig-head` 쪽에만 남는다** — 그래서 멈춘 리베이스에서는 **구조적으로 늘 0** 이 나온다. 분리된 HEAD 가 원격 위에 있고 내 커밋이 브랜치 끝에만 있는 경우도 같은 계열로 0 이 나왔다.
- **교정**: 셈의 출발점을 상태별로 바꿨다.
  - 리베이스 잔해가 있으면 `orig-head` 에서 `^origin/<브랜치>` 를 뺀다(브랜치 이름은 `head-name` 에서 읽는다 — 분리된 HEAD 에서는 현재 브랜치를 알 수 없다).
  - 잔해가 없으면 `HEAD` 와 브랜치 끝을 **합집합**으로 세어 어느 쪽에 있든 잡는다(한 커밋은 한 번만 센다).
- **문구 교정**: 「미푸시」 → **「원격에 아직 없는 내 커밋 N건」.** 「미푸시」는 「올리기만 하면 된다」로 읽히는데, 멈춘 리베이스에서는 올리기가 아니라 **복구가 먼저**다.
- ⛔ **판정·게이트·자동 복구 조건은 한 줄도 바꾸지 않았다** — 바뀐 것은 안내에 찍히는 숫자와 문구뿐이다.
- **회귀 시험 [10/10] 신설 6건. 전체 PASS 60 · FAIL 0.** 현장 모양(리베이스가 첫 명령 앞에서 멈춰 `HEAD==onto`)을 그대로 재현하며, 그 모양이 실제로 조성됐는지 **전제 검증**을 함께 단언한다. 충돌로 멈춘 리베이스(3건 중 1건 옮겨짐)에서 **이중 계산이 없는지**도 본다.
  - **음성 대조 실측**: 같은 시험을 **옛 훅에 돌리면 5건 FAIL** 이 난다(PASS 55 · FAIL 5). 새 시험이 결함을 실제로 잡는다는 뜻이다.

## v1.29.0 (2026-09-08) — 고착 자동 복구 A등급: 껍데기 잔해만 스스로 치운다

**계기**: 차민수 수석 지적 — 「정리는 락까지만 하고 복구는 사람이 한다는데, **기획·설계자는 복구를 하기 힘들 텐데** 자동화가 맞지 않나」. 실측이 그 지적을 뒷받침한다. 방치 시간 **22일 · 17시간 30분 · 15시간 30분** — 셋 다 훅은 매 턴 정확히 알리고 있었다. **알아채게 만든 것과 고칠 수 있게 만든 것은 다르다.** 설계안: `참고자료/리포트/2026-09-08-dz-sync-고착-자동복구-설계안.md`

- **A등급(껍데기 잔해)만 자동으로 치운다.** 다섯 조건을 **모두** 만족해야 한다 — ① rebase 잔해 있음 ② `git-rebase-todo` 파일이 **없음**(비어 있는 게 아니라 없어야 한다) ③ `orig-head` 가 있고 `HEAD` 와 같음(진행분 0) ④ 작업트리 깨끗 ⑤ `head-name` 브랜치가 실재하고 그 끝이 `HEAD` 와 같음. 하나라도 애매하면 손대지 않는다.
- **복구는 `--quit` + `switch`.** ⛔ `--abort` 를 쓰지 않는다 — `main` 을 되돌려 멈춰 있던 커밋 접근을 잃을 수 있다. **실측(2026-09-08): `--quit` 만으로는 분리된 HEAD 가 그대로 남아 `switch` 가 필수다.**
- **백업이 먼저다.** 안전망 갈래 `backup/autoheal-…` 를 만들지 못하면 진행하지 않는다. 복구 후 **불변식(HEAD 불변·잔해 제거·작업트리 깨끗)을 검증**하고, 어긋나면 동기화를 멈춘다. **되돌리는 한 줄**을 화면에 함께 출력한다.
- **⛔ 위험한 것은 그대로 둔다.** 2026-08-27 사고에서 잔해는 **고장이면서 동시에 안전장치**였다 — 죽은 pull 이 작업트리만 원격판으로 덮은 상태에서 잔해를 먼저 걷으면 다음 커밋이 자산화를 지운다. 그 모양은 조건 ②(todo 있음)와 ④(작업트리 더러움) 두 곳에서 걸린다.
- 스위치 `DZ_SYNC_AUTOHEAL` — `a`(기본, A등급만) · `0`(끔). B·C등급은 열지 않았다.
- **회귀 시험 [9/9] 신설 9건. 전체 PASS 54 · FAIL 0.** 합격선은 **2026-08-27 죽은 pull 재현에서 자산화를 지우는 커밋이 만들어지지 않는 것**이며, 그 시험에 **전제 검증**을 함께 넣었다(전제가 조성되지 않으면 시험이 무의미하다는 것을 시험 스스로 보고한다).
  - ⚠️ 시험 작성 함정 둘을 기록해 둔다 — ① 임시 폴더 경로에는 `douzone-forge` 문자열이 없어 **origin 가드가 먼저 걸린다.** 저장소 **이름**을 `douzone-forge.git` 으로 맞춰야 단언이 실제로 돈다(안 그러면 전건이 「공허하게 통과」한다). ② 리베이스 중에는 git 이 `branch -f` 를 거부하므로 조건 ⑤ 시험은 `update-ref` 로 우회해야 한다.
- **부수 교정**: 고착 안내의 미커밋 건수가 `0` 일 때 두 줄로 출력되던 표시 버그(`grep -c` 의 종료코드 1 + `|| echo 0` 중복).

## v1.28.1 (2026-09-08) — 계측이 새는 것을 훅이 스스로 알아채게 (리트머스 산도 미집계 대응)

**계기**: 제1AI연구소 김형진 책임의 원인 파악 협조 요청(2026-09-08). 실측 결과 **forge 가 Litmus 훅을 대신 부르는 대행이 4종 중 3종뿐**이었고(`post-merge` 누락), 그 사실을 **아무도 대조하지 않아 3개월 넘게 조용히 계측만 새어 나갔다.** 진단: `참고자료/리포트/2026-09-08-리트머스-산도미집계-훅대행-부분이식-원인진단.md`

- **고착 상태에서도 죽은 잠금을 치운다.** 종전에는 분리된 HEAD·중단된 rebase 게이트가 먼저 종료해 **본문 뒤쪽의 잠금 청소에 한 번도 도달하지 못했다.** 유지수 대리 PC 에서 `.git/packed-refs.lock` 이 8일간 남아 매 git 작업에 `File exists` 오류를 뿜었다(커밋은 성공해서 실패로 보이지 않았다). 잠금 청소 함수를 게이트 **앞으로** 옮기고 게이트 직전에 부른다. ⛔ **게이트의 판단·순서는 한 줄도 바꾸지 않았다** — 잠금만 치우고 동기화는 여전히 하지 않는다.
- **훅 대행 완전성 자동 대조 신설.** `core.hooksPath` 로 훅 경로를 독점하는 저장소에서, Litmus 훅 디렉토리 목록과 `.githooks` 대행 목록을 대조해 **빠졌거나 대행하지 않는 훅을 지목**한다. Litmus 미설치·훅 경로 미독점 PC 는 조용히 지나간다. 하루 한 번으로 제한. 끄기 `DZ_SYNC_LITMUS_CHECK=0`.
- **놓친 커밋 감시 신설.** Litmus 가 스스로 「못 받았다」고 쌓아 둔 목록에서 이 저장소 건수를 세어 임계(기본 20) 초과 시 알린다. ⚠️ 타사 도구의 내부 상태 파일에 기대므로 **읽기 전용·실패 무해**로 만들었다(파일·형식이 바뀌면 조용히 건너뜀). 임계 `DZ_SYNC_GAP_WARN`.
- **고착 안내에 숫자를 넣었다.** 「분리된 HEAD 상태 — 건너뜀」이 경고로 읽히지 않아 방치된 실측 사례 셋(17시간 30분·22일·15시간 30분)의 직접 원인은 판정이 아니라 **문구**였다. 이제 **미커밋 N건 · 미푸시 M건 · N시간 전부터 멈춤**을 함께 알리고, 고착도 **연속 실패로 세어 3회째에 환기**한다.
- **회귀 시험 PASS 29 · FAIL 0 유지.** 신설 3건은 별도 시험 21건 + 잠금 재현 4건으로 검증했다(`_selftest.sh` 편입은 후속 작업).

## v1.28.0 (2026-09-07) — dz-sync 연속 실패 환기: 「6시간 아무도 몰랐다」를 막는다 (차민수 수석 지시)

**계기**: 2026-09-07 실사고 — 공용 forge 동기화가 **10:24~16:22 약 6시간, 18회 연속** 같은 지점에서 멈춰 있었다. 원인은 `법무관리(LTE)/history/_timeline.md` 한 파일이고, **무인 모니터 루틴(30분 주기)과 대화형 세션이 그 파일 끝을 각각 이어 붙인 것**이다. 내용은 전혀 겹치지 않았고 자리만 겹쳤다. ⭐ 원격에 올라가고 있던 커밋은 전부 그 무인 루틴이었다 — **자동은 통과하고 사람이 막히는 구조**라 아무 화면에도 안 떴다. 진단: 워크스페이스 `참고자료/리포트/2026-09-07-공용forge-동기화-18회연속충돌-진단.md`

**실제 피해는 충돌이 아니라 방치였다.** 그래서 병합 정책은 그대로 두고(데이터 보존 최우선), **반복 실패를 사람에게 보이게** 하는 것만 더했다.

- **연속 실패 환기 신설** — 받기 실패·충돌·올리기 실패가 **3회 연속**이면 화면에 고지하고, 발신 수단이 있으면 밖으로도 보낸다. 재발신은 **최소 1시간 간격**이라 계속 실패해도 시끄럽지 않다. 끄기: `DZ_SYNC_ALERT=0` · 문턱 조절: `DZ_SYNC_FAIL_STREAK_N` · 간격: `DZ_SYNC_ALERT_MIN_GAP`.
- ⛔ **플러그인에는 어떤 웹훅·토큰도 담지 않는다.** 발신은 `DZ_SYNC_ALERT_CMD` 로 지정한 명령(메시지를 첫 인자로 받는다), 미지정이면 `~/.claude/dz-sync-alert.sh` 를 쓰고, 그것도 없으면 **화면 고지만** 한다. 각 사용자가 자기 발신 수단을 개인 영역에 두는 구조다.
- ⛔ **연속 횟수를 `_개인/sync-log.md` 로 세지 않는다** — 그 로그는 여러 기기 줄이 union 병합으로 섞여 **시간순이 깨진다**(2026-09-07 실측: 16:49:36 뒤에 16:49:33 이 온다). 로그로 세면 다른 기기의 성공 줄에 끊겨 오판한다. 그래서 **`.git/dz-sync-fail-streak`** 를 쓴다 — 클론(기기)별로 갈리고 git 추적도 되지 않는다.
- ⛔ **오프라인은 실패로 세지 않는다.** 망 밖은 실패가 아니라 정상 보존이다(사무실 밖 근무일마다 환기가 쏟아지면 정작 봐야 할 때 안 보게 된다). 초기화도 하지 않는다 — 망이 끊긴 사이 이미 쌓인 연속을 지우면 복귀 후 문턱까지 다시 세게 된다.
- **초기화는 「끝까지 성공」에서만** — 받기만 성공한 상태는 아직 성공이 아니다. pull 모드는 받기 성공, sync 모드는 **올리기 성공** 시점에 지운다.
- **훅 회귀 시험 6항목 추가**(`hooks/_selftest.sh` [7/7]) — 오늘 사고를 그대로 재현(다른 클론이 같은 파일 끝을 먼저 올린 뒤 로컬도 같은 자리 수정)해 ⓐ 충돌 발생 전제 ⓑ 1·2회째 무음 ⓒ **3회째 발동** ⓓ 발신 명령 실제 호출 ⓔ 끄기 스위치 ⓕ 성공 시 초기화를 단언한다. **전건 PASS 29 · FAIL 0.**
- ⛔ **시험이 실 발신 경로를 타지 않게 격리했다** — 환기는 `DZ_SYNC_ALERT_CMD` 가 없으면 `~/.claude/dz-sync-alert.sh` 를 **자동으로 찾는다.** 그 파일을 둔 기기에서 시험을 돌리면 **실제 Slack 으로 나간다**(같은 유형의 실사고: 모니터링 훅 검증에서 스텁을 두지 않아 509건이 실 서버로 갔다). 그래서 `_selftest.sh` 머리에서 무해한 스텁을 만들어 `export` 로 고정한다.
  - ⚠️ **격리를 「검증했다」고 말할 수 있게 만드는 데 한 번 더 걸렸다** — 스텁 로그가 `$TMP` 안이라 `trap 'rm -rf "$TMP"' EXIT` 가 함께 지웠고, 「로그 없음」을 「스텁이 안 불렸다」로 읽을 수 있었다. **로그 없음은 미호출의 증거가 아니다.** 로그 경로를 `DZ_SELFTEST_ALERT_LOG` 로 밖에 뺄 수 있게 하고 실측했다 — 스텁이 3회째에 정확히 1건 받았다(재발신 간격도 함께 확인됨).
- **곁가지 정리** — `_selftest.sh` 섹션 분모가 `/6` 과 `/5` 로 섞여 있던 표기 표류를 `/7` 로 통일했다.


## v1.27.1 (2026-08-28) — 대량 되돌림 가드 구멍 메움: 「62줄 리포트 통째 삭제」가 문턱 8줄 차이로 빠져나갔다

**계기**: v1.27.0 배포 직후 설치본으로 끝단 검증하다 **가드가 발동하지 않는 사례를 실측**했다. 되돌림 30줄 + 62줄 리포트 파일 삭제 = **삭제 92줄**인데, 두 발동 조건이 모두 `삭제 ≥ 100줄`을 요구해 **8줄 차이로 통과**했다. 리포트 한 건이 통째로 사라지는데도 자동 커밋된다.

- **ⓑ 갈래 문턱을 100 → 30 줄로 낮추고 비율 조건을 더했다** — `추적 파일 삭제 ≥ 1 && 삭제 ≥ 30줄 && 삭제 ≥ 추가 × 2`. 비율 조건은 대규모 재작성·이동을 오차단하지 않기 위한 것이다.
- **ⓒ 갈래 신설 — 죽은 pull 지문은 줄 수와 무관하게 잡는다.** `HEAD 가 원격보다 앞서 있고` + `작업트리가 원격판을 그대로 베끼고 있으면`(원격과의 차이 파일 수 × 3 ≤ 내 커밋과의 차이 파일 수, 그리고 차이 3파일 이상) 삭제 줄 수를 따지지 않고 정지한다. 2026-08-27 사고의 실제 모양을 조건으로 직접 옮긴 것이다.
- **회귀 시험 보강** — [6/6] 에 ⓒ(죽은 pull 지문) 추가하고, ⓑ 사례를 **실제로 빠져나갔던 92줄 경계**로 교체했다(문턱을 다시 올리면 여기서 FAIL 로 잡힌다). **전건 PASS 23 · FAIL 0.**
  - ⚠️ 시험 작성 함정도 함께 적어 둔다 — `git checkout-index -a -f` 는 **원격에 없는 파일을 지우지 않으므로** 「신규 파일」로는 죽은 pull 이 재현되지 않는다. 원격에도 있는 파일을 로컬에서 키운 뒤 되돌려야 한다.


## v1.27.0 (2026-08-28) — dz-sync 대량 되돌림 가드: 자산화를 지우는 자동 커밋 차단 (차민수 수석 지시)

**계기**: 2026-08-27 실사고 — forge 동기화가 12:22부터 약 20시간 끊겼고, 그 밑에서 **작업트리가 `origin/main` 판으로 통째로 덮여 있었다.** 그대로 자동 커밋됐다면 리포트 1건(62줄) 삭제 + 자산화 200여 줄 되돌림이 원격에 올라갔을 것이다. 진단·재현: 워크스페이스 `참고자료/리포트/2026-08-28-forge-동기화단절-작업트리롤백-복구.md` §7.

**원인(재현 확정)** — `git pull --rebase` 는 ① 작업트리 기록 → ② index 기록 → ③ HEAD·reflog 갱신 순으로 움직인다. 세션 종료 훅이 띄운 git 이 세션 정리와 함께 죽어 **①만 끝났다.** 그러면 HEAD·index 는 내 커밋에 남고 작업트리만 원격판이 되는데, git 은 이것을 **「사람이 손으로 고친 미스테이징 변경」으로 읽는다.** 6,001파일 격리 저장소에서 지문 8항목 전건 일치로 재현했다.

- **`commit_local()` 에 대량 되돌림 가드 신설** — `git add -A` 직후 staged 차이를 재어, ⓐ **삭제 100줄 이상이면서 추가줄의 2배 이상**이거나 ⓑ **추적 파일 삭제가 있으면서 삭제 100줄 이상**이면 자동 커밋을 멈추고 고지한다. 변경은 로컬에 그대로 두고 index 는 원상 복구(`git reset`)한다. 우회: `DZ_SYNC_ALLOW_MASS_REVERT=1`.
- **죽은 pull 지문 자동 지목** — 가드가 걸렸을 때 **HEAD 가 원격보다 앞서 있는데 작업트리는 원격판을 그대로 베끼고 있으면**(원격과의 차이 파일 수 × 3 ≤ 내 커밋과의 차이 파일 수) 「중간에 죽은 git pull 의 자국」이라고 짚고 복구 절차(운영가이드 §6.4)를 안내한다. 이번 사고를 정확히 이 조건으로 잡는다.
- **⚠️ 가드가 걸리면 받기·올리기도 함께 선다** — 안내 문구에 명시했다. 받기를 강행하면 autostash 복원 충돌로 **새 소실 경로**가 생기므로, 멈추되 분명히 알리는 쪽을 택했다(§6.2 데이터 보존 최우선).
- **훅 회귀 시험 4항목 추가**(`hooks/_selftest.sh` [6/6]) — 정상 작업 통과 · 되돌림 발동 · 변경 로컬 보존 · 우회 스위치 동작. 실제 임시 저장소를 만들어 스크립트를 그대로 돌려 단언한다. **전건 PASS 22 · FAIL 0.**


## v1.26.0 (2026-08-14) — 진행 대시보드 완료 표시를 체크박스 하나로 통일

> 배경: 「앞에 체크박스가 있는데 왜 또 완료 표시를 하지?」 차민수 수석 지적 → 실측으로 **두 표시가 서로 다른 값을 세고 있음** 확인.

### 원인 (실측 2026-08-14)
- 한 행에 **체크박스**와 **「✅ 완료」 배지**가 둘 다 있었다.
- 배지는 **파일 기록**(`lv="grn"`) 5건, 체크박스는 **`localStorage`** 0건 → **진행률이 「0 / 15 (0%)」로 거짓말.**
- 뿌리 — 「체크는 사용자가 하는 것」이라며 초기 씨딩(`PRESET`)을 없애면서 **진행률 기준을 함께 바꾸지 않았다.**

### 바뀐 것 — `skills/dz-progress-dashboard/SKILL.md`
- **§7.1 신설 — 완료 표시는 체크박스 하나.** 규칙 넷:
  1. 「✅ 완료」 배지 폐지 — 완료는 **체크 + 취소선**으로만
  2. 배지는 **우선순위 전용**(`🔴 오늘 · 🟠 지남 · 🟡 이번주 · 🟢 여유`) — 완료돼도 원래 우선순위 유지
  3. **진행률 = 체크 수 ÷ 전체** — 체크하면 즉시 오르는 것이 가장 직관적(수석 판단). 진행바·카운터·KPI 세 곳이 같은 값
  4. **파일 기록 완료는 `PRESET` 로 씨딩** — 체크는 `localStorage` 라 공유되지 않으므로 원천(md ① 상태·미리알림)에서 끌어와 첫 로드에 체크 상태로 그린다
     ⛔ **키 판번호를 올리지 않으면 씨딩이 무력** — 저장된 옛 배열이 이긴다
- **§5 공통 코어**에 「완료를 두 가지로 표시하지 않는다」 경고 추가.
- 예외 명시 — 「마감 지남 전건」처럼 **훑으며 고르는** 목록의 체크는 뜻이 달라 유지하되, 무엇을 위한 체크인지 한 줄 적는다.

### 검증 (실행 실측)
- `node --check` 통과 · 실제 탭 재측정 — `tcnt "5 / 15 (33%)"` · `kpi 5/15` · `bar 33%` · `checked 5` · `취소선 5행` · 배지 `🔴/🟠/🟡` 만 남고 **「✅ 완료」 문구 화면에서 소멸**(`indexOf === -1`).
- 상호작용 — 미완 1건 체크 시 `5/15(33%) → 6/15(40%)`, 해제 시 원복 확인.
