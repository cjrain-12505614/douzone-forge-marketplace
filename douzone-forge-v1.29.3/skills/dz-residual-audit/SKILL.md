...
# 잔존 검증 (dz-residual-audit) — Phase R+ R-07 신설 + Phase S S-06 5축 확장
...
- `_CLAUDE/`·`_projects/`·`reference/`·`modules/`
...
- 누락 시 경고 → {이름} 일상 운영 시 헤더 추가 (자비스 Q4 답변 — 자연 수정)
...
WS_ROOT = os.environ.get("CLAUDE_PROJECT_DIR") or os.path.expanduser("~/Workspace/douzone-forge")  # 경로 비종속 — Claude 세션 안 자동 발견, fallback 예시 (경로-비종속-운영-표준 §2.1)
LINK = re.compile(r'\[([^\]]*)\]\(([^)]+)\)')
PROTECTED = ('_archive', '99_archive')
OLD_PATTERNS = ['_CLAUDE/', '_projects/', 'projects/', 'reference/', 'modules/',
                'meta/repo-check/', 'meta/reports/',
                'meta/sessions/', 'team-tracking/']
# 오탐 제외 (R14, v1.11.2): '.claude/projects/'(Claude 시스템 경로)가 포함된 라인은 'projects/' 히트에서 건너뛴다.
# 'projects/'는 '_projects/' 라인에도 부분 매치되나 같은 옛 경로 계열이라 이중 보고만 되고 무해.
...
# 1. 깨진 링크 검출 — ⛔ 자체 정규식 금지, 공식 검사기를 호출한다 (2026-08-13 교체)
#
#   왜: 자체 정규식은 CommonMark 파서가 아니다. 균형 잡힌 괄호(`법무관리(LTE)`)를 첫 닫는
#       괄호에서 잘라 오탐하고, 목적지 공백 규칙·코드펜스를 구분하지 못한다.
#       실측 대조 — 자체 정규식 1,226건 / CommonMark 파서 688건 / 공식 검사기 129건.
#       게다가 `_개인/점검/정기종합점검-규격.md` 항목 11이 함정으로 못 박아 두었다:
#       「자체 정규식으로 세면 직전 수치와 비교할 수 없다」.
#   정본: 규칙/프로세스/scripts/linkcheck.py · 근거: 참고자료/리포트/2026-08-11-마크다운-링크-문법사고-실측재판정.md
import subprocess

broken_links = []
_checker = os.path.join(WS_ROOT, '규칙/프로세스/scripts/linkcheck.py')
if os.path.exists(_checker):
    # cwd 필수 — 검사기가 git ls-files 기준이라 어긋나면 조용히 0건이 된다
    _r = subprocess.run(['python3', _checker, 'active'],
                        cwd=WS_ROOT, capture_output=True, text=True)
    link_report = _r.stdout.strip()
else:
    link_report = '(공식 검사기 없음 — 규칙/프로세스/scripts/linkcheck.py 확인 필요)'
        ...
        with open(full, encoding='utf-8') as f:
            for ln, line in enumerate(f, 1):
                for pat in OLD_PATTERNS:
                    if pat in line:
                        if pat == 'projects/' and '.claude/projects/' in line:
                            continue  # Claude 시스템 경로 오탐 제외 (R14, v1.11.2)
...
with open(out, 'w', encoding='utf-8') as f:
    ...
    f.write(f"## 1. 깨진 링크 (공식 검사기 linkcheck.py active)\n\n```\n{link_report}\n```\n\n")
...
→ 잔존 검출 시 {이름}에 알림 (예: 깨진 링크 5건 + 옛 경로 2건)
```
