...
`dz-oneffice-writer`의 Step 0에서 호출된다.

> **변천사**: v0.1.0 신설 → **v0.2.0 dz-oneffice-kit OF 인용 전환(2026-07-14, T4 마이그레이션)** — 원피스 DOM 배관(중첩 iframe 접근·편집모드 판정/전환·CORS 서버)을 정본 `dz-oneffice-kit`의 OF 라이브러리로 인용. XHR body swap·"ONEFFICE 워드" 매칭·payload 구조는 이 스킬 고유로 유지.
