...
## v1.12.0 (2026-07-03) — 고아 훅 4종 배선 (경고 2 + 차단 2) + 라이프사이클 10단계 정합

> 차민수 수석 결정(사용자 가이드 재점검 트랙 E): 고아 훅 6종 중 **구조적 배선 가능 4종을 배선**, 프로토콜 미준수 2종은 구조적 예외로 확정.

### 변경 — `hooks/hooks.json` (고아 훅 4종 배선)
...
### 연계 — 표준 라이프사이클 10단계 승격 (규칙/프로세스, 플러그인 외)
- 도메인별-프로세스-표준 v1.1.0·단계별-도구-매트릭스 v1.0.2·개발-계획-수립-표준 v1.3.0 등 9단계→10단계(① 기획 병합 + ⑤ 개발 계획·⑥ 개발설계 독립 단계). AEGIS 정합 검증 통과. 사용자 가이드가 정본.
