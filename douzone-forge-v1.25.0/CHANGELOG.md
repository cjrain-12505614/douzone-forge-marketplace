...
## v1.25.0 (2026-08-14) — 진행 대시보드 자동 새로고침 계약 교체 (`file:` fetch 차단)

> 배경: 「대시보드 새로고침이 자동으로 안 되네」 차민수 수석 지적 → 실제 탭 탐침으로 원인 확정.
> 진단 `참고자료/리포트/2026-08-14-대시보드-file스킴-fetch차단-자동새로고침-무력화-진단.md`(forge)

### 원인 (실측 2026-08-14)
- Chrome 이 **`file:` 스킴의 `fetch` 를 막는다** → `TypeError: Failed to fetch`.
- 종전 계약(파일을 다시 읽어 `BUILD` 비교)은 **첫 단계에서 끊긴다.** 실패는 `.catch(){}` 가 삼켜 **신호가 하나도 안 남는다.**
- 탭 탐침 실측 — `probe: FETCH_FAIL` · 화면 `12:49` 에 반나절 정지 · 그동안 파일은 3회 갱신.
- ⚠️ **스크립트 결함이 아니었다** — 목록(15·55행)은 정상 렌더 중이었다. 감지 수단만 못 쓰는 상태.

### 바뀐 것 — `skills/dz-progress-dashboard/SKILL.md`
- §9 **자동 새로고침 계약 전면 교체**: 감지를 포기하고 「탭 복귀 시 무조건 `location.replace(pathname+"?v="+Date.now())`」.
  - **4초 시각 도장**(`sessionStorage`) — 재로드 직후 `focus` 재발동으로 생기는 **무한 재로드** 차단. 페이지 안 변수로는 못 막는다(재로드로 초기화).
  - **스크롤 복원**(`sessionStorage`) — 매번 재로드하므로 위치가 튀는 것을 상쇄.
  - `BUILD` 스탬프는 **비교값에서 「사람이 판본을 읽는 표시」로 역할 변경**(초 단위는 유지).
- §9.0 **함정 ④ 신설**(`file:` fetch 차단) + 함정 ① 서술에서 사실과 어긋난 「감지 성공」 문구 제거.
- §9.0 검증 항목 갱신 — 사라진 전역 상수 대신 **배열 길이 = DOM 행 수 대조 · `?v=` 도장 발동 여부**, 파일 단계 1차 관문은 **`node --check`**.
- §10 함정 ⑦ · §13 관련 자산 연동 갱신.

### 함께 갱신 (워크스페이스 · 재배포 불요)
- `규칙/프로세스/진행대시보드-환기.md` — 매 입력 주입 문구를 새 계약으로 교체.
- `CLAUDE.md` 「진행 대시보드 운영」 절 동일 교체.

### 검증
- `node --check` 문법 통과 · 실제 탭 재측정 `hdr 12:49 → 13:15` · `newContract true` · `oldFetch false` · `search "?v=…"`(발동 흔적) · `todayRows 15 · overRows 55`.

### ⚠️ 미해소
- 08-11 진단의 「fetch 감지 성공」 서술과 어긋난다. `http://` 폴백 측정이었는지 · Chrome 정책 변화인지 · 그때 오진인지 **아직 못 가림** _(확인 필요 2026-08-14)_.

## v1.24.0 (2026-08-14) — 원피스 발행본 점선(dashed outline) 원천 차단

> 배경: 트랙 A(헤드리스 `create_oneffice_doc`)로 발행한 문서의 표 셀마다 회색 점선이 남는 문제. 차민수 수석 지적 → 라이브 실측으로 원인 확정.
> 진단 `참고자료/리포트/2026-08-14-원피스-점선-outline-원천차단.md`(forge)

### 원인 (라이브 실측 2026-08-14)
- 편집기 스타일시트 규칙 — `:is(.dze_page_main, .dze_page_header, .dze_page_footer, td, th) > div:not(.dze_editable_div)` 와 `td>pre`·`th>pre` 에 `outline: lightgray dashed 1px`
- ⚠️ **편집 안내선이 아니다** — `body.readmode`·`designMode:off` 에서도 남아 **최종 산출물에 그대로 보인다**
- 규칙에 **`!important` 가 없다** → 인라인 선언이 항상 이긴다
- 실무에서 걸리는 자리는 사실상 **표 셀 안에 직접 넣은 `div`** 하나 (최상위 래퍼는 편집기가 `dze_editable_div` 자동 부여, `div` 안의 `div` 는 직계 자식이 아니라 비해당)
- 대조군 9종 전건 시험: 무처리 `div`·`pre` 2건만 점선 / `outline:none`·`outline:0`·`!important`·`p`·`span`·`dze_editable_div` 모두 차단

### 종전 대응이 비어 있던 이유 (오진 아님 — 배선 누락)
- 기존 처방은 `<style>` 블록의 `* { outline: none !important; }` 였고 **트랙 B(브라우저 DOM 주입) 경로에만** 배선돼 있었다
- 신규 발행의 기본 경로인 **트랙 A 에는 「저장 후 JS 주입」 단계가 없고**, 본문 화이트리스트에 `style` 태그가 없어 그 처방을 **쓸 수조차 없다** → 트랙 A 발행본은 매번 점선이 남았다

### 신설 — `OF.outlineGuard(html)` (dz-oneffice-kit §1)
- 본문 HTML 문자열의 모든 `div`·`pre` 에 인라인 `outline:none` 주입. **멱등**(이미 `outline` 선언이 있으면 무시) · **시각 변화 0**
- `<style>` 을 쓸 수 없는 **모드 C(평면 주입)·트랙 A 의 유일한 차단 수단**
- 단위 시험 13건 전건 통과(대문자 태그·홑따옴표·`url()` 포함 값·빈 style·멱등)

### 반영
- **`commands/dz-oneffice-write.md`** — 「공통 1. 입력 정규화」에 점선 차단 항목 추가 + **「점선 원천 차단 — 트랙 A·B 공통」 절 신설**(3중 방어·검증 스니펫). 종전 점선 항목은 「치명적 주의 — 트랙 B 한정」에 있어 트랙 A 사용자에게 닿지 않았으므로 절을 분리
- **`skills/dz-oneffice-kit/SKILL.md`** — `OF.outlineGuard()` 함수 + 함수 표 행 추가
- **`skills/dz-oneffice-writer/SKILL.md`** — 필수 CSS 3건 → **4건**(인라인 `outline:none`) · 모드 C·트랙 A 형식 요건 명시 · 트러블슈팅 점선 항목을 모드 B / 모드 C·트랙 A 로 분리
- **`skills/dz-html-oneffice-builder/SKILL.md`** — 필수 CSS 4건 + **표 셀 안 블록은 `p`·`span` 우선**(2차 방어)
- 워크스페이스(별도 커밋): `규칙/프로세스/HTML-원피스-작성-표준.md` §4 에 필수 4 등재 · 실행 스크립트 `규칙/프로세스/scripts/oneffice-outline-sanitize.py` 신설(`--check` 검사 모드 포함)

### 적용 실적
- Solar Chat 검토보고서 재발행 — 점선 **39개 → 0개**, 구조 지표(table 21·td 196·tr 79·span 58·p 5)와 본문 지문 17건 전건 일치

## v1.23.0 (2026-08-14) — 일반론 커맨드 A10 정합화 + `dz-tdd` → `dz-unit-test` 개명 (P3)

> 배경: Red Flags 점검 ⑤「프로젝트 특정이 아닌 일반론」 4건 교정. 진단 `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §19

### ⛔ 커맨드 개명 (파괴적 변경)
- **`/dz-tdd` → `/dz-unit-test`** — 종전은 RED→GREEN→REFACTOR(테스트 **선행**)였으나, A10 사내 위키 정식 항목(pageId 40470641)은 「테스트할 DAO를 **선택하여**」 만드는 **후행 검증**이다. 방향이 반대라 이름과 내용을 함께 바꿨다.
- 본문 전면 교체 — **§0 실행기 조합 확인**(`useJUnitPlatform()` + JUnit 4 + vintage 없음 = `0 tests` 로 조용히 통과) · JUnit 4 · `src/test/java` · **`testXxx()`**(A10 소스에 0건인 `should_…` 대체) · `@Test`+**`@Transactional` 자동 되돌림** · **멀티테넌트 세션 스텁**(`TestCommon.java`) · 검수(QA)와 층 구분
- ⛔ **「통과 관문이 아니다」를 본문 머리에 명시** — 개발표준 5종·coding-rules 7종에 테스트 의무 조항이 없다(2026-08-13 실측 0건)
- 참조 7지점 동반 교정(워크스페이스): 사용자가이드 md·HTML 4곳 · 업무-프로세스-맵 md·HTML 2곳 · **`forge-bridge/CLAUDE.md.tmpl`**(개발자 기기 배포 템플릿)

### 커맨드 A10 정합화
- **`commands/dz-guide.md` 전면 재작성** — 종전은 forge에 없는 구조(「로드맵(Section 7)」·「미결 의사결정(Section 9)」·스프린트 Step)를 읽으라고 했다. 영역 경계 표(`/dz-status`·`/dz-start-session`·`/dz-projects` 와 분리) + `CLAUDE.md` 「세션 시작 시 체크리스트」 기준 + ⛔ **세션 폴더를 모듈명으로 추측 금지**(실측 120개 중 일치 5개) + 근거 없이 안내 금지
- **`commands/dz-review-figma.md`** — §0 선행 확인(원격 MCP는 비공개 파일 불가 → 로컬 `127.0.0.1:3845`) · `clientFrameworks="react"`·`clientLanguages="typescript"`(React 16 + TS 4.4) · **절차 5-1 「A10 이식 가능성 판정」 신설**(Figma Make는 Tailwind+shadcn인데 A10은 SCSS+luna-orbit이며 퍼블리싱-코드스타일 §2가 Tailwind를 명문 금지) · 산출 위치·합격 게이트
- **`commands/dz-extract-design-tokens.md`** — `tailwind.config` 비교 → **대조 3곳**(designcenter `tokens/source/{base,semantic}` W3C DTCG·Style Dictionary 3 / `klago.scss` CSS변수 108건 / `참고자료/디자인자산/`) · **5 카테고리**(radius·shadow 추가)+WCAG AA · base hex 직접 사용 금지
- **`skills/dz-figma-make-reviewer/SKILL.md`** — 커맨드와 같은 옛 값 3곳을 동일 내용으로 교체(커맨드만 고치면 스킬이 옛 값을 계속 말한다)
- `commands/dz-verify-step.md` — 「`0 tests` 는 통과가 아니라 미수집 신호」 경고 + `/dz-unit-test` 연결

### 표기 정합
- **`sessions/{모듈}` 35지점 + `sessions/{작업}` 2지점 → `{대상}` 통일** (22파일). 정본은 `CLAUDE.md`(forge)이며 표기가 3갈래로 갈라져 있었다.
- **`commands/dz-init-project.md`** — 폴더 트리에서 **모듈 안 `sessions/`·`archive/` 제거**(폴더 구조 표준 룰 5 위반 — 2026-04-25 개인 영역 이전 완료분) + `유지보수/` 추가
- `hooks/HOOKS.md` — 미배선 훅 3 → **4**(`_selftest.sh` 누락분 추가, v1.19.0 신설)

### 검증 (실측)
- commands **46** · `name`↔파일명 **46/46** · 모듈 안 sessions 지시 **0**
- 훅 회귀 시험 **PASS 18 · FAIL 0**
- 플러그인→워크스페이스 상대링크 **0**(templates 예외 제외)

## v1.22.1 (2026-08-13) — 자리표시자 교정 범위 확장 (P1 후속 · 패치)

> 배경: v1.22.0 배포 후 검증에서 `assets/` 폴더가 P1 스캔 범위 밖이었음을 확인. 진단 `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §16

- `skills/dz-module-devenv/assets/module-guide-template.md` — `bash <스킬>/scripts/...` 2건을 셸 변수(`$SKILL`)로 교정. 꺾쇠는 셸 입력 리다이렉션이라 복사해 붙이면 `No such file or directory` 로 실패한다.
- 워크스페이스 동반 교정 16건(배포 무관, forge sync 로 전파): 사용자가이드 `C-개발실무.md`+`.html` 6 · `forge-bridge/README.md` 3 · 교육 데모 4 · CRM 개발환경 2 · 그 외 3.
- 워크스페이스 표준 개정: `경로-비종속-운영-표준.md` **§4.1.1 신설**(코드블록 안 꺾쇠·대괄호 금지 — 셸이 읽는 방식과 실측, 올바른 표기 3가지, grep 자가 점검). 종전 §4.1이 권장 예시로 `cd <워크스페이스 루트>` 를 가르쳐 **패턴을 재생산**하던 것을 끊었다.
- 워크스페이스 `.agents` 미러 정비: 플러그인 정본과 어긋난 19종 재생성 + 재생성 스크립트 `규칙/프로세스/scripts/sync-agents-mirror.py` 신설. CLAUDE.md·AGENTS.md 「이중 복제 금지」에 **미러 예외 명문화**, 배포 절차(§10)에 `_selftest.sh` 게이트와 미러 재생성 단계 추가.
- `.claude/skills` 중복 19종 삭제(미러 고유 `prj-*` 7종 보존) — 플러그인 스킬이 3판으로 갈려 폐지된 구조를 계속 안내하던 문제 해소.

## v1.22.0 (2026-08-13) — 실행하면 실패하던 명령·커맨드명 교정 + 모듈 골격 정본 일원화 (P1)

> Red Flags 점검 P1. 진단 `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge) §15

### 복사해 붙이면 실패하던 명령 — 15건 (P1-1)
- **꺾쇠 자리표시자 11건** → 셸 변수 + 실행 가능한 예시. `<forge>` 는 셸에서 **입력 리다이렉션**이라 `bash: forge: No such file or directory` 로 즉시 실패했다.
  - `dz-module-devenv`·`dz-deck-builder`·`dz-git-daily`·`dz-jira-query`·`dz-plugin-save` + 워크스페이스 표준 5종
  - pptx 변환기는 **세션마다 경로가 바뀌므로** 고정 경로 대신 `find` 로 잡도록 교체
- **대괄호 자리표시자 4건** → `M="모듈명"` 변수. `[모듈명]` 은 글롭(파일명 패턴)이라 **zsh 에서 `no matches found` 로 아무것도 만들어지지 않았다**(실측).

### 존재하지 않는 커맨드명 12건 (P1-2)
- `/init-project`·`/add-task`·`/update-context` 등 **접두사 없는 이름** → `/dz-*` 로 교정 (7개 파일).
- 위험은 실패가 아니라 **오작동**이었다 — 같은 이름이 개인용 solo-forge 플러그인에 실재해 엉뚱한 커맨드가 돌 수 있었다.

### 모듈 골격 — 폐지된 구조 재생산 차단 (P1-3·P1-4)
- `dz-project-bootstrap` Step 2 가 `{모듈}/sessions/archive` 를 만들어, **2026-04-25 에 모듈 밖으로 이전 완료한 구조를 새 모듈마다 되살리고 있었다.** 같은 파일 Step 3 은 `_개인/sessions/` 를 쓰라 해 자기모순이었다. 폴더 트리·mkdir 모두 룰 5 로 교정 + `유지보수/` 추가.
- `dz-session-manager` 의 파일 구조도 `[모듈명]/sessions/` → `_개인/sessions/{모듈}/`.
- 워크스페이스 동반: `CLAUDE.md` 골격·mkdir 을 룰 5 에 맞추고, 실측으로 갈린 선택 폴더를 룰 5 에 명시(`개발로드맵/` 12/23 · `시장조사/` 2/23 — 종전 CLAUDE.md 는 `시장조사/` 를 필수처럼 적고 `개발로드맵/` 은 어디에도 없었다).

### 온보딩 가이드 실패 경로 (P1 확장 — 2026-07-28 리포트 미처리분)
- `Forge-초기화-가이드.md` Step 1 clone 주소가 **미도달 옛 호스트**(`sbfigma.amaranth10.co.kr`) → `http://14.41.55.45:8089` + `--depth 1`(얕은 클론 표준).
- Step 7 은 **마켓플레이스 이름이 틀렸고**(`@douzone-forge`) `marketplace add` 선행 단계가 없어 설치가 되지 않았다 → 2줄로 교체, 버전 권장 표기 삭제(버전 숫자 미기재 방침).
- `플러그인-마켓플레이스-운영규칙.md`·`dz-plugin-save.md` 의 같은 오기도 교정.
- `CLAUDE.md` `_plugin/douzone-forge` → `../_plugin/douzone-forge`(워크스페이스 밖 형제 폴더임을 명시).

### 검증 (실측)
- 자리표시자 잔여 **0건** · 없는 커맨드명 **0건** · 모듈 안 sessions 생성 지시 **0건**
- 새 `mkdir` 블록을 임시 폴더에서 **실제 실행**해 룰 5 골격 전건 생성 + 모듈 안 `sessions/` 미생성 확인
- 훅 회귀 시험 **PASS 18 · FAIL 0** · 공용 트리 깨진 링크 증감 없음(143 = 123 + 타 세션 유입 20)

## v1.21.0 (2026-08-13) — 플러그인→워크스페이스 링크 정책 전환 + 전수 링크 스캔 자동 발동 + 승인 큐 폐기

> 배경: Red Flags 7항목 전수 점검(219파일)의 P0 교정. 진단 `참고자료/리포트/2026-08-13-forge-RedFlags-점검.md`(forge)

### 링크 정책 전환 (⛔ 파괴적 변경 — 표기 규약이 바뀜)
- **플러그인 자산이 워크스페이스를 가리킬 때 상대경로 링크를 금지**하고 백틱 평문 + `(forge)` 좌표 마커로 전환. **110건 치환.**
  - 실측 근거: 배포본 1.20.2 기준 워크스페이스 향하는 상대링크 **154건 중 해석 성공 0건**. 소스 트리에서 일부가 풀린 것은 관리자 기기의 `$HOME/Workspace` 심링크 때문인 우연이었다.
  - 종전 규정(`플러그인-자체-개발표준.md` §7 「`../../../douzone-forge/...` 로 통일 — 실측상 이 형식만 해석됨」)을 **폐기**. 그 실측이 심링크 있는 기기 기준이었다.
  - 규칙 3건 동반 개정: `경로-비종속-운영-표준.md` §4.4 신설 · `플러그인-자체-개발표준.md` §3·§7·§15 · `rules/markdown-link-policy.md` 적용 범위 예외
  - **예외**: `templates/`·`skills/*/templates/`·`skills/*/assets/` 는 복사 후 워크스페이스에 놓이는 전제라 상대경로 유지
- 부수 교정: 개명 미추종(`PRJ-2026-014_Amaranth10-Claude-Forge-구축` → `_douzone-forge-구축`) · 아카이브 이동 미추종(`코워크-클로드코드-협업-표준.md`) · `commands/` 내부 깊이 오류 2건(`../../skills/` → `../skills/`) · CHANGELOG 옛 경로 링크 4건 평문화

### 전수 링크 스캔 자동 발동 (신규 훅)
- `hooks/link-integrity-scan.sh` 신설 — **SessionStart(startup·resume) 세션 1회**, 공식 검사기 `규칙/프로세스/scripts/linkcheck.py`(forge) 호출, **증가분(델타)만** 보고.
  - 왜: 짝 훅 `link-integrity-check.sh` 는 PostToolUse 로 **방금 쓴 파일 1개**만 본다. 실측 — 2026-08-02 기준선 76건이 08-13 129건으로 11일 만에 +53건.
  - 왜 델타인가: 123건 중 48건이 Workspace_a10 소스를 가리키는 의도된 참조라 영구 잔존한다. 절대값 0을 목표로 하면 매 세션 소음이 된다.
  - 함정 방어: `cd` 필수(검사기가 `git ls-files` 기준이라 cwd 어긋나면 조용히 「0건」 거짓 통과) · 모수 0이면 무보고 · 기준선은 `.git/dz-linkcheck-baseline`(동기화 대상 아님) · 끄기 `DZ_LINKSCAN=0`
- `hooks/_selftest.sh` **[5/5] 신설** — 기준선·무음·증가 감지·**cwd 함정 회귀**·비-forge 무음 5종. 전건 **PASS 18 · FAIL 0**.
- `skills/dz-residual-audit/SKILL.md` 1축을 자체 정규식 → 공식 검사기 호출로 교체. 정규식은 CommonMark 파서가 아니라 균형 괄호를 오탐한다(실측 1,226 대 129).
- `hooks/link-integrity-check.sh` 안내 문구를 산출물 0건인 스킬 대신 실제로 도는 명령으로 교체.
- `hooks/HOOKS.md` 인벤토리 갱신(21 → 22) · `hooks/hooks.json` 배선 24 → 25

### 승인 큐 파이프라인 폐기
- `commands/dz-confirm.md`·`commands/dz-reject.md` **삭제** (commands 48 → 46).
  - 근거: `프로젝트/_mapping-queue.md`·`_mapping-history.md` 는 **워크스페이스 git 전체 이력에 추가된 적이 0건**. 스코어링 엔진은 `🚧 설계 완료` 상태로 실운영 이력이 없고(유일 산출물 = 엔진 도입 전 v0.8.1 드라이런), 엔진 입력인 PRJ `01.E 연계 키` 도 2026-04 폴더 압축 전환 때 사라져 **큐 파일을 만들어 줘도 성립하지 않는다.**
  - 40~69점 구간은 레포트 「⚠️ 수동 확인 필요」 표기로만 남긴다(파일 지속성 없음).
  - 동반 정리: `skills/dz-git-daily/SKILL.md`(승인 큐 지속성·학습 레이어 절 → 폐기 기록) · `SCORING-ENGINE-v0.9.md`(§5·§6 폐기 배너) · 워크스페이스 `참고자료/사용자가이드/md/F-레퍼런스.md`+`F-레퍼런스.html` 행 제거 · `규칙/프로세스/깃-커밋-메시지-규약.md` 2곳

### 검증 (실측)
- 배포 경로 시뮬레이션: 플러그인 밖으로 탈출하는 상대링크 **0건** (종전 154건)
- 훅 회귀 시험 `_selftest.sh`: **PASS 18 · FAIL 0**
- 공용 트리 공식 검사기: 깨짐 **129 → 123건** (Wehago 3 + Mullen 3 해소)

## v1.20.2 (2026-08-12) — 회의록 참석자 소속 조회 경로 교정 (a10 조직도 이름 필터 무시)

**계기**: 2026-08-11 회의록 3건 발행 중 `get_orgtree_detail`에 이름을 넣어 소속을 확인하려다 전사 명부를 받음. 원인 추적 결과 **도구 설명 자체가 그 경로를 지시**하고 있었다 (참고자료/리포트/2026-08-12-a10-조직도-이름필터-무시-진단.md · 차민수 수석 승인).

- **`dz-oneffice-write` 회의록 프리셋 7번 보강** — 「참석자 소속은 a10 라이브 조직도로 확인」에 조회 경로를 명시. 사람은 `find_emp_by_name`(이름→사번), 명단은 숫자 `dept_seq` + `include_lower_dept='Y'` 트리. ⛔ `get_orgtree_detail`에 이름을 넣어 좁히지 않는다 — 서버가 문자열 조건을 무시해 조직 전건이 **정상 응답 형태로** 돌아오므로 `totalCount`가 세 자리 이상이면 조건 미적용으로 판정.
- 같은 취지가 a10-mcp v0.11.2 도구 설명·CLAUDE.md·AGENTS.md·사용자가이드 E/G·회의록 원피스 템플릿 표준에 함께 반영됐다(라이브 대조 5종 실측).

## v1.20.1 (2026-08-01) — 동기화 가드 실효 격차 봉합 (환경종합점검 후속 안건 ⑤)

**계기**: 2026-07-29 a10_err.txt 실사고 — pre-commit 루트 가드(07-22 기신설)가 있어도 `core.hooksPath` 미활성 클론에서는 통째로 건너뛰어져 위반이 전사 배포됨 + 동기화가 커밋 차단 사유를 삼켜 사용자가 원인을 볼 수 없었음 (참고자료/리포트/2026-07-29-환경종합점검-실증.md · 차민수 수석 승인).

- **dz-gitlab-sync 훅 자가 활성** — 커밋 전 `core.hooksPath` 를 `.githooks` 로 멱등 설정. 미활성 클론이 다음 동기화 때 자동 봉합되어 가드(비밀·경로·루트·소실)가 전 사용자에게 실효.
- **커밋 차단 사유 표면화** — `git commit` 실패 시 pre-commit 의 차단 출력(최대 12줄)을 그대로 표시하고 동기화 로그에도 6줄 기록. "커밋 실패 — 보존" 한 줄만 남던 조용한 실패 봉합.
- **dz-personal-init v0.10.3** — 본인 식별 신호 우선순위 폴백 신설 (git user.email → git credential fill → 대화 맥락 → 1문 질문). 상위 신호가 있으면 질문하지 않는다.

## v1.20.0 (2026-07-31) — 퍼블리싱 확장자 확대 + R15 배선 + 제품 영역 2종 편입

**계기**: 2026-07-30 AI트렌드 #148(바이버스 AI 하네스 엔지니어링) 토론 중 개발 흐름 배선을 검토하다 발굴 (차민수 수석 승인).

- **퍼블리싱 계열 확장자 확대** — `code-quality-reminder`·`build-verify-reminder`가 `.java`·`.ts`·`.tsx`만 보아 **퍼블리싱Cell에 전혀 걸리지 않던** 사각을 해소. 근거: klago-pub-poc 실측 `.js` 2,072 · `.html` 86 · `.scss` 6 · **`.ts`/`.tsx` 0개**. 계열 분기 — 코드 계열(`.java .ts .tsx .js .jsx .vue`)은 기존 체크리스트, 마크업·스타일 계열(`.html .scss .css`)은 **퍼블리싱 체크리스트 신설**(시맨틱 마크업·접근성·반응형·크로스브라우징). 코드 체크리스트를 CSS에 띄우면 소음이 되므로 나눴다.
- **훅 회귀 시험 6 → 13항목** — `_selftest.sh`에 확장자별 발동 + **「무음이어야 할 때 무음인가」**(비대상 `.md`) 추가. 발동만 시험하면 확장자를 넓히다 소음이 나는 것을 못 잡는다.
- **R15 배선** — `dz-lessons-learned` 스킬에 「오진·사고일 때 체크 항목 누적」 3단계 절 신설, `dz-lesson` 커맨드에 유형 판정 분기. 동시에 두 자산이 가리키던 **「CLAUDE.md Section 10」이 존재하지 않는 위치**임을 실측하고 현행 구조(CLAUDE.md 「알려진 이슈 및 교훈」·`참고자료/리포트/`·`_개인/sessions/`)로 교정.
- **제품 영역 2종 편입** — `folder-structure-check.sh` 화이트리스트에 `Wehago`·`WehagoE` 추가(룰 1-A 요건 충족, 차민수 승인 2026-07-31). 워크스페이스 쪽 R14 연쇄 갱신(`.githooks/pre-commit`·폴더 구조 표준·CLAUDE.md)은 별도 반영 완료.
## v1.19.1 (2026-08-07) — dz-sync 저장소 건강 게이트 + 자가 해소 투명화 (패치)

**계기**: 2026-08-07 A1 공용 포지가 **분리된 HEAD**로 20시간 넘게 조용히 무동기였고, 그 상태로 채번해 AI트렌드 번호가 두 항목에 갈렸다. 복구 중에는 훅의 충돌 자가 해소가 원격 상세 기록을 소리 없이 지웠다. 진단: `참고자료/리포트/2026-08-07-A1공용포지-분리HEAD-동기화단절-채번충돌-진단.md` (차민수 수석 지시 2026-08-07 「재발 방지 4건 다 적용」).

- **분리된 HEAD·중단된 rebase 를 경고로 승격** — 종전에는 `분리된 HEAD 상태 — 건너뜀` 한 줄만 남기고 조용히 넘어가 **며칠이 지나도 무동기인 것을 아무도 몰랐다.** 이제 「동기화가 멈춰 있다 · 공용 자산화·채번을 보류하라 · 복구 절차 §6.4」를 명시하고 `sync-log` 에 사유를 남긴다. **브랜치가 붙어 있어도 rebase 잔해가 있으면 동일하게 멈춘다**(종전에는 감지 자체를 안 했다).
- **자가 해소 시 「무엇을 버렸는지」 기록** — 백업 폴더에 `버린판-요약.txt`(파일별 채택·버림 판과 줄 수)를 남기고, 대상이 **누적 기록**(`_timeline.md`·`AI트렌드/INDEX.md`·`_index.md`·`INDEX.md`·`_current.md`·`sync-log.md`)이면 「한쪽 판 채택으로 끝내지 말고 양쪽을 합치라」고 경고한다. 종전에는 세 판을 백업만 하고 **버린 내용의 요약이 없어** 사람이 소실을 알아채지 못했다.
- **복구 종료 시 「삭제줄 전수 검사」 안내** — 충돌로 걸리지 않는 조용한 되돌림이 있어(실사고: 다른 담당자 작업 8건·보고서 220행 포함) 병합·복구 뒤 확인을 유도한다.
- (워크스페이스 동반) `규칙/프로세스/Forge-GitLab-운영가이드.md` **§6.4 저장소 건강 자가 점검 · §6.5 병합·복구 뒤 삭제줄 전수 검사** 신설 · `참고자료/AI트렌드/INDEX.md` 채번 규칙 6번을 **「사내망이 붙었는가」에서 「받기가 실제로 됐는가」로** 개정하고 로컬·원격 최대값 대조 명령으로 교체 · 스케줄 루틴 2종(`a10-direct-channel-watch` §0.3 · `a10-messenger-monitor` §0.2)에 회차 시작 게이트 신설.
- 검증 — `hooks/_selftest.sh` PASS 6·FAIL 0 + 워크스페이스 밖 격리 저장소에서 4개 시험(정상 동작 유지 · 분리 HEAD 경고 · rebase 잔해 경고 · 버린 판 요약과 누적 기록 경고) 실측 통과.

## v1.19.0 (2026-07-29) — P0 안전 조치: 조용한 실패 4계열 차단 (차민수 세션 forge 개선 발굴 7축 후속)

**계기**: 2026-07-28~29 forge 개선 포인트 발굴(참고자료/리포트/2026-07-28-forge-개선포인트-발굴-리포트.md) — "조용한 실패" 패턴 8건 중 P0 4건 즉시 집행 (차민수 수석 승인).

- **무발동 훅 4종 수복** — link-integrity-check + 개발자 하네스 3종(code-quality-reminder·security-auto-trigger·build-verify-reminder)이 죽은 환경변수(`CLAUDE_FILE_PATH`)만 읽어 배선돼 있어도 영구 무발동이던 결함을 stdin JSON(`tool_input.file_path`) 우선 + env/$1 폴백으로 교정 (v1.14.1 교정 패턴 승계 — 당시 4종이 누락됐던 것. 2026-07-28 링크 17건 장기 잔존 실사고 배경).
- **훅 회귀 시험 신설** — `hooks/_selftest.sh`: 실제 훅 프로토콜(stdin JSON)을 파이프해 발동을 단언 (6항목). "배선돼 있으나 무발동" 5회 반복 사고의 재발 방지 장치. 배포 전 실행 권장.
- **build.sh 배포 3중 게이트** — ① 개발원본 더티 시 하드 중단 ② 개발원본·마켓플레이스가 origin 보다 뒤(미pull)면 중단 (타 기기 배포분 회귀 방지) ③ 직전 배포판 대비 소실 파일 검출 시 중단(NFC 정규화, 우회 `DZ_DEPLOY_ALLOW_REMOVALS=1`). "미커밋·미pull 정본 이원화" 4회 반복 사고의 근본 차단.
- **dz-gitlab-sync 신원 게이트** — git user.name 이 Amaranth 10 ID 형식(`[`로 시작)이 아니면 자동 커밋·올리기를 건너뛰고 /dz-personal-init 안내 (익명 fallback 제거, 우회 `DZ_SYNC_ALLOW_ANON=1`). 2026-07-28 공용 자산 98KB 0바이트 소실 실사고의 직접 경로 차단.
- (워크스페이스 동반) `.githooks/pre-commit` ④ 소실 가드 — 기존 추적 파일의 0바이트 축소 커밋 차단(우회 `DZ_ALLOW_TRUNCATE=1`) + 10KB+ 파일 90%+ 축소 경고. douzone-forge 레포에 직접 반영.

## v1.18.2 (2026-07-27) — v1.18.1 회귀 복구: 원피스 지면 폭·이미지 정본 되살림 (패치)

**v1.18.1 배포가 v1.18.0 에 살아 있던 원피스 개선 98줄을 되돌린 회귀를 복구했다.** 기능 추가는 없고, v1.18.0 배포본에 있던 내용을 소스 저장소에 **처음으로 커밋**해 다시 배포한다.

- **원인** — v1.18.0 은 **미커밋 작업 트리**에서 배포됐다. `build.sh` 는 작업 트리를 묶으므로 배포본에는 원피스 4파일 수정이 들어갔지만, 같은 날 커밋은 `plugin.json`·`CHANGELOG.md`·`hooks/dz-gitlab-sync.sh` 셋만 담았다. 이후 소스만 보고 작업한 v1.18.1 이 그 4파일을 v1.17.0 상태로 되돌려 배포했다
- **복구 대상 4파일** (v1.18.0 배포본에서 복원 + 커밋) — `skills/dz-oneffice-kit/SKILL.md`(561→608줄, `OF.pageMetrics()`·`OF.PAPER_MM`·`OF.mm2px`) · `skills/dz-oneffice-writer/SKILL.md`(1,147→1,175줄, v0.6.1→**v0.7.0**) · `skills/dz-oneffice-reader/SKILL.md`(397→411줄, v0.4.0→**v0.5.0**, 축약본 경고) · `commands/dz-oneffice-write.md`(170→179줄, 즉시 로딩·문서옵션 교체 지침)
- **v1.18.1 의 대시보드 성과는 그대로 유지** — `dz-progress-dashboard` v0.3.0(§8.1·§9.1·§9.2·함정 ⑧) 손대지 않음
- **CHANGELOG 사실 정정 2건** — ① 「(미배포 · 버전 재지정 필요)」 항목은 실제로 **v1.18.0 에 배포되어 있었다**(마켓플레이스 배포본 실측) → 「v1.18.0 동반 릴리스」로 정정하고 v1.18.0 아래로 이동 ② **v1.18.1 편집 때 실수로 지워진 v1.18.0(dz-gitlab-sync) 헤딩 복원** — 본문만 남아 앞 항목에 붙어 있었다
- **미해결로 남긴 것** _(확인 필요)_ — 위 정본이 전제하는 **a10-mcp v0.8.0 은 확인되지 않는다**(저장소·origin·구동 wheel 모두 v0.7.0, `eager_images` 파라미터 부재). 스킬 지침이 도구보다 앞서 있는 상태이며, 도구 쪽 확보·재구현은 별건이다
- **재발 방지** — 배포 전 **직전 배포본과 소스를 대조**한다(`diff -rq` 로 사라지는 파일이 없는지). 커밋하지 않은 채 배포하면 다음 사람이 그 내용을 모르고 되돌린다

## v1.18.1 (2026-07-27) — 진행 대시보드 「갱신 맵 주석 자기 파괴」 재발 방지 (차민수 수석 지시)

대시보드가 **"색이 안 먹는 화면"** 으로 열리는 사고를 스킬 차원에서 막았다. 계기: 2026-07-27 WEHAGO Q 발표문서 자산화 작업 — 스킬이 권하는 「갱신 맵 주석 1개」 방식을 문자 그대로 적용해 **주석 안에 마커 예시를 그대로 적었더니**, 그 예시의 주석 종료 기호에서 주석이 조기 종료되고 이어진 꺾쇠 태그명이 실제 태그로 파싱되어 **스타일 블록의 `:root` 변수 정의가 통째로 소실**됐다. 테마를 바꿔도 재현돼 **테마 문제로 오진**했다. (진단: forge `참고자료/리포트/2026-07-27-대시보드-갱신맵주석-CSS파괴-진단.md`)

- **`dz-progress-dashboard` v0.2.0 → v0.3.0**
  · **§8.1 갱신 맵 주석 작성 규칙 신설** — 주석 안에 **주석 종료 기호·꺾쇠 태그명 금지**(마커는 이름만 `마커쌍 DZ:NOW : 현재 위치`, 태그는 한글로 「스타일 블록」·「문서 제목 태그」). 본문에 실제로 심는 마커쌍은 **각각 독립 주석이라 무해**함을 함께 명시 — 금지 대상은 "주석 안에서 주석을 인용하는 것" 하나뿐. 안전한 갱신 맵 서식 예시 수록(교정 정본 인용)
  · **§9.1 표시 직후 자가 검증 3항목 신설** — ① `document.head` 자식에 문서 제목·스타일 태그가 있는가 ② `:root` 변수가 값을 반환하는가(빈 문자열이면 스타일 블록 파손) ③ 주요 요소 대비비 4.5 이상인가. 브라우저 계산값 조회 코드 + **브라우저 없이 쓰는 원본 패턴 탐지**(주석 안 주석 · head 맨텍스트) 병기 — 정상판·파괴판 양쪽으로 검증 완료
  · **§9.2 미학 기준 신설 — 대비 4.5 하한(물러난 톤 포함)** · 대기 배지·하단 고지문·단계 설명처럼 물러나야 하는 요소도 읽을 수는 있어야 한다. 위계는 유지하고 최저값만 올린다. **각 요소의 "제 배경"으로 잰다**(같은 색도 카드 위 5.26 / 페이지 배경 위 4.68). 실측 3건 재계산 확인(3.35→5.26 · 2.98→4.68 · 2.68→4.57)
  · **§10 함정 8종으로 확장** — ⑧ 갱신 맵 주석 자기 파괴(증상·원인·회피) 행 추가
- 검증: 파괴판 재구성 → 탐지 2신호 모두 적중 ✅ / 정상판 → 오탐 0 ✅ / 대비 계산식이 진단 리포트 수치 재현 ✅

## v1.18.0 (2026-07-23) — dz-gitlab-sync 자가 치유·진단 3종 (허정명 연구원 요청)

동기화가 **조용히 멈추는** 문제를 엔진 차원에서 해소했다. 계기: 2026-07-23 허정명 연구원 실사고 — 중단된 fetch가 남긴 `.git/refs/remotes/origin/main.lock` 때문에 모든 받기가 실패했는데 로그는 "네트워크 등"만 반복해 **원인이 몇 시간 가려졌고** 미푸시 커밋 5건이 누적됐다. (진단: forge `참고자료/리포트/2026-07-23-dz-sync-죽은-ref-lock-진단.md`)

- **① 죽은 잠금 자동 청소** — 받기 전 `.git/**/*.lock` 점검. **①실행 중 git 작업 없음 ②60초 이상 경과** 두 조건을 모두 만족할 때만 제거해 정상 작업 중인 잠금은 건드리지 않는다. 받기 실패 원인이 잠금이면 청소 후 1회 재시도. 끄기 `DZ_SYNC_LOCKCLEAN=0`, 나이 기준 `DZ_SYNC_LOCK_MIN_AGE=60`
- **② 오류 사유 구분 보고** — "네트워크 등" 뭉뚱그림 대신 **잠금 / 망 미도달 / 인증 실패 / 병합 충돌 / 얕은 클론**을 분류하고 대처법 1줄 + git 원문 첫 줄을 함께 표시(stderr 캡처)
- **③ 미푸시 누적 경고** — 원격에 못 올린 커밋이 3건 이상 쌓이면 🚨 경고 + 확인 명령 안내. 오프라인 경로에도 적용. 임계치 `DZ_SYNC_AHEAD_WARN=3`
- 실측 검증: 죽은 잠금 정리→받기 성공 ✅ / 최신 잠금 보호 ✅ / 끄기 스위치 존중 ✅ / 오류 분류 4종 정확 ✅ / 누적 4건 경고 ✅
- 문서: forge `규칙/프로세스/Forge-GitLab-운영가이드.md` §6.0 신설

## v1.18.0 동반 릴리스 (2026-07-23) — 원피스 지면 폭·이미지 실측 정본화 (문서옵션 조용한 무시 해소 + 즉시 로딩 자동화)

> **버전·배포 상태 정정 (2026-07-27 실측)** — 본 항목은 종전 「(미배포 · 버전 재지정 필요)」로 표기돼 있었으나, **실제로는 v1.18.0 배포본에 이미 포함되어 나갔다.** 확인 근거: 마켓플레이스 `douzone-forge-v1.18.0/` 안에 `OF.pageMetrics()`·축약본 절·즉시 로딩 지침이 모두 존재하고 writer v0.7.0·reader v0.5.0으로 올라가 있다. 다만 **소스 저장소에는 커밋되지 않아**(배포가 미커밋 작업 트리에서 나감) 소스만 보면 없는 것처럼 보였고, 그 탓에 v1.18.1 배포에서 통째로 회귀했다 → v1.18.2로 복구. 번호는 임의 지정이 아니라 배포본 실측으로 확정한 것이다.

AI-Hub 세션에서 접수한 원피스 개선 2건(지면 폭이 좁게 나옴 · 이미지가 첫 화면에 안 뜸)을 사내망 라이브 실측(시험 문서 20여 건)으로 규명했다. **접수 진단 중 폭 건은 원인이 달랐고, 이미지 건은 정확했다.** 실측으로 확정한 사실을 도구·스킬 정본에 반영했다.

- **폭 — "자동 계산"이 아니라 "문서옵션 조용한 무시"가 원인이었다** (a10-mcp `create_oneffice_doc`)
  · 12조합(A4·A3·B5·LETTER × 좁게·보통·넓게) 전건 실측 결과, **주입 루트에 폭을 지정하지 않아도 지면을 정확히 채운다**. 접수된 "컨테이너가 지면을 못 채운다"는 재현되지 않았고, 마스트헤드 축소 대조(64px→13px)로 내용 폭 탓(shrink-to-fit)도 기각
  · 관찰된 971px은 **A3·보통(20mm)의 정상 폭**이었다 — 즉 `narrow`(10mm)가 적용되지 않은 상태. 원인은 **본문 HTML에 `<dze_doc_property>`가 있으면 용지·여백·단일페이지·가로 네 파라미터를 통째로, 경고 없이 무시**하던 분기(이전 판 HTML 재사용 시 그대로 걸림)
  · **개선**: 옵션을 하나라도 명시하면 **본문의 것을 교체**하고, 아무것도 명시 안 하면 본문을 존중하며, 결과를 응답 `docProperty`에 표기. 회귀 방지 시험 2건 추가
- **이미지 — 즉시 로딩 자동 부여** (a10-mcp `create_oneffice_doc` `eager_images=True` 기본)
  · 편집기가 `loading` 없는 `<img>`에 **`lazy`를 자동 부여**해 **외부 주소 이미지가 열람 화면에 뜨지 않는다**(첫 화면 안이든 밖이든). 접수 진단이 정확했다
  · `loading="eager"` 명시는 **보존되며 정상 로드**된다(실측) → 도구가 자동 부여. `figure`·인라인 `style`·`width`/`height` 속성·**내장 base64도 모두 보존**되고 본문 **4MB까지 안전**(3000×2200 이미지 완전 보존 확인)
  · **PDF 출력은 서버 렌더**라 지연 지정과 무관하게 이미지가 모두 포함된다 — 접수 프롬프트가 우려한 인쇄 누락은 실측상 발생하지 않으며, **화면 열람 한정 문제**임을 확정
- **`dz-oneffice-kit` v0.1.0 → v0.2.0** — 하드코딩 폭 프리셋 4종을 **`OF.pageMetrics()` 계산으로 대체**. 공식 `px = ceil(mm × 96/25.4)`(**반올림 아님** — 넓게 30mm는 반올림 113 ≠ 실측 114, B5는 665 ≠ 666), 12조합 자동 지원. 종전 프리셋은 실측보다 **+2px 어긋나** 있었고 A3·좁게 등 8조합이 아예 없었다. `alignBracket`은 **예외 상황 전용**으로 재정의(폭이 좁으면 정렬로 덮기 전에 여백 설정부터 의심)
- **`dz-oneffice-reader` v0.4.0 → v0.5.0 · `get_oneffice_content` 설명 보강** — `one001A45`가 **원본이 아니라 축약본**임을 명시. `<style>`·구조 태그·인라인 style·loading·**내장 이미지의 src 값**이 서버에서 제거된 채 온다. 텍스트·표 맥락화엔 적합하나 **재편집용 원본·서식 보존 자산화엔 부적합**(이 축약본을 원본으로 오인해 "내장 이미지가 저장되지 않는다"고 오판한 실사례 수록)
- **`dz-oneffice-writer` v0.6.1 → v0.7.0 · `dz-oneffice-write` 커맨드** — 폭 실측표(12조합)·문서옵션 교체·이미지 즉시 로딩 표준 수록. **"폭이 좁아 보이면 폭을 강제하지 말고 여백 설정을 먼저 확인"** 지침 신설
- **연계: a10-mcp v0.7.0 → v0.8.0** (시험 147건 통과) · 실측 리포트 = forge [`2026-07-23-원피스-지면폭-이미지-개선계획.md`](참고자료/리포트/2026-07-23-원피스-지면폭-이미지-개선계획.md) 부록 A·B
  · ⚠️ **a10-mcp v0.8.0 은 확인되지 않는다** _(확인 필요 — 2026-07-27 실측)_: 관리자 기기의 a10-mcp 저장소·origin·**실제 구동 중인 wheel 이 모두 v0.7.0** 이고 `eager_images`·`docProperty` 가 0건이며, 구동 도구 설명문에 `<dze_doc_property>` **무시** 동작이 그대로 남아 있다. 따라서 본 항목의 **스킬 쪽 지침은 살아 있으나 도구 쪽 뒷받침은 미확인** 상태다 — `eager_images=True` 호출은 현재 구동본에서 실패하고, 문서옵션 조용한 무시 함정도 아직 해소되지 않았다. 다른 기기에 v0.8.0 이 있는지 확인하거나 재구현이 필요하다

## v1.17.0 (2026-07-23) — 아마링크(관련 문서) 카드 헤드리스 삽입 기법 정본화

원피스 본문에 '관련 문서'를 **파란 아마링크 카드**로 넣는 정본을 `dz-oneffice-kit`(OF 라이브러리)에 신설하고 writer·write 커맨드에서 인용. 정현수 전무 미팅 회의록의 관련 문서 실작업에서 실측 확정: **`resolve_amalink` 의 convertText 마크업(`|>@…@<|`)을 본문 HTML 에 넣으면 헤드리스 `create_oneffice_doc`·편집기 로드 모두 문자 그대로 남고 카드로 변환되지 않는다**(원피스 서버는 content 저장만, 파싱은 편집기 붙여넣기 담당). **cmd+v 붙여넣기 변환도 자동화(신뢰 키 요구)에서 불가.** → **완성된 `dze_amalink` 앵커 요소를 HTML 에 직접 주입**해야 렌더·클릭된다(class·href·style 보존 실측).

- **신규 — `skills/dz-oneffice-kit/SKILL.md` (OF 라이브러리 kit-0.1.0 → kit-0.2.0)**
  · `OF.amalinkAnchor({url, title, moduleGbn, icon})` 빌더 추가 — `dze_amalink` 앵커 HTML 문자열 생성(파란 글자 `rgb(28,144,251)` + 연파란 배경, 📄 아이콘, `?token=` 상대 공유링크).
  · **§5 「아마링크(관련 문서) 카드 삽입」 신설** — 실패 모드 2건(convertText 마크업 문자 잔존 · cmd+v 붙여넣기 불가) + 정본 앵커 구조 + `resolve_amalink` 로 제목·링크·moduleGbn 확보 절차 + DOM 인스펙션 시 URL 반환 금지(Chrome MCP `[BLOCKED]` 회피). §2 함수 표 행 추가.
- **보강 — `skills/dz-oneffice-writer/SKILL.md` Step 3.3** — 기존 "탭 URL 을 `<a href>`" 지침은 **평문 링크**임을 명시, 카드는 kit §5 앵커(`OF.amalinkAnchor`) 사용(트랙 A·B 공통).
- **보강 — `commands/dz-oneffice-write.md` 공통 1** — 관련 문서 아마링크 삽입 항목 추가(kit §5 인용).

## v1.16.0 (2026-07-22) — 원피스 조회 헤드리스 전환 + 문서 옵션·크롬 자동 표시

원피스(ONEFFICE) 문서 **조회**를 브라우저(Chrome DOM) 대신 **a10 MCP 헤드리스 경로 우선**으로 전환. 텍스트·표는 `get_oneffice_content`(`one001A45`), 이미지는 `download_oneffice_images`(`one006A02` 서명 GET) + Read 시각 판독으로 **브라우저·Canvas 없이 텍스트+표+이미지까지 완전 맥락화**(라이브 실측 확정 2026-07-22). 아울러 **문서 옵션(용지·여백·단일페이지·가로)을 생성 파라미터로 확정**하고 **생성 후 크롬 자동 표시**를 기본 동작으로 정했다.

- **문서 옵션 생성 파라미터화 + 크롬 자동 표시** (`dz-oneffice-write` 커맨드 · `dz-oneffice-writer` v0.6.0 → v0.6.1)
  · `paper`(A4·A3·B5·LETTER)·`margin`(narrow 10/normal 20/wide 30mm)·`onepage`(단일 페이지)·`landscape`(가로)를 **생성 시점에 확정** — `dze_doc_property` 주입이 그대로 저장됨을 라이브 실측. 브라우저 문서설정 팝업 불필요
  · 커맨드 옵션 `--paper`·`--margin`·`--landscape` 신설, `--long` → `onepage=True` 매핑
  · **생성 후 편집기를 크롬으로 항상 열되, 추가 검증(스크린샷·재조회)은 매번 하지 않음**(차민수 수석 지시)
  · **트랙 B 폴백 사유 축소** — 단일페이지·회의록 골격이 트랙 A로 이동해 **재편집·MCP 미가용 2건만** 남음. 회의록 프리셋도 `paper="A4", onepage=True, margin="narrow"` 로 트랙 A 처리

- **`dz-oneffice-reader` v0.3.0 → v0.4.0** — 상단 「🚀 우선 경로」(a10 MCP 헤드리스: 텍스트·표 + 이미지 판독) 신설. 아마링크 URL→fileUID base64 디코드→조회. 기존 Chrome DOM 방식은 **폴백**(MCP 실패·권한 부족·렌더 화면 확인·추가 확인)으로 재정의(「최우선 규칙」→「폴백 경로 B 최우선 규칙」)
- **CLAUDE.md 4곳 갱신** — 스킬 매핑 표(원피스 **조회** 행 신설 + **발행** 트랙 A 반영), 아마링크 필수 규칙 핵심 요약·분석 최우선 확인·알려진 이슈에 "a10 MCP 우선, Chrome 폴백" 반영
- **연계: a10-mcp v0.5.0 → v0.6.0** — ① `download_oneffice_images` 도구 신설(`client.download_get` GET 바이너리 서명 헬퍼 + `one006A02` path 서명, base64 인라인 제외) — 원피스 이미지를 헤드리스 다운로드→로컬 저장→Read 판독 ② `create_oneffice_doc` 에 문서 옵션(`paper`·`margin`·`onepage`·`landscape`) 파라미터 추가 ③ 원격 v0.5.1(`list_onechamber_folders` 하위 폴더 진입 결함 수정 — `ecm005A03` 경유) 병합 — 세 변경이 모두 포함된 단일 빌드본

## v1.15.0 (2026-07-22) — 원피스 발행 A10 직접 생성 경로 전환 (create_oneffice_doc 우선)

원피스(ONEFFICE) 문서 신규 발행을 리트머스(AI Hub) 경유·브라우저 DOM 주입 대신 **a10 MCP `create_oneffice_doc`(생성 API `one001A01`, 헤드리스)** 우선 경로로 전환. 함수 한 번으로 문서 생성 + 본문(HTML) 주입 + 저장이 끝난다(라이브 실측 확정 — forge `참고자료/리포트/2026-07-22-A10원피스-직접생성-실측확정.md`).

- **`/dz-oneffice-write` 커맨드 2-트랙 개편** — 트랙 A(A10 직접, 함수 한 번) / 트랙 B(브라우저 폴백). 트랙 선택 규칙 명시: 신규·표준설정(A4)=트랙 A / 재편집·단일페이지·회의록 골격·MCP 미가용=트랙 B. 「치명적 주의」는 트랙 B(브라우저) 한정으로 재범위화
- **`dz-oneffice-writer` v0.5.4 → v0.6.0** — 상단 「🚀 우선 경로」 안내 신설(create_oneffice_doc 우선, 이 스킬 DOM 주입은 폴백·재편집용). HTML 형식·콘텐츠 원칙·모드 C 평면은 두 경로 공통
- **`dz-oneffice-new-doc-opener` v0.2.0 → v0.2.1** — 신규 발행은 빈 탭 확보 불필요(트랙 A), 본 스킬은 트랙 B 폴백용임을 명시
- **이점**: 브라우저 탭 확보·편집모드 전환·꺾쇠 정렬·저장 버튼 클릭·문서명 커밋 전부 불필요. 저장 클릭 함정·읽기모드 no-op·체크아웃 경쟁 원천 제거
- **연계**: a10-mcp v0.5.0(`create_oneffice_doc`/`delete_oneffice_doc` 신설 — `one001A01`/`one001A10`)

## v1.14.3 (2026-07-15) — prj-filename-policy 훅 정교화 — 파일명 표준화 배치 연계 (패치)

워크스페이스 PRJ 산출물 파일명 표준화 배치(211건 YYYYMMDD- 정규화, 정책 A)와 함께 훅 규칙 정교화:

- **`_current.md` 화이트리스트 추가** — 세션 체크포인트 표준 파일명인데 경고 대상이던 허위 경보 해소
- **PRJ `0[1-6]_*/` 하위는 직속 파일만 검사** — 하위 묶음 폴더(날짜 폴더·연구 묶음 등)는 폴더가 맥락을 담보하므로 면제(감사 §C-10 권고). 종전엔 묶음 내부 수십 건이 허위 경보 대상이었음
- 실측 4케이스 검증: 직속 위반=경고 / 직속 표준=침묵 / 하위 묶음=면제 / _current.md=면제

## v1.14.2 (2026-07-15) — deliverables/ 폐기 사문(死文) 제거 — 훅 자기모순 해소 (패치)

`structure-awareness.sh`가 `deliverables/`를 "옛 경로, 분배 폐기(Q-2 Q-13)"로 경고하던 사문을 제거(L37). 같은 플러그인 `folder-structure-check.sh`는 `deliverables/`를 정본 공용 루트로 허용해 훅 2개가 정면 충돌했고, 무엇보다 플러그인 자신이 `/dz-daily-digest`로 `deliverables/보고서/일일보고/원본캐시/`에 파일을 쓴 뒤 곧바로 "폐기됨"이라 경고했다(임직원이 산출물 위치를 헷갈리는 직접 원인).

- Q-13(2026-04-27)의 deliverables/ 폐기는 V-06 Phase V-06-03(2026-05-11)에서 되돌려졌고 2026-05-22 폴더구조표준이 정본 루트로 확정 — 물증: `deliverables/보고서/일일보고/원본캐시/_README.md`("종전 `_개인/보고/Q-3이연/…` 로컬 only 영역을 공용 표준 위치로 정합화", Q-3이연이 곧 Q-13 이관처)
- 동반: `rules/phase-q-structure.md` 옛 경로 표 #5행(deliverables) 제거 + 변천사 기록
- 계기: 2026-07-15 워크스페이스 오배치 전건 조사(douzone-forge `참고자료/리포트/2026-07-15-워크스페이스-오배치-정리-조사.md`)

## v1.14.1 (2026-07-14) — 배치·파일명 경고 훅 3종 무발동 결함 교정 (패치)

배치·파일명 경고 훅 3종(`output-location-policy`·`folder-purpose-check`·`prj-filename-policy`)이 배선돼 있는데도 **조용히 무발동**이던 결함 교정. 세 훅이 Claude Code가 설정하지 않는 죽은 환경변수(`CLAUDE_FILE_PATH`)만 읽어 경로가 늘 비어 즉시 종료 → 경고가 한 번도 뜨지 않았음(v1.11.2에서 `structure-awareness.sh`만 교정, 나머지 3종 누락). 입력부를 **stdin JSON(`tool_input.file_path`) 우선 + env/`$1` 폴백**으로 통일(`structure-awareness.sh`와 동일 패턴).

- **실측**: 3종 모두 정상 발동(모듈 하위 산출물·PRJ 분기 날짜없는 파일명·_README 부재 경고) + 정상 경로 침묵 + 터미널 stdin 무한대기 없음 확인
- **계기**: 2026-07-14 파일 오배치 상시 문제 진단. Windows clone 실패(폴더명 U+0008 제어문자)와 함께 접수돼 조사. clone 방어는 워크스페이스 `.githooks/pre-commit` 경로 위생 검사 + `server-pre-receive.sample.sh`(L3)로 별도 처리 — 진단: douzone-forge `참고자료/리포트/2026-07-14-경로-크로스플랫폼-호환.md`

## v1.14.0 (2026-07-13) — 원피스(ONEFFICE) DOM 공용 배관 정본(dz-oneffice-kit) 신설 _(CHANGELOG 소급 기록 2026-07-14)_

원피스 6종 스킬이 각자 구현하던 중첩 iframe(문서 안 문서 창) 접근·편집모드 판정/전환·주입·꺾쇠 정렬·문서설정·저장·읽기·셀렉터/좌표/프리셋을 `window.OF` 하나로 통합.

### 신규 — `skills/dz-oneffice-kit/SKILL.md`
- 원피스 DOM 공용 배관 정본(OF 라이브러리). 사내망 실측 검증 통과(§3.5), 미확인 후속 항목 §4 명시.

### 변경 — 원피스 스킬 6종 (OF 인용 전환 + 저장 절차 2단계 정정, 인라인 폴백 유지)
- `dz-oneffice-writer`(v0.5.4)·`dz-oneffice-new-doc-opener`(v0.2.0)·`dz-oneffice-form-writer`(v0.2.0)·`dz-html-oneffice-builder`(v0.2.0)·`dz-oneffice-reader`(v0.3.0)·`dz-oneffice-comment`(v0.2.0)
- **재편집 저장 병목 진짜 원인 규명**: 재열람 시 문서가 읽기(R)모드로 열림(`isContentEditable=false`) → 편집 토글 JS `.click()` 전환으로 저장 정상 작동(저장 API `one001A17`). 기존 "재편집은 실제 마우스 저장만 유효" 오정보 정정.

> 별도 세션(원피스 생성방식 개선) 산출. 최초 배포 시(마켓 `d901f9a`) CHANGELOG 누락 → 본 항목 소급 기록. 검증: OF 라이브러리 사내망 실측 + writer 스모크 통과. 미검증 항목(form-writer 직접 저장·`pageSetup` 팝업·편집탭 좌표 드리프트·편집모드 신호 동치)은 kit §4 후속. 6종 전환은 인라인 폴백 유지라 OF 미주입 시 기존 동작 보장.

## v1.13.1 (2026-07-10) — templates/ 패키징 + HOOKS.md 안내처 교정 (패치)

- **templates/ 8종 배포 편입** — 사용자 가이드 E6이 안내하는 "위치 templates/(플러그인)"이 실제 배포본에서 거짓이던 문제 시정(2026-07-10 전체 재검증 R2, 차민수 결정 (a)): build.sh 패키징 목록에 templates/ 추가. 요구사항-정의서·화면설계서·디자인-토큰·퍼블리싱-자산·개발계획서·검수-시나리오·배포-보고·유지보수-검토안 8종이 설치본에 포함됨
- **HOOKS.md §2 안내처 교정**(재검증 R11 원류) — commit-message-check 예외 사유의 "커밋 규약은 룰 계층에서 안내"가 사실과 다름(룰 13종에 커밋 규약 없음) → CLAUDE.md + 깃-커밋-메시지-규약.md(SSoT)로 정정. 가이드 E4가 이 오류를 전파했던 재유입 경로 차단

## v1.13.0 (2026-07-09) — progress-dashboard-remind 훅 신설: 진행 대시보드 3중 상시 인지 셋째 다리 복원

### 신규 훅 — progress-dashboard-remind.sh (UserPromptSubmit)

진행 대시보드(dz-progress-dashboard) 자동 발동을 매 입력마다 환기하는 1줄 주입 훅. 배선 20 → **21**.

- **배경**: 3중 상시 인지(CLAUDE.md 절 + 스킬 + 매 입력 환기, 2026-06-01 v1.3.0 확정) 중 셋째 다리가 워크스페이스 로컬(`.claude/진행대시보드-환기.md` + settings.json 두 번째 cat)로만 구현돼 있었음 — `.gitignore`가 `.claude/`를 전체 제외해 git 이력·동기화 없이 **조용히 소실**, 복잡·다단계 작업에서 대시보드 상시 미발동(2026-07-09 사용자 질의로 적발. 진단: douzone-forge `참고자료/리포트/2026-07-09-진행대시보드-환기훅-소실-진단.md`)
- **설계**: force-rules-inject 패턴(메커니즘=플러그인 → 설치만으로 전 직원 배선) 승격 + **기본 문구 스크립트 내장** — 외부 파일이 없어도 절대 침묵하지 않음(소실 사고의 구조적 재발 방지). `규칙/프로세스/진행대시보드-환기.md` 존재 시 그 내용 우선(문구 갱신은 재배포 없이 GitLab 동기화로 전파)
- **검증**: 격리 테스트 4종 전건 통과(기본 문구·override 우선·`CLAUDE_PROJECT_DIR` 미설정 set -u 내성·실워크스페이스) + hooks.json JSON 유효성
- HOOKS.md §1(21)·§5 신설 갱신

### 위생 — v1.12.4 CHANGELOG 누락분 소급 재구성

v1.12.4는 코드·버전만 push되고 CHANGELOG 항목이 누락됐었음 — 아래 소급 작성(변천사 보존).

## v1.12.4 (2026-07-09) — dz-gitlab-sync 미해결 충돌 자동 복구(rc.24) _(소급 재구성 2026-07-09)_

- autostash(자동 임시보관) 복원 충돌이 미해결(unmerged) 파일을 남기면 git이 pull 자체를 거부해 **자동 동기화가 조용히 전면 중단**되던 사각지대 해소(2026-07-09 실사고 — 포지교육 회의록 파일명 변경+수정 중 pull)
- 감지 3지점(실행 초입·받기 성공 후 잔존·받기 실패 후) → 3판 백업(`_개인/sync-conflict-백업/`) → 로컬 작업 판 채택 자가 해소 → 1회 재시도 + 성공 자가 검증
- 한글 경로 8진수 이스케이프(core.quotePath) 함정 교정 · 끄기 `DZ_SYNC_AUTORESOLVE=0` (SSoT: `Forge-GitLab-운영가이드.md` §6.2)

## v1.12.3 (2026-07-06) — HTML→원피스 "사실 전용 + 인포그래픽" 콘텐츠 원칙 전사화

> 리걸테크에디션 사장님 주간보고 준비 중 차민수 수석이 명시적으로 확정한 원칙을 전 임직원에게 적용되도록 스킬·커맨드에 반영: **md 파일(회의록·계획서 등 작업 기록)까지는 다각도로 자유롭게 작성 가능하나, HTML을 만들어 원피스로 주입하는 단계부터는 반드시** 자비스-사용자 소통 흔적(질의·비판적 검토·`_(확인 필요)_` 메타 서술)을 걷어내고 확정된 사실만으로 재구성하며, 시각화가 이해를 돕는 지점에는 인포그래픽을 삽입해야 한다. 기존에는 "회의록 프리셋" 한정으로만 있던 규칙이었으나, 모든 HTML→원피스 변환에 적용되는 일반 원칙으로 승격.

### 변경 — `skills/dz-oneffice-writer/SKILL.md` (v0.5.1 → v0.5.2)
- 신규 섹션 「⛔ 콘텐츠 원칙 — 사실 전용 + 인포그래픽」 추가 (배경 섹션 직후, HTML 가공 원칙 이전) — 4원칙(사실 전용·내부 코드 제거·인포그래픽 삽입·적용 경계) 명문화.

### 변경 — `commands/dz-oneffice-write.md`
- 신규 섹션 「⛔ 콘텐츠 원칙」 추가 — 기존 「회의록 프리셋」의 "공유 가능한 사실만" 규칙이 이 원칙의 회의록 특화 사례임을 명시.

## v1.12.2 (2026-07-06) — dz-oneffice-writer 모드 C 평면 주입 실측 함정 3건 반영 (패치)

> 2026-07-06 리걸테크에디션 개발현황 원피스 문서 재작업 중 실측 발견. 모드 B(컨테이너 보존, `width:973px !important` 고정)로 만든 문서가 편집 화면은 정상이나 모바일·PDF 변환 시 우측 여백 과다 증상(v0.4.0 기 문서화 증상)이 재발해 모드 C(평면 주입)로 재작업하며, 스킬 문서에 없던 신규 함정 3건을 발견·해결·수록.

### 변경 — `skills/dz-oneffice-writer/SKILL.md` (v0.5.0 → v0.5.1)
- **치명적 함정 5 신설** — 저장 직렬화 시 빈 장식용 `<span>`은 삭제, `<b>`/`<strong>`은 통째 언랩(태그만 사라지고 부모 안에서 줄바꿈 유발), `<div>`만 항상 생존. 장식용 span·b·strong은 `<div style="display:inline">`(또는 `inline-block`)로 치환 필요. 검증은 저장 직후가 아니라 반드시 새로고침 후 DOM 재조회 — 저장 직전 캐시 상태로는 문제가 보이지 않는다.
- **모드 C 4원칙 보강 — `getComputedStyle()` 절대값 고정 주의** — 넓은 브라우저 창에서 값을 구우면 원본 CSS의 `margin:auto`·`grid-template-columns`(`1fr` 등 상대값)가 그 순간 viewport 기준 절대 px로 고정돼 오버플로가 재발한다. grid/auto-margin을 쓰는 클래스는 사전 목록화 후 원본 상대값으로 명시 오버라이드 필요.
- **Step 5 CORS 서버 `charset=utf-8` 누락 함정** — `SimpleHTTPRequestHandler` 기본 응답에 charset 미지정 시 한글이 `U+FFFD`로 영구 손상(재로드해도 복구 안 됨, 캐시버스트 쿼리스트링 강제 재요청 필요). `guess_type` 오버라이드 스니펫을 Step 5에 추가.
- 디버깅 체크리스트 3건 추가 + 검증된 상수 요약 표(CORS 서버 행) 갱신.

## v1.12.1 (2026-07-04) — 고아 훅 4종 배선(경고 2 + 차단 2) + v1.12.0 회귀 대체

> **배경(재정합)**: 앞서 v1.12.0(고아 훅 4종 배선)이 **v1.11.2 트리에서 빌드·배포**되어, 그 뒤 dev 레포에만 커밋된 **v1.11.3(dz-personal-init 결함 2건 교정)이 배포본에서 누락(회귀)**됨. v1.12.1은 v1.11.3 위에 훅 배선을 재적용해 **양쪽을 모두 담아 v1.12.0을 대체**한다. (v1.12.0 마켓플레이스 디렉토리는 유효 최신본에서 제외 — marketplace.json이 v1.12.1을 가리킴.)

### 변경 — `hooks/hooks.json` (고아 훅 4종 배선, v1.12.0 내용 계승)
- **경고 2종 (PreToolUse Write|Edit)**: `simple-approval-md-block`(검토의견 .md 단순 결재 경고) · `umbrella-vocab-block`("우산 X" 어휘 → "마스터 X" 권고). exit 0 경고만.
- **차단 2종 (PreToolUse Write)**: `memory-rule-content-block`(메모리 룰성 본문 차단 — 화이트리스트: 본인 역할·KPI 등 통과) · `v2-version-bump-block`(같은 사이클 v2 계획서 신규 작성 차단 — archive·변천사·Edit 예외). exit 2 차단. 격리 환경 오탐 테스트 통과(memory 5케이스·v2 6케이스).
- 배선 결과: **19 활성** + 구조적 예외 2. HOOKS.md §1·§2 갱신.

### 구조적 예외 2종 (배선 불가 — 결함 아님)
- `commit-message-check`: Claude Code에 커밋 시점 이벤트 부재.
- `external-report-check`: 평문 stdin 검사기(훅 JSON 프로토콜 미준수) — 배선 시 SSoT·D-NN·에이전트명 든 모든 내부 .md Write에 오발(경로 필터 없음). dz-external-report 스킬 보조 검사기로 유지.

### 포함 — v1.11.3 교정 유지(회귀 해소)
- `commands/dz-personal-init.md`(§9 python 버전 판정 전환 — 3.13+ 오탐 해소) · `skills/dz-people-context/SKILL.md`(`_개인/` 폴더명 `{이름}_{ID}` 정정) — v1.12.0에서 누락됐던 교정을 v1.12.1에 포함.

## v1.11.3 (2026-07-03) — dz-personal-init 재실행 실측 결함 2건 교정 (패치)

> 2026-07-03 dz-personal-init 재실행 실측에서 확인된 결함 2건 교정. 대상: `commands/dz-personal-init.md`(v0.10.0 → v0.10.1) + `skills/dz-people-context/SKILL.md`(v0.3.0 → v0.3.1 — 동일 폴더명 규정 공유).

### 변경
- **§9 자가 진단 python 버전 체크 오탐 해소** — 종전 `command -v python3.12 || python3.11 || python3.10` **이름 매칭** 방식은 python 3.13+ 단독 설치 환경에서 거짓 ⬜(미충족) 판정. `python3 -c "import sys; exit(0 if sys.version_info>=(3,10) else 1)"` **버전 판정** 방식으로 교체 — 어떤 3.10+ 배포(3.13·3.14 포함)든 정확 판정.
- **§5.2 폴더명 규정 정정 `{본인이름}_{사번}` → `{본인이름}_{ID}`** — dz-personal-init 5곳(서두 요약·§5.2 제목/본문·§5.3·§5.4) + dz-people-context 4곳(frontmatter·1순위 lookup·부하 식별·의존 자산). **정본 판정 = {이름}_{ID}** (ID = xlsx 13번 사원명(ID), Amaranth 10 ID — 사번 아님). 근거 3중: ① 사용자ID-매핑.md §1 Q-01 표준(2026-04-27, ID 최우선 식별자) ② 같은 SSoT의 폴더명 예시 자체가 `신무광_smkgood2`(ID 형식 — 그의 사번은 B4459) ③ 정착 실측 전건 일치(`_개인/차민수_cjrain/` + 조직 트리 재적자 전원 {이름}_{ID}, 사번 형식은 `조직/archived/` 퇴사자 2건뿐). 종전 표기는 D9 변천사 어휘("이름_사번")가 Q-01 ID 전환 후에도 문서에 잔존한 것. 각 파일 변천사 항목 원문은 보존.
- **_R&R.md "a10-personal-init" 스킬명 오타 (참고)** — 플러그인 원본 전건 grep 0건(원본 무결). 정착본(`_개인/차민수_cjrain/_R&R.md`) 한정 오타였으며 2026-07-03 재실행 재정착으로 이미 해소 — 워크스페이스 전역 grep 0건 확인. 플러그인 측 조치 불요.

### 잔여 과제 (별도 승인 사이클)
- `조직/{이름_사번}/` 표기 동일 계열 어휘 잔존 — 워크스페이스 CLAUDE.md·사용자ID-매핑.md §1 문구("폴더명은 사번을 그대로 사용" — 자기 예시·실측과 모순)·플러그인 dz-people-index/dz-org-* 등 광범위. 실폴더는 전부 {이름}_{ID}이므로 표기 일괄 정정은 R14급 cascade로 이관.

## v1.11.2 (2026-07-03) — R14 이행: 옛 경로 감지기 보강 + structure-awareness 무발동 결함 수복 (패치)

> forge 연쇄 갱신 SSoT v2.1.0의 **R14 신설**(워크스페이스 폴더 개편 → 플러그인 자산 참조 cascade — v1.11.1 잔존 33파일 사고 재발 방지) 이행분. 감지기 소급 보강 중 **structure-awareness.sh의 프로덕션 무발동 결함**을 추가 발견·수복.

### 변경
- **`hooks/structure-awareness.sh` stdin JSON 파싱 전환 (실결함 수복)** — 종전엔 `CLAUDE_FILE_PATH` env/`$1` 인자만 읽었으나 hooks.json 배선은 인자 없이 호출 → 경로가 항상 비어 **배선돼 있어도 조용히 무발동**이었음(전체 점검의 스모크 "무출력 exit 0"도 실은 이것). 표준 훅 프로토콜(stdin JSON `.tool_input.file_path`, jq 폴백 grep)로 전환 — 타 훅(prj-code-naming-check 등)과 동일 패턴. env/인자는 수동 테스트 하위 호환 폴백으로 유지. 검증 4케이스(JSON 옛 경로 경고·정상 무경고·시스템 경로 무경고·인자 하위 호환) 통과.
- **감지 패턴에 `projects/`(밑줄 없음) 추가** — structure-awareness.sh + dz-residual-audit OLD_PATTERNS. 두 감지기 모두 `_projects/`만 있고 `projects/`가 없어 v1.11.1이 교정한 잔존이 R11 광역 grep에 안 걸리고 장기 생존했던 **두 번째 구조적 원인** 해소.
- **Claude 시스템 경로 오탐 제외** — `.claude/projects/`(메모리 등)는 옛 경로가 아니므로 양 감지기에서 명시 제외(case 선순위 branch + 매칭 로직 continue).

## v1.11.1 (2026-07-03) — 전체 점검 후속: 옛 경로 잔존·frontmatter 위생 일괄 교정 (패치)

> 2026-07-03 플러그인 전체 점검(forge `참고자료/리포트/2026-07-03-플러그인-전체점검.md` — 설치·훅 실발동·자산 전건 기계검증·의미 정합 워크플로우·MCP 실측)에서 확정된 결함 4계열을 일괄 교정. 뿌리는 워크스페이스 폴더 개편(Step 2~10, `projects/`→`프로젝트/`·`modules/`→`Amaranth10/`)이 플러그인 참조에 전파되지 않은 것. 변경 33파일·89라인(순수 치환 — 삽입/삭제 대칭), 의도외 변경 0 검증(diff 전건 리뷰).

### A. `projects/` → `프로젝트/` 옛 경로 잔존 교정 — 24파일
- commands 11종(dz-add-project·dz-confirm·dz-decision·dz-dispatch·dz-end-session·dz-init-project·dz-jira·dz-projects·dz-reject·dz-start-session·dz-update-project) + skills 11종(dz-context-analyzer·dz-context-manager·dz-decision-tracker·dz-git-daily/SCORING-ENGINE·dz-jira-query·dz-maintenance-triage(+triage-review 템플릿)·dz-project-bootstrap·dz-project-tracker·dz-session-protocol·dz-status-reporter) + rules 2종(markdown-link-policy·prj-code-naming).
- 실기능 결함 해소 포함: **생산자·소비자 엇갈림**(dz-git-daily는 `프로젝트/_mapping-queue.md`에 기록하는데 dz-confirm·dz-reject는 `projects/`에서 읽던 것) 정합.
- 변형 개별 처리: `_projects/`(dz-project-tracker) → `프로젝트/` · triage-review 관련 프로젝트 링크 깊이 `../../` → `../../../`(Amaranth10/{모듈}/유지보수/ 3계층 실측).
- **보존(오탐 — 치환 금지 확인)**: `memory-scope.md`·`memory-rule-content-block.sh`의 `.claude/projects/`(Claude 시스템 경로) · `dz-residual-audit`(옛 패턴 탐지기 본연) · `dz-module-devenv`(Eclipse `.projects/`·webpack publicPath).

### A′. `modules/` → `Amaranth10/` 등 동계열 옛 경로 교정 — 8파일
- 예시 문구: dz-add-task·dz-load-history·dz-load-context의 `modules/법무관리(LTE)/` → `Amaranth10/법무관리(LTE)/`.
- dz-jira `--module` 허용값 열거의 `modules/` 접두 잔재 제거.
- p1-research 사산(死産) 링크 5건(dz-jira L71 + dz-jira-query L75·81·215~217): `modules/업무관리(KISS)/p1-research/` → 실존 확인된 `프로젝트/PRJ-2026-012_업무관리-프로젝트관리-기능확장/05_산출물/p1-research/jira-integration/`(대상 3파일 실존 검증).
- PRJ-2026-012 링크 대상 폴더형 전환 반영: `..._기능확장.md` → `..._기능확장/00_overview.md` + dz-jira 표시 텍스트 PRJ 코드 단독 표기 → 프로젝트명 병기 교정.
- markdown-link-policy 상대 경로 표: `modules/` 행 2건을 `Amaranth10/` 계층 반영 깊이로 재계산(`../../`→`../../../` 등). prj-code-naming의 `reference/` → `참고자료/`.

### B. 부재 템플릿 참조 교정 — 5건 (인라인·동봉 참조로 전환)
- `templates/{tasks,history,context}/`는 원본·배포본·워크스페이스 어디에도 없는 참조였음(build.sh는 templates/를 패키징하지 않고, 평면 템플릿 8종은 경로형 런타임 참조 0건 실측 — 관리자용 비배포 자산으로 확정).
- dz-add-task → dz-task-manager 「enhancements/{날짜}-{제목}.md」 절(절명 실측 정합) · dz-load-tasks → dz-task-manager 파일 형식 절 · dz-load-history → dz-history-manager 파일 형식 절 · dz-context-analyzer → 동봉 `references/output-format.md` · dz-context-manager → 동봉 `references/setup-guide.md` (대체 참조 5건 전부 실존 검증).

### C. `rules/no-v2-rebump.md` frontmatter YAML 파싱 실패 교정
- `description: {이름} ...`이 `{`로 시작해 YAML flow-mapping으로 오인 → 파싱 실패(룰 메타 통째 미로드 위험). 값 전체를 작은따옴표로 감싸 해소 — v1.9.1 스킬 3종 교정과 동일 계열의 잔존분.

### D. 스킬 frontmatter `name` ↔ 폴더명 불일치 4종 통일
- dz-agent-dispatch·dz-deliverable-management·dz-figma-make-reviewer·dz-lessons-learned — `name:`을 폴더명(dz-*)으로 통일(v0.4.0 dz-project 통합 때 잔재).

### 검증 (배포 전 전건)
- YAML frontmatter 129건(스킬 68·커맨드 48·룰 13) 재파싱 실패 0 · name 불일치 0 · 옛 경로 잔존 0(보존 4파일 제외) · diff 전건 리뷰 의도외 변경 0.

## v1.11.0 (2026-07-02) — 회의록 원피스 템플릿 표준(네이비 브리프형) 전사화 + writer 재편집 저장 함정 보강

> 배경: 기존 회의록 원피스가 좌측 정렬 20px 제목·본문 표와 동일한 메타 표 등으로 시각 인지성이 부족하다는 차민수 수석 지적 → 3안(클래식 공문형·네이비 브리프형·미니멀 그리드형) 비교 제안 후 **B안 네이비 브리프형 확정**(2026-07-02). 첫 적용 = 260702_법무실미팅(1차)_회의록. 이를 전 임직원이 재현할 수 있도록 표준·골격·커맨드 진입점을 배선.

### 변경 — `commands/dz-oneffice-write.md` (회의록 프리셋)
- **`--template minutes` 옵션 신설 + 회의록이면 옵션 없이도 자동 적용** — 「회의록 프리셋」 절 신설: forge 표준(`규칙/프로세스/회의록-원피스-템플릿-표준.md`)·골격 정본(`규칙/프로세스/템플릿/회의록-원피스-골격-네이비브리프.html`)을 읽어 `{중괄호}`만 교체(새 스타일 창작 금지), A4·단일페이지·좁은여백·모드 C 고정, 결정·액션 상태 칩 필수, 공유 가능 사실만 수록(내부 분석은 forge md), 문서명 규칙, 참석자 소속은 a10 라이브 조직도 확인. 인식 패턴에 "회의록을 원피스로" 추가.
- 골격·표준의 SSoT(단일 출처)는 forge 워크스페이스(GitLab 동기화 전파) — 플러그인에 사본을 두지 않음(이중 복제 금지). 비 forge 환경이면 사용자 고지 후 일반 절차.

### 변경 — `skills/dz-oneffice-writer` v0.4.0 → v0.5.0 (재편집 저장 함정 실측 수록)
- **치명적 함정 4 신설**: 저장된 문서 재열람 세션에서 JS `.click()`·Cmd+S 저장이 조용히 무시됨(요청 0건) — 재편집 저장은 **실제 마우스 클릭**(`computer` left_click, 리본 💾)만 유효. 저장 발생 판정 = 탭 제목 "데이터를 저장하고 있습니다." 전환.
- Step 10(b) 경고 블록 + (c) 재편집 검증 절차(새로고침 후 DOM — a10 MCP 조회 API 캐시 구버전 오판 주의) + 디버깅 체크리스트 항목 추가. 근거 실측: forge `참고자료/리포트/2026-07-02-원피스-재편집-저장-함정-진단.md`.

### 동반 조치 (워크스페이스 douzone-forge — GitLab 동기화 전파)
- `규칙/프로세스/회의록-원피스-템플릿-표준.md` v1.0 신설(구조 7블록·상태 칩 6종·색 토큰·발행 절차) + `규칙/프로세스/템플릿/회의록-원피스-골격-네이비브리프.html` 골격 정본 신설.
- forge `CLAUDE.md` 스킬 사용 규칙 표에 「회의록 원피스 발행」 진입점 행 추가 — 임직원 자비스가 회의록 요청 시 표준을 자동 인지.

## v1.10.0 (2026-07-02) — dz-personal-init ⛔ 인증 게이트 (온보딩 MCP 인증 건너뜀 해소)

> 실제 임직원 온보딩 실측에서 `/dz-personal-init`이 **MCP 등록까지는 되는데 자격증명 인증(jira 입력·a10 크롬 시드)을 건너뛰어 진행이 멈추는 사례가 다발**. 원인 2가지를 확정: ① 문서에 인증 완수를 강제하는 지시가 없어 자비스가 생략(크롬 확장 미연결이면 조용히 통과) ② a10 쿠키 시드 헬퍼 `.a10-mcp/scripts/seed_from_cookie.py`가 2026-06-16 빌드본 전용 전환(forge `b445ddf`) 때 MCP 소스와 함께 **오삭제**되어 §8.5가 깨진 경로를 참조(a10 인증 실패의 근본 원인).

### 변경 — `commands/dz-personal-init.md` v0.7.0 → v0.10.0 (§8 내부 라벨 v0.9.0까지 선점된 frontmatter 불일치 정정 포함)
- **⛔ 인증 게이트 신설 (§8 서두)** — §8.0~8.6 순차 실행 의무. 사용자가 명시적으로 "보류"하지 않는 한 인증 확인 전 완료 선언 금지. 전제 미충족(크롬 확장 미연결·A10 미로그인·uv 미설치)이면 **말없이 건너뛰지 말고 그 자리에서 해결 절차 안내 + 해결 후 재시도**. 완수 판정 = §9 자가 진단 ✅.
- **§8.3 jira 인증 — 자비스 주도 3단계**: 편집기(텍스트 편집기 — 메모장) 자동 열기 + "JIRA_USER·JIRA_PASSWORD 두 칸 채우고 저장(Cmd+S), 마치면 알려달라" 안내 그대로 출력 → 완료 응답 **대기**(임의 진행 금지) → 채움 확인(grep, **값 미열람**) → 미채움 시 재안내 루프.
- **§8.5 a10 인증 — 자비스 주도 5단계** (사용자에게 명령 복붙 요구 금지):
  - (1) 크롬 확장 연결 판정(`mcp__claude-in-chrome__*` 존재?) — **케이스 A**(미설치·미연결): 웹스토어 Claude 확장 설치 → 확장 로그인 → "연결되면 'a10 인증 다시 해줘'라고 말씀해 주세요 — 이어서 진행해 드립니다" 안내 후 보류(§9 ⬜ 기록). **케이스 B**(설치·로그인까지 했는데도 미연결 — 실측 다발): **클로드 데스크탑 앱 완전 종료(⌘Q) 후 재실행** → 새 세션에서 "a10 인증 다시 해줘" 안내.
  - (2) 자비스가 크롬 MCP로 직접 `gwa.douzone.com` 열어 로그인 판정 — 미로그인이면 사용자 로그인 요청·대기(비밀번호는 사람 몫).
  - (3) 쿠키 시드 실행(복원된 헬퍼 경로) — macOS "Chrome Safe Storage" 키체인 허용 안내 포함.
  - (4) 시드 검증(A10_OAUTH_TOKEN·A10_SIGN_KEY 채움만 grep — 값 미열람).
  - (5) 완료 안내 + 토큰 만료 시 "a10 인증 다시 해줘" 재시드 안내.
- **§9 자가 진단 보강** — 크롬 확장 연결 체크 항목(케이스 A/B 재시도 방법 병기) 추가 + "⬜가 하나라도 있으면 해당 § 재시도 안내까지 출력 — 말없이 종료 금지".
- 검증 스니펫 자기완결 정정 — §8.3-3·§8.5-(4) bash에 `WS=` 재정의 추가(각 bash 호출은 새 셸이라 변수 미승계 — 선재 잠복 결함).

### 동반 조치 (워크스페이스 douzone-forge — GitLab 동기화 전파)
- **`.a10-mcp/scripts/seed_from_cookie.py` 복원** — forge git 이력 `d8a66e5`에서 byte 동일 복원 + 구문 컴파일 검증. MCP 서버 소스가 아닌 온보딩 헬퍼이므로 빌드본 전용 정책의 예외로 `.a10-mcp/README.md` 구성 목록에 명시.
- forge `CLAUDE.md` 플러그인 버전 절·`README.md` 온보딩 3단계 문구 정합.

### 동승 수록 (사전 미커밋분 정리 — 배포 시점 실측 발견)
- **`skills/dz-api-ref` v0.1.0 정식 수록** — API 정의 참조·정합 검증 스킬(위키 API 정의 + 소스 3중 대조, SSoT = forge `규칙/프로세스/API참조-체계-표준.md`). 2026-06-30 표준 신설 때 개발됐으나 플러그인 레포 커밋·배포가 누락돼 있던 것을 본 릴리즈에 정식 수록(스킬 68종째).
- **고아 훅 6종 판정 주석** — `hooks/HOOKS.md` §4 신설 + 6종 스크립트 헤더에 배선 상태(의도적 보류/구조적 비배선 — 등록 누락 결함 아님) 명기 (2026-07-02 판정, 사실 기록. 상세: forge `참고자료/리포트/2026-07-02-플러그인-위생이슈-3건-진단.md`).

## v1.9.1 (2026-06-29) — 훅 정의 정식 형식 이관(hooks/hooks.json) + 스킬 프론트매터 교정

> v1.9.0의 매 턴 4원칙 주입(force-rules-inject, UserPromptSubmit 배선)이 CoWork 앱 세션에서 발동하지 않던 문제를 진단. 원인은 **앱이 마켓플레이스판 대신 inline 사본을 물고 있던 것**(CLI는 마켓플레이스 1.9.0·훅 6종 정상 등록, 앱은 inline 사용 — 사용량 실시간 증가로 확정). 코드 형식 자체는 무죄였으나, 가장 견고하고 공식 권장인 정식 위치로 이관해 모호성을 제거. CoWork inline 잔재 제거는 앱 플러그인 패널에서 별도 조치.

### 변경
- **훅 정의 위치 이관**: `plugin.json`의 인라인 `"hooks"` 객체 → 정식 위치 **`hooks/hooks.json`**(공식 플러그인 표준·자동 발견). 이벤트 6종(PreToolUse·PostToolUse·SessionStart·SessionEnd·UserPromptSubmit·Stop)·matcher·명령 문자열(인자 `pull`/`sync` 포함) **전부 동일**하게 이동. plugin.json의 `hooks` 키는 제거 — 중복 발동 방지(둘 다 두면 병합·이중 발화).
- `claude plugin validate` 통과 확인(hooks/hooks.json 형식 정상 수용).

### 정리 (선재 결함 교정)
- **스킬 프론트매터 YAML 파싱 오류 3건 교정** — `dz-status-show`·`dz-mcp-inventory`·`dz-user-request`. `description` 값이 큰따옴표로 시작(`"현황 보여줘"…`)해 YAML이 인용 스칼라로 오인·파싱 실패 → 런타임에 메타데이터가 통째 비어 로드되던 문제. 값 전체를 작은따옴표로 감싸 해결. 이 교정 전까지 `claude plugin validate` 실패 상태였음.

## v1.9.0 (2026-06-28) — 강제 작업 수행 원칙 매 턴 주입 훅 전사화 (force-rules-inject)

> 강제원칙 4대(작업수행-강제원칙.md v1.2.0 — 전건순차·검토 분위기 신설)의 **매 턴 주입**을 전 임직원으로 전파. 그간 매 턴 주입은 사용자 로컬 `.claude/`(gitignore)에만 배선돼 전사 미전파였음(CLAUDE.md 매 세션 로딩으로는 도달하나 턴당 강화는 로컬 한정). 메커니즘=플러그인, 내용=동기화 규칙 파일로 분리해 해소.

### 신규
- **`hooks/force-rules-inject.sh`** (UserPromptSubmit 배선) — 매 입력마다 `규칙/프로세스/강제규칙-주입.md`(forge 동기화 SSoT)를 출력. 파일 없으면 `.claude/` 폴백, 항상 exit 0(프롬프트 미차단).
- 내용 SSoT = forge `규칙/프로세스/강제규칙-주입.md`(동기화) → 원칙 내용 갱신은 **GitLab 동기화만으로 전파**(플러그인 재배포 불필요). 압축본 전환도 그 파일 1곳 수정.
- 설계 원리: 메커니즘(훅)=플러그인 설치/업데이트로 전 직원 자동 배선 / 내용=동기화. 측정: 주입 ~700토큰/턴(입력), 훅 실행 밀리초(지연 무시). 사용자(기보유)엔 순증 0, 미보유 직원만 동일 비용 획득.

## v1.8.1 (2026-06-23) — 플러그인 자체 개발표준 신설 + 일관성·위생 교정(스트림 4)

> 플러그인 자산 6 유형(skills·commands·hooks·rules·agents·meta) 일관성 감사(워크플로 131 자산 실측) → 자체 개발표준 신설 + 위험 0/저위험 부채 교정. 런타임 동작·식별 변경 위험 항목은 백로그(표준 부록 A).

### 신규
- **플러그인 자체 개발표준** (forge `규칙/프로세스/플러그인-자체-개발표준.md` v1.0.0, 15섹션) — 자산 유형별 저작 규약·버전/빌드/배포·검증·자가 점검. 감사 리포트 동반.
- **`hooks/HOOKS.md`** — 21 훅 분류표(배선 14·고아 7, 실제 차단 1=db-guard, 게이트 3, set -e 12/21).

### 변경·정리 (Phase C 위험 0 + 저위험)
- rules SSoT 끊긴 상대링크 5건(master-vocab·memory-scope·no-v2-rebump·single-approval-policy·prj-code-naming) → 비링크 코드텍스트 `(forge)` 정규화.
- frontmatter 보완: `commands/dz-form-write.md` name 추가 · skills version 3건(dz-deck-builder·dz-external-report·dz-oneffice-form-writer) `0.1.0` 추가.
- **에이전트 수량 표기 14/15 → 16 통일**(dz-agent-dispatch: BLUEPRINT·ATLAS 행 추가, SSoT 정합).
- 위생: `.bak.*` 9건·정체불명 zip 잔재 2건·소스루트 잔재 marketplace.json(v0.16.0) 제거 + `.gitignore`에 `*.bak.*`.

### 백로그(미처리 — 런타임/식별 위험, 표준 부록 A)
- hook `set -euo pipefail` 9건 · 차단형 hook opt-out 2건 · skill name-디렉토리 불일치 4건 · rules paths 3건 · hook 메타 헤더 형식 점진 통일.

## v1.8.0 (2026-06-23) — ⑤ 검증·확정 자동화 + ⑥ 개발자 하네스(비강제) H3·H4·H5

> 개발 표준화 스트림 2 파이프라인의 ⑤ 산출물 검증·확정 자동화와 ⑥ 개발 구현(코딩) 단계 개발자 하네스를 합산 배포. 모두 **비강제(opt-in/opt-out)** — 기존 개발환경 불변, 차단은 db-guard 하나(+opt-out). 전략 전제 정정 동반: 구조 무결성 강제는 현재 서버 게이트(CI·머지)가 거의 비어 있어(실측), 본체(③ D-06·④ D-08·⑤ AEGIS)+하네스 보조로 추측 제거에 집중(서버 게이트 신설은 관리자 협조 별도 사이클).

### 신규 — ⑤ 산출물 검증·확정 자동화
- **dz-verify-design 게이트 스킬 신설** — AEGIS 독립 검증으로 개발 계획(D-06)·개발설계(D-08) 산출물의 추적 키 grep 조인·품질 게이트·제안→확정 판정. 시연: 법무관리(LTE) 개발계획 '확정 가능'(게이트 16/16·재계산 일치).
- **dz-aegis-agent** D-06·D-08 검증 프로토콜 확장. (forge 표준 연계: D-06 §9·D-08 §9·CLAUDE.md cross-ref)

### 신규·변경 — ⑥ 개발자 하네스(비강제)
- **H3 비강제 게이트**: `hooks/_forge-gate.sh` 신설 — `_forge` 심링크 상향 탐색 + `FORGE_DEV_HOOKS` 환경변수로 코드 리마인더 게이트 on/off. 리마인더 3종(code-quality·security·build-verify)을 게이트 적용(브리지 켠 폴더/​env에서만 동작, 아니면 무음). `db-migration-guard` 는 안전 가드라 기본 차단 유지 + `FORGE_DB_GUARD=off` opt-out 경로.
- **H4 에이전트 경계 opt-in + 모듈 파라미터화**: dev 에이전트 3종(CORE·NOVA·SYNAPSE)에 위임 경계(브리지 켠 개발 세션에서만 위임). dz-nova 에 A10 FE 번들러 제약(klago-ui-micro·node-sass→dart-sass·Node16·setupProxy). `rules/enum-consistency`·`coding-standards` 의 forge-PM 가정·틀린 스택 하드코딩을 **SSoT 위임**(coding-rules INDEX·개발환경 표준). `commands/dz-verify-step`·`dz-dev-instructions` 를 A10 레포 구조(루트 gradlew·번들러·PRJ+D-06/D-08)로 파라미터화.
- **dz-module-devenv §8.5** 브리지 채택 모델 opt-out 명문화(기본 설치 권장 + 의도 1회 확인 + 언제든 `--remove`).
- (forge 측 연계 — 본 배포와 함께 forge GitLab 반영: H1 브리지 dev 자산 노출·H2 coding-rules INDEX 자동 로드·**H5 `setup-forge-bridge.sh --status`** — 브리지·게이트·db-guard 상태 + 템플릿 stale `diff -q` 감지)

### 함의
- 강제 규칙은 브리지/하네스가 아니라 게이트(CI·머지)가 담당하는 별도 층 — 하네스는 보조(우회 가능·opt-out). 미배포 부채: 커밋훅 배선·A10 비밀 스캔 가드(§6 WHITELIST 정책 선결)·Flyway↔dbPatch 정합(보안/정합 백로그).

## v1.7.0 (2026-06-22) — 개발 계획 수립(D-06) 표준·에이전트·스킬·템플릿 신설

### 신규 — 개발 표준화 스트림 2 ③ 개발 계획 수립 (D-06)
- **개발 계획 수립 표준 D-06** v1.2.0 신설 (forge `규칙/프로세스/개발-계획-수립-표준.md`): 개발 관점 분석 → 개발 Task 분해·의존성 → 공수 측정(Bottom-up + 3점/PERT) → 일정·마일스톤(CPM) → 이후 개발 산출물. **협업 기본**(여러 개발자 동시 투입·Cell리더 참여) · **공수(인일)≠일정(달력)** 자원 배치 · **개발자 가용성**(전 프로젝트 task 부하 기준) · **WBS 제안→확정**(실무 개발담당자/리더 의견 수렴). 부록 B **자동화 입력 스키마·산정 규칙**(PERT·CPM·보정계수, 기계 판독).
- **ATLAS 에이전트(#16)** 신설 — 개발 계획 수립 전담 서브에이전트(`.claude/agents/dz-atlas.md` + `skills/dz-atlas-agent`). 에이전트 레지스트리 15→16.
- **dz-devplan-write 스킬** 신설 — ATLAS(작성)→AEGIS(검증), 공수·일정 부록 B 공식 자동 산정.
- **개발계획서 템플릿** 신설 (`templates/개발계획서.md`, D-03 패밀리) — 자동화 입력 YAML 스키마 포함.
- cross-ref 정합: 도메인 프로세스(⑥ 개발 진입)·D-07·D-08(선행 입력)·CLAUDE.md 스킬표·PRJ-2026-014. 실증: 법무관리(LTE) 사건비용 개발계획서(제안 — 2명 5.25일/1명 6.42일, 산정 검산 일치).

## v1.6.2 (2026-06-21) — 개발설계 다이어그램 PlantUML 기본 전환

### 변경 — dz-devdesign-write ⑤: 다이어그램 도구 PlantUML 우선
- 호출관계=**시퀀스**, 처리 흐름=**액티비티**를 **PlantUML 텍스트 매크로**로 생성(서버 렌더·뷰 즉시 표시·자동 생성 용이·첨부/압축/revision/PNG 불요). `wiki_create_page`/`wiki_update_page` 본문에 plantuml 매크로 텍스트만 포함(별도 업로드 도구 불요).
- draw.io는 사람 자유 편집 시 선택(압축 mxfile + 매크로 revision 일치 + PNG 1회 저장 — 함정 D-08 §B.3).
- 근거: 218127304(PlantUML 시퀀스=호출관계) 실측 + LTEWI019 360612209 PlantUML 2종 뷰 렌더 실증. 연계 D-08 v1.0.8(부록 B PlantUML 기본).
...
(`_참조자료/프로세스/20260420-claudecode-하네스4계층-심층조사.md` _(옛 경로 — 현재 없음)_)
...
### 상세 계획
- `_참조자료/프로세스/20260420-하네스-3계층-적용계획.md` _(옛 경로 — 현재 없음)_
- `_참조자료/프로세스/20260420-claudecode-하네스4계층-심층조사.md` _(옛 경로 — 현재 없음)_
- `_projects/PRJ-2026-014_Amaranth10-Claude-Forge-구축.md` _(옛 경로 — 현재 없음)_
