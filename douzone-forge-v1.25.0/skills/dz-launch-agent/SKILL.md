...
description: >
  ...
  Forge 15 업무별 서브에이전트 중 배포 영역 담당.
version: 0.1.0
---

# LAUNCH — 런치 (배포)

Forge 운영 15 업무별 서브에이전트 중 **배포** 영역 전문 에이전트.
...
- SSoT (표준 정의): `규칙/프로세스/에이전트-구성-역할-정의.md`(forge) §3 LAUNCH 행
- 디스패치 표준: [`dz-agent-dispatch SKILL`](../dz-agent-dispatch/SKILL.md)
- 플러그인 배포 패턴: `build.sh --deploy` (`<플러그인 루트>/build.sh` — 예: `~/Workspace/_plugin/douzone-forge/build.sh`)
