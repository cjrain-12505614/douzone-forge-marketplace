...
4. 폴더 구조 생성:
   ...
   > ⛔ **세션은 모듈 안에 두지 않는다** — `_개인/sessions/{대상}/` 이다(폴더 구조 표준 룰 5 「모듈 외부 배치」). 종전 트리에 있던 `sessions/`·`sessions/archive/` 는 2026-04-25 개인 영역으로 이전 완료된 폐지 구조라 제거했다. <!-- auto: 자비스 2026-08-13 -->
...
5. 초기 파일 생성:
   ...
   - `_개인/sessions/{대상}/_current.md` — 초기 세션

6. 온보딩 안내:
   - 다음 단계로 설계서 PDF 분석(`dz-context-analyzer`) 또는 스크린샷 분석을 권장

### 새 프로젝트 등록

3. `프로젝트/_dashboard.md`에서 최신 PRJ ID를 확인하고 다음 순번을 채번한다.
   - 단일 모듈 귀속: `PRJ-{YYYY}-{모듈코드}-{NNN}` (신규 규칙)
   - 수명업무·크로스모듈: `PRJ-{YYYY}-{NNN}` (구규칙)
4. `프로젝트/_templates/project-detail.md`를 복사하여 규칙에 맞는 파일명으로 생성한다 (`PRJ-{YYYY}-{모듈코드}-{NNN}_{프로젝트명}.md` 또는 `PRJ-YYYY-NNN_{프로젝트명}.md`).
5. 사용자에게 필수 정보를 확인한다:
