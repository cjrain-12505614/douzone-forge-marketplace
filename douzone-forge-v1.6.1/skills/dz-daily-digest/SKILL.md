...
- 경로 인덱스: `douzone-forge/참고자료/운영사이트/일일보고-링크-매트릭스.md` (조직별 링크 테이블)
- 읽기 방식: `dz-oneffice-reader` 스킬 활용 (`innerText` 추출)
...
└── deliverables/
    └── 보고서/
        └── 일일보고/
            ├── YYYYMMDD-SBUnit-일일보고-분석.md  ← PM 분석({이름} 관점, 기존 컨벤션)
            └── 원본캐시/
...
> **참고**: 모듈은 `Amaranth10/{모듈}/`, SBUnit 운영 산출물은 `deliverables/`·`참고자료/`·`프로젝트/` 폴더를 사용한다 (Step 1~11 재편 반영).
...
### STEP 2: 아마링크 목록 해석
`참고자료/운영사이트/일일보고-링크-매트릭스.md` 에서 대상 사원·날짜의 문서 URL을 구한다.
문서명 패턴 미확정 시 사용자에게 확인 요청.

### STEP 3: ONEFFICE 본문 추출
각 문서에 대해 `dz-oneffice-reader` 절차:
- Chrome MCP navigate
...
- 캐시: douzone-forge/deliverables/보고서/일일보고/원본캐시/2026-04/ (18 files)
```
...
grep -r "CSA10-44921" douzone-forge/deliverables/보고서/일일보고/원본캐시/

# 특정 사원의 최근 7일 보고
ls douzone-forge/deliverables/보고서/일일보고/원본캐시/2026-04/*-2505614.md
```

`dz-triage-status` 커맨드가 이 캐시를 기반으로 3-way 매트릭스를 구축한다.
...
- `dz-oneffice-reader` — 본문 추출 위임
- `dz-triage-status` — 3-way 크로스 체크에서 이 캐시를 소비
- `dz-git-daily` — JIRA 키·PRJ 참조 교차 매칭
