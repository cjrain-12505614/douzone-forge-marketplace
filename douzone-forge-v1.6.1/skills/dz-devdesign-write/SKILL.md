...
⑤ 발행 (선택) — 위치·양식은 **모듈별 실측**(D-08 §부록 B). 모듈마다 다름.
   ...
   - **양식**: 테이블=Description+Table Columns(컬럼명·Data type·Not Null·Default·Comment, PK 노랑) · API=기본정보+파라미터+요청/응답 code · 프로세스=흐름 요약+draw.io.
   - **발행 도구**: JIRA-MCP `wiki_create_page`(공간·제목·본문 storage)/`wiki_update_page` + **워크플로우 draw.io = `wiki_upload_attachment`**(평문 mxGraph, content_type=application/drawio, filename=diagramName) 후 본문에 drawio 매크로(B5 v0.5.0). (+ 필요 시 원피스·PDF)
```
