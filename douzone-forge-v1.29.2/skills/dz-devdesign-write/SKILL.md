...
>
> **SSoT(단일 출처)**: `규칙/프로세스/개발설계-산출물-작성-표준.md`(forge)(D-08). 본 스킬은 그 표준을 **실행**하는 절차다. 작성 주체 = BLUEPRINT(`.claude/agents/dz-blueprint.md`), 검증 = AEGIS.
...
4. **세션 체크** — `_개인/sessions/{대상}/_current.md` 진행 중 건이 있으면 이어받기.
...
⑤ 발행 (선택) — 위치·양식은 **모듈별 실측**(D-08 §부록 B). 모듈마다 다름.
   - **위치**: 루트 "4. Amaranth 10 개발"(40207049) → 모듈 → "1. 개발" 하위 테이블/API/프로세스 영역. 법무: 45(245468987)/1.개발/테이블 245469001·API 245469007·프로세스 245469012. 다른 모듈은 `wiki_get_child_pages`로 트리 실측 후 결정.
   - **양식**: 테이블=Description+Table Columns(컬럼명·Data type·Not Null·Default·Comment, PK 노랑) · API=기본정보+파라미터+요청/응답 code · 프로세스=흐름 요약+**다이어그램**.
   - **다이어그램 = PlantUML 기본**(텍스트, 서버 렌더): **호출관계 → 시퀀스**(participant/`->`/note/alt) · **처리 흐름 → 액티비티**(start/`:활동;`/if-else/stop). `plantuml` 매크로 본문(CDATA)에 텍스트만 넣으면 됨(@startuml 불요·첨부/PNG 불요·뷰 즉시 렌더). draw.io는 사람 자유 편집 시 선택(압축 mxfile+매크로 revision 일치, PNG 1회 저장 — D-08 §B.3 함정).
   - **발행 도구**: JIRA-MCP `wiki_create_page`/`wiki_update_page`(본문 storage에 plantuml 매크로 텍스트 포함 — PlantUML은 별도 업로드 도구 불요). draw.io 선택 시 `wiki_upload_attachment`. (+ 필요 시 원피스·PDF)
```
...
- 표준(SSoT, D-08): `규칙/프로세스/개발설계-산출물-작성-표준.md`(forge) (부록 A LTEWI019 앵커)
