...
# ⛔ git 설정 격리 (v1.29.2) — 시험 저장소는 이 PC 의 시스템·전역 git 설정을 읽지 않는다.
#    ① 계측 누수: 전역 core.hooksPath 가 Litmus(사내 AI 계측) 훅이면 시험 저장소마다 훅이 붙어
#       **시험 커밋이 중앙 KPI 로 전송된다**(2026-09-11 실측: 맥미니B 70건 · A2 22건).
#    ② 기기 간 결과 차이: Apple Git 은 시스템 설정에 init.defaultBranch=main 이 있고 Homebrew git 은
#       없다(→ master). 빈 원격을 main 으로 가정하는 시험 4건이 Homebrew git 기기에서만 실패했다.
#    그래서 시스템 설정은 끄고, 전역은 기본 분기 한 줄만 둔 임시 파일로 고정한다.
#    진단: 워크스페이스 참고자료/리포트/2026-09-11-맥미니B-rebase중단-미푸시0오표시-진단.md §8
GIT_ISO_CFG="$TMP/isolated.gitconfig"
printf '[init]\n\tdefaultBranch = main\n' > "$GIT_ISO_CFG"
export GIT_CONFIG_NOSYSTEM=1
export GIT_CONFIG_GLOBAL="$GIT_ISO_CFG"
...
if [ -z "$(git -C "$ISO_PROBE" config --get core.hooksPath 2>/dev/null)" ]; then
  echo "  PASS  격리(전역 훅 경로 없음 — Litmus 등 전역 훅이 시험 저장소에 붙지 않음)"; PASS=$((PASS+1))
else
  echo "  FAIL  격리(전역 훅 경로가 보임: $(git -C "$ISO_PROBE" config --get core.hooksPath) — 시험 커밋이 계측으로 샐 수 있음)"; FAIL=$((FAIL+1))
fi
if [ "$(cat "$ISO_PROBE/HEAD" 2>/dev/null)" = "ref: refs/heads/main" ]; then
  echo "  PASS  격리(새 저장소 기본 분기 main)"; PASS=$((PASS+1))
else
...
SD="$TMP/stalldetail"; mkdir -p "$SD"
sd_mk() {   # $1=시나리오 → 원격이 한 칸 앞선 클론
  ( set -e; rm -rf "$SD/$1"; mkdir -p "$SD/$1"; cd "$SD/$1"
    git init -q --bare douzone-forge.git
    git clone -q "$SD/$1/douzone-forge.git" work 2>/dev/null; cd work
    git config user.name '[UC]시험'; git config user.email t@t
    echo a > f.txt; git add -A; git commit -qm base; git branch -M main; git push -q -u origin main
    cd ..; git clone -q douzone-forge.git other 2>/dev/null; cd other
    git config user.name o; git config user.email o@o
    echo r > r.txt; git add -A; git commit -qm remote; git push -q origin main ) >/dev/null 2>&1
}
sd_run() { env CLAUDE_PROJECT_DIR="$SD/$1/work" DZ_SYNC_AUTOHEAL=0 bash "$HERE/dz-gitlab-sync.sh" sync 2>&1 | grep '매달린 것' || true; }

# ⓐ 현장 모양 — 리베이스가 첫 명령 앞에서 멈춤: HEAD == onto, 내 커밋 2건은 orig-head 에만
sd_mk a
( cd "$SD/a/work"; echo 1 >> f.txt; git commit -qam l1; echo 2 >> f.txt; git commit -qam l2; git fetch -q
  GIT_SEQUENCE_EDITOR="sed -i '' '1i\\
break
'" git rebase -i origin/main ) >/dev/null 2>&1
if [ "$(git -C "$SD/a/work" rev-parse HEAD 2>/dev/null)" = "$(cat "$SD/a/work/.git/rebase-merge/onto" 2>/dev/null)" ]; then
  echo "  PASS  원격미반영(전제: HEAD==onto 로 멈춤)"; PASS=$((PASS+1))
else echo "  FAIL  원격미반영(전제 조성 실패 — 아래를 믿지 말 것)"; FAIL=$((FAIL+1)); fi
O="$(sd_run a)"
check "원격미반영(현장 모양 2건)" "원격에 아직 없는 내 커밋 2건" "$O"

# ⓑ 대조군 — 리베이스 없는 분리된 HEAD, 내 커밋이 HEAD 에만 1건 (종전에도 맞던 경우 · 무회귀)
sd_mk b
( cd "$SD/b/work"; git fetch -q; git checkout -q --detach origin/main; echo x > x.txt; git add -A; git commit -qm dh1 ) >/dev/null 2>&1
O="$(sd_run b)"
check "원격미반영(분리HEAD·HEAD 쪽 1건)" "원격에 아직 없는 내 커밋 1건" "$O"

# ⓒ 리베이스 없는 분리된 HEAD, 내 커밋 2건은 브랜치 끝에만 (HEAD 는 원격 위) — 같은 계열 사각
sd_mk c
( cd "$SD/c/work"; echo 1 >> f.txt; git commit -qam m1; echo 2 >> f.txt; git commit -qam m2; git fetch -q; git checkout -q --detach origin/main ) >/dev/null 2>&1
O="$(sd_run c)"
check "원격미반영(분리HEAD·브랜치 쪽 2건)" "원격에 아직 없는 내 커밋 2건" "$O"

# ⓓ 충돌로 멈춘 리베이스 — 3건 중 1건은 이미 옮겨짐. 이중 계산하면 4 가 나온다
sd_mk d
( cd "$SD/d/work"; echo x > a.txt; git add -A; git commit -qm d1; echo 1 >> f.txt; git commit -qam d2; echo y > b.txt; git add -A; git commit -qm d3
  cd ../other; git pull -q; echo R >> f.txt; git commit -qam rf; git push -q origin main
  cd ../work; git fetch -q; git rebase origin/main ) >/dev/null 2>&1
O="$(sd_run d)"
check "원격미반영(충돌 정지 3건·이중계산 없음)" "원격에 아직 없는 내 커밋 3건" "$O"
