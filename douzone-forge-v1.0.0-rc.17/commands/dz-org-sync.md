---
name: dz-org-sync
description: "xlsx 갱신 후 admin 동기화 — 변경 행만 검출, 영향받는 폴더만 갱신 (자동 archive 안 함, {이름} 재확인 의무)"
version: 0.1.0
---

# /dz-org-sync 커맨드 (Phase Q-2 신설)
...
- **자동 archive 안 함** — 퇴사 후보 발견 시 알림만, 실제 archive는 `/dz-org-archive` 별도 명령 + {이름} 재확인
...
2. 변경 분류:
   - **신규**: 신규 인원 → `dz-org-bootstrap` 호출 권장
...
```
✅ org-sync 완료
  - 신규 후보: N건 (dz-org-bootstrap 권장)
  - 부서 변경: N건 (적용 또는 보류)
  - 메타 변경: N건
  - 퇴사 후보: N건 (dz-org-archive 별도 명령 필수)
```

## 의존 자산

- `skills/dz-people-index/SKILL.md`
- `참고자료/조직정보/*.xlsx`

## 관련 명령

- `/dz-org-bootstrap` — 신규 인원 폴더 신설
- `/dz-org-archive` — 퇴사 인원 처리
