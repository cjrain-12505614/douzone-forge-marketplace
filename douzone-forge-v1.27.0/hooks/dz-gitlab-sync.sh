...
commit_local() {
  ...
  if [ "${DZ_SYNC_ALLOW_MASS_REVERT:-0}" != "1" ]; then
    ...
    _rg_hit=""
    if [ "$_rg_del" -ge 100 ] && [ "$_rg_del" -ge $(( _rg_add * 2 )) ]; then
      _rg_hit="삭제 ${_rg_del}줄이 추가 ${_rg_add}줄의 2배 이상"
    elif [ "$_rg_delf" -ge 1 ] && [ "$_rg_del" -ge 100 ]; then
      _rg_hit="추적 파일 ${_rg_delf}건 삭제 + 삭제 ${_rg_del}줄"
    fi
    if [ -n "$_rg_hit" ]; then
      ...
      # 죽은 pull 의 지문이면 곧바로 짚어 준다 — 작업트리가 원격판을 그대로 베끼고 있는 모양
      if git rev-parse --verify -q "origin/$BRANCH" >/dev/null 2>&1; then
        ...
        if [ "${_rg_ahead:-0}" -gt 0 ] && [ "${_rg_vh:-0}" -gt 0 ] && [ $(( ${_rg_vr:-0} * 3 )) -le "${_rg_vh:-0}" ]; then
          say "   ⛔ 작업트리가 origin/$BRANCH 를 그대로 베끼고 있습니다 (원격과 ${_rg_vr}개 차이 / 내 커밋과 ${_rg_vh}개 차이)."
