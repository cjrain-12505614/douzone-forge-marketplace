...
3. **_개인/sessions/{모듈}/_current.md** — **세션 체크** (진행 중인 작업 확인 필수!)
...
작업이 끝나면 반드시 해당 LNB 파일을 업데이트한다. `/dz-update-context` 커맨드를 사용하면
...
새 모듈에 처음 적용할 때는 `references/setup-guide.md`의 초기화 절차에 따라 골격 파일을 생성해
