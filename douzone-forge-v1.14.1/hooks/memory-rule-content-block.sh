...
# 화이트리스트: "본인·내 ~·본인 R&R·본인 KPI·본인 일일보고" 매칭 시 통과
#
# 배선 상태: 배선됨 (v1.12.0, 2026-07-03 — 차민수 수석 결정, HOOKS.md §1·§4). PreToolUse 등록. 격리 테스트 통과.
#   (2026-07-02까지 미배선 고아 — 정책 결정 대기였음)
#   기원: v1.0.0-rc.2(2026-05-11) V-07-01 학습 강제 메커니즘으로 신설. 당시 CHANGELOG는
#         "차단 강제(H)" 발효로 기록했으나 배선 이력 전무(git -S plugin.json·hooks.json 0건 실측).
#   보류 사유: 차단(exit 2) 훅의 전 임직원 활성화는 정책 결정 사항 — 배선·opt-out 정책 결정 대기
#         (HOOKS.md §2, 2026-06-23). 자비스-운영-룰 학습(관리자 운영 규율)의 전사 강제 여부 미결.
...
if echo "$CONTENT" | grep -E "(본인|내[[:space:]]+(역할|R&R|KPI|일일보고|업무|PRJ|결정|호칭|부서|상사))" > /dev/null 2>&1; then
  # 사람 기준 메모리로 인정 → 통과
  exit 0
fi
...
🚫 [memory-rule-content-block] 메모리에 룰성 본문 감지 — Write 차단
    파일: $FILE_PATH
    학습 #11 — 메모리 한정 (사람 기준만)
    - 메모리 허용: 본인 역할·호칭 / 본인 부서 사람 매핑 / 본인 PRJ 비전·KPI / 본인 일일보고 메타
