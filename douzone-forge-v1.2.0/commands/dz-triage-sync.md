...
`/dz-triage-status` 의 자동 전이 결과를 신뢰할 수 없거나, 오프라인에서 개발된 건을
...
6. `dz-forge-bridge` 호출하여 `.forge/` 동기화

## 일일보고 참조
해당 JIRA 키가 언급된 일일보고 캐시를 `douzone-forge/deliverables/보고서/일일보고/원본캐시/` 에서 역검색.
...
- 진행 로그: 5행 재작성
- 일일보고 마지막 언급: 2026-04-19 ({이름})
- .forge 주입: amaranth10-board, klago-ui-board-micro
```

## 사용 시나리오

- `/dz-triage-status` 가 놓친 커밋 발견
- 개인 브랜치(`dev-{사번}`) 작업이 있어 `--include-personal` 로 재조사 필요
- 검토안 파일을 수동 편집한 뒤 정합성 복구

## 관련

- [`/dz-triage-status`](dz-triage-status.md)
- [`/dz-triage-close`](dz-triage-close.md)
