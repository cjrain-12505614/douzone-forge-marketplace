...
echo "[1/8] link-integrity-check — 깨진 링크 md 에서 경고가 나오는가"
...
echo "[2/8] code-quality-reminder — 게이트 켜짐 + 확장자별 체크리스트가 나오는가"
...
echo "[3/8] security-auto-trigger — 보안 파일명에서 경고가 나오는가"
...
echo "[4/8] build-verify-reminder — 5회째 편집에서 리마인더가 나오는가"
...
echo ""
echo "[5/8] link-integrity-scan — 전수 스캔이 기준선·증가를 잡는가 (2026-08-13 신설)"
...
echo ""
echo "[6/8] dz-gitlab-sync 대량 되돌림 가드 — 자산화를 지우는 커밋을 막는가 (v1.27.0)"
...
echo "[7/8] dz-gitlab-sync 연속 실패 환기 — 3회째에 방치를 알리는가 (v1.28.0)"
...
echo ""
echo "[8/8] dz-gitlab-sync 계측 누수 자가 감지 — 훅 대행·놓친 커밋·고착 안내 (v1.28.1)"
