...
- **nodeId**: 빈 문자열 `""` (전체 파일)
- **clientFrameworks**: `"react"` — A10 프런트는 React 16
- **clientLanguages**: `"typescript"` — TypeScript 4.4 (`규칙/프로세스/개발환경-구성-표준.md`(forge))

⛔ **원격 Figma MCP(`mcp.figma.com`)는 비공개 파일을 못 읽는다.** 로컬 Dev Mode MCP(`127.0.0.1:3845`)를 쓴다 — 진단 절차 `규칙/프로세스/디자인도구-MCP-운영가이드.md`(forge) §1. 인자 이름이 거부되면 추정해 바꾸지 말고 도구 스키마를 먼저 확인한다.
...
### Step 3-1: ⛔ A10 이식 가능성 판정 (필수)

Figma Make 기본 산출은 **Tailwind + `components/ui/`(shadcn) 골격**이고 **A10은 그 계층이 아니다.**

| 확인 | 근거 |
|---|---|
| A10 퍼블리싱은 SCSS + luna-orbit(OBT) 오버라이드 — **BEM·유틸리티퍼스트(Tailwind) 아님** | `규칙/프로세스/퍼블리싱-코드스타일.md`(forge) §2 |
| 대응 OBT 컴포넌트와 `pub*` 공통 프리셋이 있는지 대조 | 같은 표준 §3.1·§4 |
| **prop·컴포넌트명 추정 금지** — 실사용례 grep 확인 | 같은 표준 §6 |
| 모바일용 컴포넌트를 웹 화면에 그대로 인스턴스화하지 않는다 | `규칙/프로세스/와이어프레임-작성-표준.md`(forge) |

→ 이식 불가 컴포넌트는 발견 이슈에 **Major** 로 올린다.
...
1. `styles/theme.css` 또는 유사 파일에서 CSS 변수 추출 · Figma 변수는 `get_variable_defs` 계열로 직접 조회
2. `tailwind.config` 등 Figma Make 쪽 빌드 설정에서 커스텀 토큰 추출
3. **A10 현행 토큰과 비교 — `tailwind.config` 아님** (A10에는 Tailwind가 없다). 대조 3곳:
   - designcenter-figma `tokens/source/{base,semantic}/*.json` — W3C DTCG + Tokens Studio, base→semantic 2계층, Style Dictionary 3 빌드 (`Amaranth10/_소스분석/designcenter-figma-분석.md`(forge))
   - `klago-pub-www/src/www/css/klago.scss` — CSS 변수(`--`) 108건 / Sass 변수 0건 (`규칙/프로세스/퍼블리싱-코드스타일.md`(forge) §6)
   - `참고자료/디자인자산/화면설계서표준-디자인토큰.json`(forge)
4. 토큰 매핑(5 카테고리 color·typography·spacing·radius·shadow + WCAG AA 대비) + 컴포넌트별 className 변경 목록 작성
5. 개발팀 지시서에 반영 — 하류 WEAVER 는 **토큰 적용률 90% 이상**으로 채점하므로 미매핑 항목을 표에 남긴다
