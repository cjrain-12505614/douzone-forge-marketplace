...
본 SKILL 은 `규칙/프로세스/에이전트-구성-역할-정의.md`(forge) SSoT §3 의 운영본이다.

## 위임 경계 (비강제 opt-in — H4)

본 에이전트(개발 구현)는 **forge 맥락 브리지를 켠 개발 세션**(개발 폴더에 `_forge` 심링크가 있는 듀얼 마운트)에서만 위임 대상이다. forge 기획·설계·분석(PM) 단독 세션에는 소스 코딩 대상이 없으므로 **자동 위임하지 않는다** — 사용자가 명시 호출할 때만 동작. (근거: 개발자 하네스 설계안 `참고자료/리포트/2026-06-20-개발자하네스-설계안.md`(forge) §3 4단계 · 비강제 철학)
...
## A10 FE 번들러 제약 (실측 반영 — H4)

A10 프런트엔드는 일반 단독 React 앱이 아니라 **번들러(klago-ui-micro)가 마이크로 모듈(klago-ui-{모듈}-micro)을 묶어 구동**하는 구조다. 구현·구동 시 아래 제약을 따른다 — 상세·정본은 `규칙/프로세스/개발환경-구성-표준.md`(forge) SSoT(단일 출처) 및 `dz-module-devenv` 스킬 참조(여기서 값을 중복 정의하지 않는다).

- **번들러 경유 구동** — 모듈 micro 단독이 아니라 번들러 klago-ui-micro 작업공간에서 빌드·구동.
- **node-sass → dart-sass 폴백** — Apple Silicon(맥)에서 node-sass 빌드 실패 시 dart-sass로 대체.
- **Node 버전 제약(Node 16 계열)** — 상위 Node에서 빌드 실패 가능, 표준 버전 준수.
- **로컬 프록시(setupProxy.js)** — 단일 포트(:3000)에서 `/{모듈}`→로컬 BE·`/system`→develop 자동 라우팅(브라우저 CORS 불요).
...
- SSoT (표준 정의): `규칙/프로세스/에이전트-구성-역할-정의.md`(forge) §3 NOVA 행
