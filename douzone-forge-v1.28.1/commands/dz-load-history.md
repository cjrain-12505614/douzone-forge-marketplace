...
예: `Amaranth10/법무관리(LTE)/phase-2` → `Amaranth10/법무관리(LTE)/history/phase-2.md`

**3단계 — 고객 요구사항 이력이 지정된 경우**
`{모듈명}/history/requests/` 아래 해당 파일 읽기
예: `Amaranth10/법무관리(LTE)/requests/가온` → 파일명에 "가온"이 포함된 파일 탐색 후 읽기
...
파일이 없으면 `dz-history-manager` 스킬의 파일 형식 절 기준으로 새로 생성할 수 있다고 안내한다.
