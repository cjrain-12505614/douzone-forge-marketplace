...
2026-04-17 PRJ-005 분할 안내 댓글 (@{이름} 대상) 자동화에서 검증된 패턴을 표준화.

> 변천사: v0.1.0 표준화(2026-04-17 검증 패턴) · **v0.2.0 dz-oneffice-kit OF 인용 전환(2026-07-14, T4 마이그레이션 8단계) — 등록 버튼 좌표를 정본 `OF.COORD.commentSubmit` 인용으로 전환(인라인 좌표는 폴백 사본으로 1 마이너 버전 병행). 멘션 신뢰 키 시퀀스는 이 스킬 고유 로직으로 불변.**

## 원피스 DOM 배관 — 정본 dz-oneffice-kit(OF 라이브러리) 인용

이 스킬의 원피스 DOM 조작(중첩 iframe 접근·편집모드 판정/전환·저장 등 배관)은 정본
`dz-oneffice-kit`의 OF 라이브러리를 사용한다. 실행 시 그 SKILL.md의 OF 코드블록을
`javascript_tool`로 원피스 탭에 1회 주입(`window.OF` 상주)한 뒤 `OF.*` 를 호출한다.
셀렉터·좌표·프리셋은 `OF.SEL`/`OF.COORD`/`OF.PRESET` 단일 정의를 따른다.

**단, 이 스킬은 OF 접점이 최소다.** 댓글은 본문 편집기 iframe(`#dzeditor_0`) 안이 아니라
**바깥 문서의 댓글 패널(`#rpText`)** 을 대상으로 하므로, OF의 중첩 iframe 배관·편집모드
가드·주입/저장 함수는 쓰지 않는다(저장 절차 자체가 없고, 댓글 "등록"은 편집기 저장과
별건이다). 이 스킬이 실제로 인용하는 OF 자산은 **단 하나** —

- **`OF.COORD.commentSubmit`** (`[1411, 290]`) — 등록 버튼 좌표 폴백. 좌표 단일 정의는
  키트에만 있으므로, 값이 바뀌면 키트 한 곳만 고치면 6종에 전파된다.

**고유 자산은 그대로 둔다**: 멘션 신뢰 키 시퀀스(Step 5 — `@이름<공백>` → `Backspace` 2회
신뢰 네이티브 키 이벤트)는 이 스킬 고유 로직이며 OF에 넣지 않는다(OF는 DOM 배관만).
...
3. **등록 버튼 좌표 `(1411, 290)` 하드코딩은 1920×1080 / 130% 줌 환경 한정.**
   ...
   `onclick*="saveComment"`). 이 좌표는 정본 `dz-oneffice-kit`의 `OF.COORD.commentSubmit`
   에 단일 정의돼 있다 — 실행 시 OF를 주입했다면 하드코딩 대신 그 값을 참조한다(아래 인라인은
   되돌림 안전판용 폴백 사본).
...
- 선택된 멘션 태그 삽입 결과:
  ...
  <input type="button" id="mention" name="mentionSpan"
         ...
         empseq="344710302" value="{이름}">
  ```
- 등록 버튼: 화면 우상단 좌표 `(1411, 290)` — **좌표 방식이 현행 정본**이다(동작 확인됨). DOM 셀렉터 전환은 「후속 개선」 항목이며 지금 필요한 작업이 아니다.
...
```
{{@{이름}}} PRJ-005 분할 완료했습니다. 확인 부탁드립니다.
```
...
실패 시 좌표 fallback (**OF 정본 사용 권장, 아래는 폴백 사본** — 좌표 단일 정의는
`dz-oneffice-kit`의 `OF.COORD.commentSubmit = [1411, 290]`. 실행 시 OF를 주입했다면 하드코딩
대신 `OF.COORD.commentSubmit`을 읽어 그 좌표로 클릭한다. 되돌림 안전판으로 인라인 좌표는
1 마이너 버전 병행 유지):

```
mcp__Claude_in_Chrome__left_click(1411, 290)   // = OF.COORD.commentSubmit 폴백 사본
```
...
| 등록 버튼 좌표 (1920×1080, 130%) | `(1411, 290)` = `OF.COORD.commentSubmit` (정본: dz-oneffice-kit) |
...
## 후속 개선 — 백로그 _(2026-07-14 기재 · 2026-08-14 확인)_

> 아래는 **지금 스킬을 쓰는 데 걸림돌이 아니다.** 현행 방식으로 동작하며, 여유가 생길 때 다듬을 항목을 모아 둔 것이다.

- 등록 버튼 DOM 셀렉터 확정 (현재는 좌표 폴백으로 동작)
...
- **dz-oneffice-kit** — 원피스 DOM 배관 정본(OF 라이브러리). 이 스킬은 등록 버튼 좌표
  폴백 `OF.COORD.commentSubmit` 하나만 인용한다(멘션 신뢰 키 시퀀스는 이 스킬 고유).
- **dz-oneffice-reader** — 댓글 작성 전 문서 내용 확인
- **dz-oneffice-writer** — 댓글을 쓸 문서가 먼저 만들어져야 할 때

## 관련 Command

- **`/dz-post-comment`** — 이 스킬을 래핑한 슬래시 커맨드

<!-- auto: 차민수의 자비스 2026-07-14 dz-oneffice-kit OF 인용 전환 (T4 마이그레이션 8단계 = C comment). OF 접점 최소(COORD.commentSubmit 폴백만) — 멘션 신뢰 키 시퀀스는 고유 불변, 인라인 좌표는 되돌림 안전판으로 폴백 사본 병행. v0.1.0→v0.2.0 -->
