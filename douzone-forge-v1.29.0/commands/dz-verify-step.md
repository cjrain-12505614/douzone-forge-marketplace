...
3. **테스트 결과 집계**
   ...
   - ⚠️ **`0 tests` 는 통과가 아니라 미수집 신호일 수 있다** — `useJUnitPlatform()`(JUnit 5 실행기)과 JUnit 4 라이브러리가 함께 선언되고 vintage 엔진이 없으면 JUnit 4 테스트가 조용히 수집되지 않는다. 작성 규약·실행기 조합 확인은 `/dz-unit-test` §0.
