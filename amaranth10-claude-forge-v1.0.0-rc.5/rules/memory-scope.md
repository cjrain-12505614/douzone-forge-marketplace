---
name: memory-scope
description: 자비스 메모리는 차민수 개인 사람 기준만. 범용 룰·운영 워크플로우는 SSoT 이전.
...
자비스 메모리는 차민수 개인 사람 기준만 잔존한다. 범용 룰·운영 워크플로우·도구 사용법·범용 매핑·자비스 학습 룰은 douzone-forge SSoT (`규칙/프로세스/`) 로 이전한다.

### 메모리 허용·금지 (자비스-운영-룰 §6.2 인용)

**허용**:
- 차민수 본인 역할·호칭 (SBUnit장, cjrain)
- 차민수 부서 사람 매핑 (SBUnit 21명 ID·R&R)
- 차민수 직접 진행 PRJ 비전·결정·KPI
- 차민수 본인 일일보고 메타
...
- 예외: 본문에 "본인·내 ~·차민수·cjrain·본인 R&R·본인 KPI·본인 일일보고" 매칭 시 통과

## Why

Forge 는 사내 전사 배포 플랫폼 (Beta 3인 → SBUnit 21명 → 사내 4,157명). 자비스 메모리는 차민수 로컬에만 존재하므로 배포 사용자에게 부재. 범용 룰을 메모리에 두면 격차 발생 → 배포 사용자에게 부재할 본문은 SSoT 인용으로 대체.
...
- "본인·내 ~·차민수·cjrain·본인 R&R·본인 KPI·본인 일일보고·내 역할·내 호칭·내 부서·내 상사" 매칭 시 통과
...
- Phase U 본질 — 메모리 = 차민수 사람 기준만 (차민수 직접 룰 확정)
